...
{
Our goal is to say whether various functors are representable (of the form $X\mapsto\smset(A,X)$ for some $A$, called the representing set).
\begin{enumerate}
	\item The identity functor $X\mapsto X$ is represented by $A=\1$.
	\item The constant functor $X\mapsto\2$ is not representable.
	\item The constant functor $X\mapsto\1$ is representable by $A=\0$.
	\item The constant functor $X\mapsto\0$ is not representable.
	\item The functor $X\mapsto X^\nn$ is represented by $A=\nn$.
	\item The functor $X\mapsto 2^X$ is not representable.
\end{enumerate}
}
