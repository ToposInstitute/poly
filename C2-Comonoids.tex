...
In particular, given categories $\cat{C}_i$ (for each $i \in I$ for a small set $I$), their product is the limit of the canonical monoidal functor from the free monoidal category generated by distinct comonoid objects $c_i$ for each $i \in I$ to $\smcat^\sharp$ that sends each $c_i$ to $\cat{C}_i$.
\end{proposition}
\begin{proof}
