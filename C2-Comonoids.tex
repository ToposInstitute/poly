...
The counits $[i] \to []$ and comultiplications $[i] \to [i,i]$ for each $i \in I$ generate all the morphisms of $\Delta_I\op$ via composition and taking the tensor product, while satisfying associativity and left and right unit laws.
...
\begin{example}[Binary products in $\smcat^\sharp$]
...
% So to specify ___, it suffices to specify ___.
\end{example}



\begin{example}[Unexpectedly-many objects]\label{ex.unexpectedly_many_objects}
Let $\cat{C}\coloneqq\fbox{$\LMO{A}\To{f}\LMO{B}$}$ be the walking arrow category. The product $\cat{C}\times^\sharp\cat{C}$ of $\cat{C}$ with itself, taken in $\smcat^\sharp$, has infinitely many objects. Namely, it has an object for every pair of sequences $(s_1, s_2)$, finite or infinite, that one can write in the alphabet $\{A_1, A_2, B_1, B_2\}$ subject to the following conditions:
\begin{itemize}
