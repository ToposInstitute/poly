...
An object in $\cat{C}\times^\sharp\cat{D}$ is roughly-speaking a tree for which each node is either an object of $i\in\cat{C}$ or an object of $j\in\cat{D}$, and where the branching at that node is the set of emanating morphisms from $i$ or $j$ respectively; the tree must respect identities and composites in $\cat{C}$ and $\cat{D}$, as we'll explain.
...
Let $\cat{C}\coloneqq\fbox{$\LMO{A}\To{f}\LMO{B}$}$ be the walking arrow category. The product $\cat{C}\times^\sharp\cat{C}$ of $\cat{C}$ with itself, taken in $\smcat^\sharp$, has infinitely many objects. Namely, it has an object for every sequence $s$, finite or infinite, that one can write in the alphabet $\{A_1,A_2,B_1,B_2\}$ subject to the following conditions
\begin{itemize}
	\item after $A_1$ comes either $A_2$ or $B_2$,
	\item after $A_2$ comes either $B_1$ or $A_1$,
	\item after $B_1$ or $B_2$, the sequence stops.
\end{itemize}
For example, here are two objects in $\fbox{$\LMO{A}\To{f}\LMO{B}$}\times\fbox{$\LMO{A}\To{f}\LMO{B}$}$:
\[
(A_1,A_2,A_1,A_2,\ldots)\qquad (A_2,A_1,A_2,A_1,B_2)
\]
\end{example}
