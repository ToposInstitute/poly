...
In this chapter we will investigate a remarkable category called $\poly$. We will see its intimate relationship with dynamic processes, decision-making, and the storage and transformation of data. But our story begins with something quite humble: middle school algebra.
\begin{align}\label{eqn.polynomial1892}
\yon^\2+\yon+\1 \quad&\quad
\textit{polynomial}
\intertext{
All our polynomials (or just \emph{polys}) will involve one variable, $\yon$, chosen for reasons we'll explain soon. Polynomials in one variable can be depicted as a set of mini-trees:
}
\label{eqn.poly1892}
...
\end{align}
Each mini-tree in \eqref{eqn.poly1892} corresponds to a power of $\yon$ in the polynomial given in \eqref{eqn.polynomial1892}.
In particular, the mini-tree with $2$ leaves corresponds to $\yon^\2$; the mini-tree with $1$ leaf corresponds to $\yon$, which can be rewritten as $\yon^\1$; and the mini-tree with no leaves corresponds to $\1$, which can be rewritten as $\yon^0$.
More technically, these mini-trees—rooted trees of which all leaves are children of the root—are called \emph{corollas}.
So what we're calling an \emph{arena}, in this chapter, is a set of corollas. 

Intuitively, one might think of each corolla as representing a \emph{decision}. Associated to every decision is a set of \emph{options}. The three decisions we exhibit in \eqref{eqn.poly1892} are particularly interesting; they respectively have two options, one option, and no options. Having two options is familiar from life---it's the classic yes/no decision---as well as from Claude Shannon's Information Theory. Having one option is also familiar theoretically and in life: ``sorry, ya just gotta go through it.'' Having no options is when you actually don't get through it: it's ``the end.'' While the corollas $\1,\yon,$ and $\yon^\2$ are each interesting as decisions, their sum $\yon^\2+\yon+\1$ has very little theoretical interest; it's just a first example.
...
in the associated arena. Intuitively, there are 42 situations in which you have a decision with three options.
...
Here is a table of terminology, capturing four different perspectives from which we may view our objects of study. The first column is book-wide terminology, the second is algebraic terminology about polynomials as in \eqref{eqn.polynomial1892}, the third is about the pictures of corollas as in \eqref{eqn.poly1892}, and the fourth column is intuitive terminology.
...
Direction $c\in p[i]$ & Element of $p[i]$ & Leaf & Option
\end{tabular}
\end{equation}


\begin{exercise}\label{exc.arenas}
...
Consider the following three trees, the first two of which are infinite (though that's hard to draw):
\[
\begin{tikzpicture}[trees]
\begin{scope}[
...
Throughout this text, in any category, given objects $A, B, C$ and morphisms $f \colon A \to B$ and $g \colon B \to C$, we denote their composite morphism $A \to C$ as $f \then g$ (instead of the usual $g \circ f$).
...
Imagine you are at a table, and you see some chalk. An amazing thing about reality is that you will then have the chalk, in the sense that you can move it around. How is this modeled in $\poly$?
...
\]
If both you and the chalk can also receive pressure from the other, we could define your interfaces as
\[
...
So now we've got you and the chalk in a closed system together. Let's add some dynamics to the chalk and call it a day. The chalk is a pretty simple character: it has four states $C=PH$. We will give a dynamical system $C\yon^C\to\textit{chalk}$ with states $C$ and chalk-interface, i.e.\ a map
...
\footnote{You might want the falling down to be not discrete. In order to get continuous time, we need to use cofunctors, which are coming in \cref{chapter2}.}
...
\begin{exercise}[Conway's Game of Life]
