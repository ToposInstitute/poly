...
\[
    (\mu^{n-1})_1 \colon m(\1)^n \to m(\1)
\]
sends every $n$-tuple of positions $(i_j)_{j \in \ord{n}}$ to a single position $i'$, while the on-directions function
\[
    (\mu^{n-1})^\sharp_{(i_j)_{j \in \ord{n}}} \colon m[i'] \to \prod_{j \in \ord{n}} m[i_j]
\]
at that $n$-tuple sends every direction at $i'$ to $n$ directions of $m$, one at every position in the $n$-tuple.
