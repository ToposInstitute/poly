...
\paragraph{Coproducts of polynomials.}
...
\paragraph{Polynomial morphisms, concretely.}
...
Alternatively, we invoke the Day convolution to obtain the monoidal structure $(\yon, \otimes)$ directly in \cref{prop.day}.
\end{proof}
...
Formally we might denote it by $\mu^{n-1}$, so that $\mu=\mu^1\colon m\otimes m\to m$,\footnote{Here $\mu\inv = \eta$ and $\mu^n = (\mu^{n-1} \otimes \id_m) \then \mu$ for $n \in \nn$, so that $\mu^0 = \id_m$ and $\mu^1 = \mu$.} but for typographical reasons it is nicer to just write $\mu$ for all of them.
...
\[\ell\coloneqq\1+\yon+\yon^\2+\cdots,\]
the ``list'' polynomial. The first monoid structure we'll discuss is commutative, the second is not. We will check that they satisfy the necessary conditions of an $\otimes$-monoid in \cref{exc.list_monoids}.

We get a commutative $\otimes$-monoid via the Cartesian map $\yon\to\ell$ sending $\yon\mapsto\yon$ and the Cartesian map $\ell\otimes\ell\to\ell$ that on positions sends $(i,j)\mapsto i\times j$ and on directions is the pair of coordinate projections. % **Haven't defined Cartesian maps yet

We get a noncommutative $\otimes$-monoid via the map $\yon\to\ell$ sending $\yon\mapsto\1$ and the map $\ell\otimes\ell\to\ell$ that on positions sends $(i,j)\mapsto i+j$ and on directions is given by
\[
d\mapsto \big(\min(d,i),\max(0,d-i)\big).
\]
For example, suppose the position is $i=3, j=5$; it is sent to position $8$. Given a direction $d\leq 8$ there, it is distributed to a pair $(d_1,d_2)$ of directions $d_1\leq 3$ and $d_2\leq 5$. One can imagine a measuring cup with height 8 and a line at 3: if $0\leq d\leq 3$ then $d_1=d$ and $d_2=0$; if $3\leq d\leq 8$ then $d_1=3$ and $d_2=d-5$.
\end{example}

\begin{exercise}\label{exc.list_monoids}
\begin{enumerate}
	\item Check that the first $\otimes$-monoid structure announced in \cref{ex.list_monoids} really satisfies the necessary equations.
	\item Check that the first $\otimes$-monoid structure announced in \cref{ex.list_monoids} really satisfies the necessary equations.
\qedhere
\end{enumerate}
\end{exercise}
...
\begin{exercise}
Find a $\otimes$-monoid structure carried by the polynomial $1+\yon$.
\end{exercise}
...
\begin{enumerate}
	...
	\begin{tikzcd}
    	...
    	p[i] \ar[from=d, "!"'] \\
    	\varnothing \ar[from=r, "!"] &
    	\varnothing \ar[ul, phantom, very near end, "\lrcorner"]
    \end{tikzcd}
    \]
...
Since $\eta_p$ is an isomorphism on positions and $p\mapsto p(\1)$ is a right adjoint, the map $f^*p\to A$ is also an isomorphism on positions and $\fun{cart}_1 = f$.
Furthermore, for each position $a \in A$, the on-directions function $f^\sharp_a \colon \0 \to \0$ is trivially an isomorphism; since the set of directions of $f^*p$ at $a$ is constructed by a pushout (see \cref{ex.pullbacks_in_poly}), each on-directions function $\fun{cart}^\sharp_a\colon p[f(a)]\to(f^*p)[a]$ is an isomorphism, too.
We'll see this as part of a bigger picture in \cref{prop.basechange,thm.triple_adjoint_basechange}.
