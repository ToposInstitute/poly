...
Each corolla in \eqref{eqn.forest_example} corresponds to a \emph{pure-power summand} of the form $\yon^A$ in the polynomial given in \eqref{eqn.poly_example}: the corolla with $2$ leaves corresponds to $\yon^\2$; the two corollas that each have $1$ leaf correspond to the two copies of $\yon = \yon^\1$; and the corolla with no leaves corresponds to $\1 = \yon^0$.
...
\end{tikzpicture}
\]
This suggests yet another depiction of the polynomial \eqref{eqn.poly_example}: as a \emph{dependent set} $(p[i])_{i \in I}$.
...
Here is a table of terminology, capturing five different perspectives from which we may view our objects of study. The first column the terminology we just introduced for polynomials as dependent sets, as in \eqref{eqn.arena_example}; the second is algebraic terminology, as in \eqref{eqn.poly_example}; the third is the pictorial terminology of corolla forests, as in \eqref{eqn.forest_example}; the fourth column is dynamical systems terminology, which we will explore in \cref{**}; and the fifth column is decision-making terminology, which we will introduce in \cref{sec.poly.intro.dec} and further develop in \cref{**}.
...
%---- Subsection ----%
\subsection{Dynamical systems} %\label{sec.poly.intro.dyn_sys}
...
%-------- Section --------%
\section{Introduction to $\poly$ as a category}%\label{sec.poly.as_cat}
...
% 	\item Read the formula for $\otimes$ and $\ihom{-,-}$ in \eqref{eqn.parallel_def} and \eqref{eqn.dir_hom} and find a formula for a map $p\otimes\ihom{p,\yon}\to \dot{p}$ that works for any $p\in\poly$.
...
% 	\item Read the formula for $\otimes$ and $\ihom{-,-}$ in \eqref{eqn.parallel_def} and \eqref{eqn.dir_hom} and find a formula for a map $p\otimes\ihom{p,\yon}\to p'$ that works for any $p\in\poly$.
...
% The above exercise tells us how to interpret commutative diagrams in $\poly$.
% Given polynomials $p, q, r$ and natural transformations $f \colon p \to q, g \colon q \to r,$ and $h \colon p \to r$ with associated arena morphisms $(f_1, f^\sharp)$, $(g_1, g^\sharp)$, and $(h_1, h^\sharp)$, the diagram
% \[
% \begin{tikzcd}
...
% \end{tikzcd}
% \]
% commutes in $\poly$ if and only if the diagram
% \[
% \begin{tikzcd}
...
% \end{tikzcd}
% \]
% commutes in $\smset$ and, for each $i \in p(\1)$, the diagram
% \[
% \begin{tikzcd}
...
%     \item Given $p,q,r \in \poly$, we use \eqref{eqn.main_formula} and  \eqref{eqn.poly_times} to verify that $\poly(r, p \times q) \iso \poly(r, p) \times \poly(r, q)$.
%     We have that
%     \begin{align*}
...
\begin{equation} \label{eq.prod_univ_prop}
\begin{tikzcd}
	r \ar[d, "f"'] \ar[r, "g"] \ar[dr, "h", dashed] & q \\
	p & pq. \ar[l, "\pi"] \ar[u, "\phi"'] \\
\end{tikzcd}
\end{equation}
In order for \eqref{eq.prod_univ_prop} to commute, it must commute on positions---that is, the following diagram of sets must commute:
\begin{equation} \label{eq.prod_univ_prop_pos}
\begin{tikzcd}
	r(\1) \ar[d, "f_\1"'] \ar[r, "g_\1"] \ar[dr, "h_\1", dashed] & q(\1) \\
	p(\1) & (pq)(\1). \ar[l, "\pi_\1"] \ar[u, "\phi_\1"'] \\
\end{tikzcd}
\end{equation}
But since $(pq)(\1) \iso p(1) \times q(\1)$ along with the projections $\pi_\1$ and $\phi_\1$ form the product of $p(\1)$ and $q(\1)$ in $\smset$, there exists a unique $h_\1$ for which \eqref{eq.prod_univ_prop_pos} commutes.
Hence $h$ is uniquely characterized on positions.
In particular, it must send each $k \in r(\1)$ to the pair $(f_\1(k), g_\1(k)) \in (pq)(\1)$.

Then the commutativity of \eqref{eq.prod_univ_prop} on directions suggests that for every $k \in r(\1)$, the following diagram of sets commutes:
\begin{equation} \label{eq.prod_univ_prop_dir}
...
\end{tikzcd}
\end{equation}
As $(pq)[(f_\1(k), g_\1(k))] \iso p[f_\1(k)] + q[g_\1(k)]$ along with the inclusions $\pi^\sharp_{(f_\1(k), g_\1(k))}$ and $\phi^\sharp_{(f_\1(k), g_\1(k))}$ form the coproduct of $p[f_\1(k)]$ and $q[g_\1(k)]$ in $\smset$, there exists a unique $h^\sharp_k$ for which \eqref{eq.prod_univ_prop_dir} commutes.
Hence $h$ is also uniquely characterized on directions, so it is unique overall.
Moreover, we have shown that we can define $h$ on positions so that \eqref{eq.prod_univ_prop_pos} commutes, and that we can define $h$ on directions such that \eqref{eq.prod_univ_prop_dir} commutes.
As the commutativity of \eqref{eq.prod_univ_prop_pos} and \eqref{eq.prod_univ_prop_dir} together imply the commutativity of \eqref{eq.prod_univ_prop}, it follows that there exists $h$ for which \eqref{eq.prod_univ_prop} commutes.
\end{solution}
\end{exercise}

\paragraph{The parallel product} \label{sec.parallel_prod}
...
%-------- Section --------%
\section{Dynamical systems using $\poly$}\label{sec.poly.dyn_sys}
...
\niu{More about regular languages, either in this section or the next. Maybe this section should just be called ``deterministic finite state automata.''}
...
Another way to combine two polynomials is by taking their parallel product, as in \cref{sec.parallel_prod}.
As with the categorical product, the parallel product of monomials is still a monomial.
So parallel products give us another way to create new Moore machines from old ones by combining them.
...
\paragraph{Composing morphisms: wrapper interfaces.} % migrating interfaces? overlaying interfaces?
...
\paragraph{Situations.} % Fields? Enclosures?
...
\end{equation}
so a situation $\gamma \in \Gamma(p)$ can be thought of as a dependent function that takes every position $i\in p(\1)$ and returns a direction $\gamma(i)\in p[i]$.
The idea for the name is that a situation dictates what you'll see (the input you receive), given anything you might do (the output you provide). If the situation is that you're at a party, then that dictates what direction you'll receive, given what position you take.
...
\begin{exercise}
\begin{enumerate}
	\item State and prove a generalization of \cref{prop.situations2} for $n$-many polynomials $p_1,\ldots,p_n\in\poly$.
	\item Generalize the ``idea'' statement between \cref{prop.situations2} and its proof.
\qedhere
\end{enumerate}
\begin{solution}
\begin{enumerate}
    \item We generalize \cref{prop.situations2} for $n$ polynomials as follows.
    ...
    \]
    The $n=1$ case is clear, and the $n=2$ case is given by \cref{prop.situations2}.
    Then by induction on $n$, we have
    \begin{align*}
        \Gamma\left(\bigotimes_{i=1}^n p_i \right) &\iso \smset\left(p_n(\1), \Gamma\left(\bigotimes_{i=1}^{n-1} p_i \right)\right) \times \smset\left(\prod_{i=1}^{n-1} p_i(\1), \Gamma(p_n)\right) \tag{\cref{prop.situations2}} \\
...
\begin{exercise}
We will use \cref{prop.situations2} to consider the interaction $\varphi$ between \textit{you} and \textit{chalk} from \cref{ex.pickup_chalk} as a pair of functions $\textit{you}(\1)\to\Gamma(\textit{chalk})$ and $\textit{chalk}(\1)\to\Gamma(\textit{you})$.
\begin{enumerate}
...
\begin{solution}
\cref{prop.gamma_pres_coproduct} follows directly from \cref{exc.poly_coprods}: we have that $\Gamma(\0) = \poly(\0,\yon) \iso \1$ since $\0$ is initial in $\poly$, and $\Gamma(p + q) = \poly(p+q,\yon) \iso \poly(p+q,\yon) = \Gamma(p) \times \Gamma(q)$ since $+$ gives coproducts in $\poly$. 
\end{solution}
\end{exercise}
...
\end{enumerate}
\begin{solution}
Our goal is to compute $\ihom{q,r}$ for various values of $q, r \in \poly$.
\begin{enumerate}
    \item If $q \coloneqq \0$, then $q(\1) \iso \0$, so by \eqref{eqn.dir_hom}, we have that $\ihom{q,r}$ is an empty product.
    Hence $\ihom{q,r} \iso \1$.
    \item If $q \coloneqq \1$, then $q(\1) \iso \1$ and $q[1] \iso \0$, so by \eqref{eqn.dir_hom}, we have that $\ihom{q,r} \iso r \circ (\0\yon) \iso r(\0)$.
    \item If $q \coloneqq \yon$, then $q(\1) \iso \1$ and $q[1] \iso \1$, so by \eqref{eqn.dir_hom}, we have that $\ihom{q,r} \iso r \circ (\1\yon) \iso r$.
	\item If $q \coloneqq A$ for $A \in \smset$, then $q(\1) \iso A$ and $q[j] \iso \0$ for every $j \in A$, so by \eqref{eqn.dir_hom}, we have that $\ihom{q,r} \iso \prod_{j \in A} (r \circ (\0\yon)) \iso r(\0)^A$.
	\item If $q \coloneqq A\yon$ for $A\in\smset$, then $q(\1) \iso A$ and $q[j] \iso \1$ for every $j \in A$, so by \eqref{eqn.dir_hom}, we have that $\ihom{q,r} \iso \prod_{j \in A} (r \circ (\1\yon)) \iso r^A$.
	\item If $q\coloneqq\yon^\2+\2\yon$ and $r\coloneqq\2\yon^\3+\3$, then by \eqref{eqn.dir_hom},
	\begin{align*}
...
\begin{align*}
    ...
    &\iso \sum_{f_1 \colon q(\1) \to r(\1)} \, \prod_{j \in q(\1)} (q[j]\yon)^{r[f_1(j)]} \tag{\cref{prop.push_prod_sum_set_indep}} \\
    ...
    &\iso \sum_{f \colon q \to r} \yon^{\sum_{j \in q(\1)} r[f_1(j)]}. \tag{\cref{prop.poly_maps_prod_sum}}
\end{align*}
\end{solution}
\end{exercise}
...
Alternatively, we can apply \cref{prop.dirichlet_closure}.
...
\begin{enumerate}
	\item Obtain the evaluation map $\fun{eval}\colon \ihom{p,q}\otimes p\too q$ from \eqref{eqn.poly_closure_brackets}. \label{exc.eval_dirichlet.explicit}
...
Finally, the fourth puts a copy of bimorphic lenses inside $\poly$, as we saw in \cref{subsec_prepare_dyn} (page~\pageref{page.bimorphic_lens}).
...
Replacing $p$ with $\yon^B$ in \cref{prop.two_var_adj}, we obtain the following using the Yoneda lemma.
...
\]
Replacing $p$ with $\yon^B$ in \cref{prop.two_var_adj}, we obtain the natural isomorphism
\[
...
\]
Then the desired natural isomorphism follows from \cref{exc.adjoint_quadruple} \cref{exc.adjoint_quadruple.pos_const} and \cref{prop.yoneda_left_adjoint}.
\end{proof}
...
Conversely, suppose that $f_1$ is an epimorphism and that, for each $j\in q(\1)$,0 the function $f^\flat_j$ is a monomorphism. 
...
\]
from \eqref{eqn.useful_misc472} must be a monomorphism if it exists, but we know from the solution to \cref{exc.practice_sum_prod} \cref{exc.practice_sum_prod.useful} that if $f$ exists, then the corresponding $f^\flat$ must exist as well.
...
Let's begin with something fairly simple: that $+$ gives a coproduct in $\poly$. We could have said this elementary fact much earlier, but we were in a rush to get to the applications in \cref{sec.poly.dyn_sys}.

\begin{exercise} \label{exc.poly_coprods}
Show that the category $\poly$ has finite coproducts as follows.
\begin{enumerate}
	\item \label{exc.poly_coprods.init} Show that $\0$ is an initial object in $\poly$, i.e.\ that for any polynomial $p$ there is a unique morphism $\0\to p$.
...
\end{equation}
which is manifestly a sum of representables, i.e.\ a polynomial, and which one can check satisfies the universal property of the coproduct; see \cref{exc.check_coprod_prod}.
...
\end{equation}
Again, it is a sum of representables, so a polynomial, and one can check that it satisfies the universal property of products; see \cref{exc.check_coprod_prod}.
\end{proof}
...
Recall that for any polynomial $p$, there is a corresponding function $\pi_p\colon\dot{p}(\1)\to p(\1)$, i.e.\ the set of directions mapping to the set of positions. A map of polynomials $(f_1,f^\sharp)\colon p\to q$ can then be described as a function $f_1\colon p(\1)\to q(\1)$ and a function
