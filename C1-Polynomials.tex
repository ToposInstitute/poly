...
\begin{exercise}
Given a representable polynomial $\yon^S$ for a set $S$ and another polynomial $q$, show that there is an isomorphism % Do I want to write out q??
\[
    \poly(\yon^S, q) \iso q(S)
\]
that is natural in both $S$ and $q$. % What does it mean to be natural in S, huh??
