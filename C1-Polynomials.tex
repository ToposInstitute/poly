...
\begin{enumerate}
    ...
    \item To show that $\prod_{i \in I} X_i \cong \{s : I \to \sum_{i \in I} X_i \mid s \then \pi_1 = \id_I\}$, we will exhibit maps in either direction and show that they are mutually inverse.
    ...
    Hence $f \mapsto s_f$ is a map $\prod_{i \in I} X_i \to \{s : I \to \sum_{i \in I} X_i \mid s \then \pi_1 = \id_I\}$.
    In the other direction, for each section $s \colon I \to \sum_{i \in I} X_i$ we have $\pi_i(s(i)) = i$ for all $i \in I$, so we can write $s(i)$ as an ordered pair $(i, x_i)$ with $x_i \in X_i$.
    So we can define a function $f_s \colon (i \in I) \to X_i$ that sends each $i \in I$ to $x_i$ (i.e. $\pi_2(s(i))$).
    Then $s \mapsto f_s$ is a map $\{s : I \to \sum_{i \in I} X_i \mid s \then \pi_1 = \id_I\} \to \prod_{i \in I} X_i$.
...
\]
We denote by $\0,\1\colon\smset\to\smset$ the constant functors that respectively assign $\0$ and $\1$ to each $X\in\smset$.
\end{definition}
