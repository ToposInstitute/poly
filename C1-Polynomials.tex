...
\begin{definition}
A \emph{$\otimes$-monoid in} $\poly$ consists of a polynomial $m$, a position $\eta\colon\yon\to m$, and a map $\mu\colon m\otimes m\to m$ that form a monoid in $(\poly,\yon,\otimes)$.
\end{definition}
% ** Show that a map y -> m is a position of m?

These $\otimes$-monoids have a kind of swarm-like semantics, letting individuals summarize the positions of a group of subordinates and distribute instructions to those subordinates in a coherent way.
...
\end{tikzpicture}
\end{equation}
Each box in this picture is labeled $m$. Whenever a box has $n$-many subboxes, it represents a map $m^{\otimes n}\to m$, for which the monoid structure gives us a canonical map
\[m^{\otimes n} \to m.\]
Formally we might denote it $\mu^{n-1}$, so that $\mu=\mu^1\colon m\otimes m\to m$,\footnote{Here $\mu^{-1} = \eta$ and $\mu^n = (\mu^{n-1} \otimes \id_m) \then \mu$ for $n \in \nn$, so that $\mu^0 = \id_m$ and $\mu^1 = \mu$.} but for typographical reasons it is nicer to just write $\mu$ for all of them.

The on-positions function $\mu_1 \colon m(\1)^n \to m(\1)$ sends every $n$-tuple $i\coloneqq (i_1,\ldots,i_n)$ of positions, one for each subordinate, to a single position $i'\coloneqq\mu(i)$, while the on-directions function 
\[\mu^\sharp_{i} \colon m[i'] \to m[i_1]\times\cdots\times m[i_n]\] 
at that $n$-tuple $i$ distributes a direction at $i'$ to a direction for each  subordinates, at whatever position it's currently in.
...
For any set $S$, the polynomial $\yon^S$ carries a canonical $\otimes$-monoid structure. Indeed there is a unique map $\yon\to\yon^S$ to serve as the unit. The map $\mu\colon\yon^S\otimes\yon^S\to\yon^S$ amounts to a function $S\to S\times S$, for which we use the diagonal.

This construction is again (contravariantly) functorial and fully faithful.
\end{example}

\begin{example}\label{ex.swarm_reals}
Let $m\coloneqq\rr_{\geq0}\yon^{\rr_{\geq0}}$. If we take $\eta\coloneqq0$ and take $\mu_1(a,b) \coloneqq a+b$ and $\mu^\sharp_{(a,b)}(t) \coloneqq \frac{at}{a+b},\frac{bt}{a+b}$, then $(m, \eta, \mu)$ is a $\otimes$-monoid.
\end{example}

\begin{exercise}
Explain how to think of \cref{ex.swarm_reals} in terms of swarms of dynamical systems.
\begin{solution}
...
\begin{solution}
This is true in any symmetric monoidal category. The unit $\yon\to m\otimes n$ is given by the tensor of the units, 
...
\begin{solution}
We know one exists by \cref{ex.tensor_monoid_linear,ex.tensor_monoid_rep,prop.tensor_tensor_monoids}, since $\nn$ has a monoid structure given by $0,+$. The monoid structure then adds positions and just copies directions.
\end{solution}
\end{exercise}

\begin{proposition}[Free $\otimes$-monoid] \label{prop.free_parallel_monoid}
The forgetful functor from $\otimes$-monoids to $\poly$ has a left adjoint sending $p$ to $\sum_{n \in \nn} p^{\otimes n}$.
\end{proposition}
\begin{proof}
...
Given a position $\ell=(i_1,\ldots,i_n)$, the set of directions there is given by $\prod_{j\in\ord{n}} p[i_j]$.

\begin{exercise}
Explain how to think of the free monoid on a polynomial $p\in\poly$ in terms of swarms of dynamical systems.
\begin{solution}
From \cref{prop.free_parallel_monoid}, we know that the carrier of the free monoid on $p$ is
\[
	p'\coloneqq\sum_{n \in \nn} p^{\otimes n}
\]
...
While there, whenever we ask the machine to produce output, it will give us $\text{expose}(s) \in B$.
...
\end{tikzpicture}
\]
Each state is labeled by its exposed value, an element of $B\coloneqq\{b_1,b_2\}$. Each state has two outgoing arrows, one orange and one green, so $A\coloneqq\{{\color{orange}\text{orange}},{\color{dgreen}\text{green}}\}$.
...
S\yon^S\to B\yon^A,
\]
with $\text{get} \coloneqq \text{expose}$ and $\text{set} \coloneqq \text{update}$.
...
We can generalize both $S\yon^S$ and $B\yon^A$ in \cref{def.moore_machine}, though the latter is much simpler---we'll spend much of the section on it. 
After some examples, we'll briefly preview something we have already hinted at and will spend a lot of time on later, nameley what's special about $S\yon^S$.
Then we'll discuss how to model regular languages using Moore machines and how products of polynomials allow multiple interfaces to act on the same system.
...
\]
Its output type is $\rr^2$, which we might think of as a position in the 2-dimensional plane, and its input type is $[0,1]\times[0,2\pi)$, which we might think of as a command to move a certain distance in a certain direction. The map itself is given by the lens
\begin{align}
  \rr^2&\To{\text{get}}\rr^2&
  \rr^2\times[0,1]\times[0,2\pi)&\To{\text{set}}\rr^2
