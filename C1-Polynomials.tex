...
\begin{proposition}
$\poly$ is a biCartesian closed category, meaning it has sums, products, and exponential objects. It therefore supports the simply typed lambda calculus.
\end{proposition}
...
\end{align*}
where ($\ast$) follows from the fact that for any function $f_1 \colon p(\1) \to q(\1)$, the set $p(\1)$ can be written as the disjoint union of sets of the form $f_1^{-1}(j) = \{i \in p(1) \ | \ f_1(i) = j\}$ for each $j \in q(\1)$.
...
Here are two examples of $\otimes$-monoid structures carried by $\ell\coloneqq\1+\yon+\yon^\2+\cdots$, the ``list'' polynomial, one of which is commutative, the other of which is not. We will check that they satisfy the conditions in \cref{exc.list_monoids}

We get a commutative $\otimes$-monoid via the Cartesian map $\yon\to\ell$ sending $\yon\mapsto\yon$ and the Cartesian map $\ell\otimes\ell\to\ell$ that on positions sends $(i,j)\mapsto i\times j$ and on directions is the pair of coordinate projections.
...
\begin{enumerate}
	...
	\item We have that $(\yon^A)^{\yon^B} \iso (\yon^A) \circ (\yon + B) \iso (\yon + B)^A \iso \sum_{f \colon A \to \2} B^{f^{-1}(1)} \yon^{f^{-1}(2)}$.
\end{enumerate}
\end{solution}
\end{exercise}
...
\end{theorem}
\begin{proof}
Recall that a category has all limits if and only if it has equalizers and products, so by \cref{prop.poly_prods} it suffices to show that $\poly$ has equalizers. 
...
\end{theorem}
\begin{proof}
Recall that a category has all colimits iff it has coequalizers and coproducts, so by \cref{prop.poly_coprods} it suffices to show that $\poly$ has coequalizers.
