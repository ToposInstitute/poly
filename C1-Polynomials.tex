...
More precisely, any $\otimes$-monoid gives rise to a canonical morphism $\mu^{n-1} \colon m^{\otimes n} \to m$ for each $n \in \nn$, which can be thought of as an $n$-ary operation on $m$.\footnote{Here $\mu^{-1} = \eta$ and $\mu^n = (\mu^{n-1} \otimes \id_m) \then \mu$ for $n \in \nn$, so that $\mu^0 = \id_m$ and $\mu^1 = \mu$.} The on-positions function $(\mu^{n-1})_1 \colon m(\1)^n \to m(\1)$ sends every $n$-tuple of positions $(i_j)_{j \in \ord{n}}$ to a single position $i'$, while the on-directions function $(\mu^{n-1})^\sharp_{(i_j)_{j \in \ord{n}}} \colon m[i'] \to \prod_{j \in \ord{n}} m[i_j]$ at that $n$-tuple sends every direction at $i'$ to $n$ directions of $m$, one at every position in the $n$-tuple.

% \begin{exercise}
% Explain how $\otimes$ has the ``swarm-like'' semantics described above.
% \end{exercise}
