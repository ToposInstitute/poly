...
Suppose that we have a system $\varphi\colon p\otimes c\to r$, where we think of $p$ as a player and $c$ as a coin. By \cref{prop.tensor_as_pushout} we know that $\varphi$ can be identified with a commutinb diagram
\[
\begin{tikzcd}
...
We begin with a simpler version of the result.
\begin{corollary}[Interaction graphs]
...
This follows directly (use $M_i=\1$ for each $i$) from \cref{prop.mode_dep_interaction_graphs}. The intuitive reason for the condition that each polynomial $p_i$ has at least two positions is that otherwise there is no distinction between hearing $p_i$ and not hearing $p_i$.

\begin{proposition}[Mode-dependent interaction graphs]\label{prop.mode_dep_interaction_graphs}
Suppose given $p_1,\ldots,p_n\in\poly$, $M_1,\ldots, M_n\in\smset$, and $\tau_1,\ldots,\tau_n$ where $\tau_i\colon p_i\to M_i$ for each $1\leq i\leq n$. Then there is an adjunction
\[
	\adj
		{\prod_{(m_1,\ldots,m_n)\in M_1\times\cdots M_n}\Cat{SimpleGraph}(\ord{n})\op}
		{}{}
		{p_1\otimes\cdots\otimes p_n/\poly}
\]
Moreover, the left adjoint is fully faithful iff $n\eq 1$ or $\left|\left(f_i^*m\right)(\1)\right|\geq 2$ for each $1\leq i\leq n$ and $m_i\in M_i$, i.e.\ iff there are at least two positions in each mode.
\end{proposition}
\begin{proof}
**
\end{proof}
