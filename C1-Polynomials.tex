...
\begin{enumerate}
	\item Give the instructions for choosing an element of $B$ as a nested list, like we did for $A$ just below \cref{eqn.misc98339}.
...
The right hand side denotes the set of dependent functions $(d \in q[j]) \to \sum_{c \in p[i]} \1$, but since $\sum_{c \in p[i]} \1$ does not actually depend on $d$, 
