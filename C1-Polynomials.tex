...
\]
Moreover, the left adjoint is fully faithful iff $|p_i(\1)|\geq 2$ for each $1\leq i\leq n$.
\end{corollary}

This follows directly (use $M_i=\1$ for each $i$) from \cref{prop.mode_dep_interaction_graphs}.
...
\[
	\adj
		...
		{p_1\otimes\cdots\otimes p_n/\poly}
\]
Moreover, the left adjoint is fully faithful iff $\left|\left(f_i^*m\right)(\1)\right|\geq 2$ for each $1\leq i\leq n$ and $m_i\in M_i$, i.e.\ iff there are at least two positions in each mode.
\end{proposition}
\begin{proof}
**
\end{proof}
