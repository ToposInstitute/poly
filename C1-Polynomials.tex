...
\emph{$I$-indexed collection of sets} --- a \emph{dependent set} --- in the form of a functor as $X\colon I\to\smset$ (where we view the set $I$ as a discrete category) or more
...
\begin{exercise}\label{exc.dependent_product_as_sections}
  ...
  \begin{enumerate}
    ...
    $$\prod_{i \in I} X_i \cong \left\{s : I \to \sum_{i \in I} X_i \,\middle|\, s \then \pi_1 = \id_I\right\}.$$\qedhere
  \end{enumerate}
\begin{solution}
...
\paragraph{Interactions between sums and products.}
Here's how to understand and work with a set that is written with both $\sum$'s and $\prod$'s, e.g.\ something like
\begin{equation}\label{eqn.misc98339}
\sum_{i\in I}\prod_{j\in J(i)}\sum_{k\in K(i,j)}X(i,j,k).
\end{equation}
To understand such an expression, ask yourself: ``to specify an element of this set, what choices must I make?'' % **Write pseudocode instead?? Indentation might be easier to understand

To answer this question, begin by saying ``To choose an element of [the expression]..." followed by pronouncing the formula. Every time you see $\sum_{i\in I}$, say ``choose an element $i\in I$; then, ....'' Every time you see $\prod_{j\in J}$, say ``for each $j\in J$, ....'' At the very end when you see a set $X$, say ``choose an element of $X$.''
(You may adjust words for clarity, the same way you would add parentheses to clarify a mathematical expression.)

So the formula in \eqref{eqn.misc98339} would be pronounced as follows:
\begin{quote}
To choose an element of \eqref{eqn.misc98339}, first choose an element $i\in I$; then, for each $j\in J(i)$, choose an element $k\in K(i,j)$ along with an element of $X(i,j,k)$.
\end{quote}
...
The following is sometimes called the \emph{type-theoretic axiom of choice} or the \emph{completely distributive property}, in this case of $\smset$. It is almost trivial, once you understand what it's saying; in particular, once the statement is written in Agda, its proof is one short line of Agda code. 
...
First, let's go from left to right. An element of the set on the left is a dependent function $f \colon (i \in I) \to \sum_{j \in J(i)} X(i, j)$, which we can compose with projections from its codomain to yield $\pi_1(f(i)) \in J(i)$ and $\pi_2(f(i)) \in X(i, f_1(i))$ for every $i \in I$.
We can then form the following pair in the right hand set:
\[
    (i \mapsto \pi_1(f(i)), i \mapsto \pi_2(f(i))).
\]

Now let's go from right to left. An element of the set on the right is a pair of dependent functions, $j \colon (i \in I) \to J(i)$ and $g \colon
(i \in I) \to X(i, j(i))$. We then get an element of the set on the left as follows:
\[i \mapsto (j(i), g(i)).\]

Now, we just need to check that a round trip takes us back where we were. If we
start on the right, our round trip gives us the pair
\[(i \mapsto \pi_1(j(i), g(i)), i \mapsto \pi_2(j(i), g(i))).\]
But $\pi_1(j(i), g(i)) = j(i)$ and $\pi_2(j(i), g(i)) = g(i)$ by definition, so
we're back where we started. On the other hand, starting on the left gives us
the function
...
Below, e.g.\ in \cref{exc.practice_sum_prod}, you'll often see alternating products and sums; using \eqref{eqn.set_completely_distributive}, you can always write it as a sum of products (in ``disjunctive normal form'').

When $J(i)=J$ does not depend on $i\in I$, the formula in \eqref{eqn.set_completely_distributive} is much easier:
\[
\prod_{i\in I}\sum_{j\in J}X(i,j)\cong\sum_{j\colon I\to J}\;\prod_{i\in I}X(i,j(i)).
\]
...
\[
    \poly\left(\sum_{i \in I}\yon^{p[i]}, q\right) \iso \prod_{i \in p(\1)} \poly(\yon^{p[i]}, q),
\]
...
When we wish to emphasize the arena perspective, we will call the data of $(f_1, f^\sharp)$ a \emph{morphism of arenas} or \emph{arena morphism} between $p$ and $q$; but since \cref{prop.poly_maps_colax_prod_sum} tells us that polynomial morphisms and arena morphisms carry the same data, we may use these terms interchangeably.
...
\paragraph{Composing arena morphisms.}
