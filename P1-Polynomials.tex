...
Given polynomial functors $p$ and $q$, a \emph{dependent lens} (or simply \emph{lens}) from $p$ to $q$ is a natural transformation $p\to q$.
...
In our alternative characterization of dependent lenses below, recall that a polynomial $p\iso\sum_{i\in p(\1)}\yon^{p[i]}$ can be uniquely identified with an indexed family $p[-]\colon p(\1)\to\smset$, a functor from the set $p(\1)$ viewed as a discrete category.
...
\end{equation}
natural in $p$ and $q$.
In particular, a lens $p\to q$ can be identified with a pair $(f_\1,f^\sharp)$
