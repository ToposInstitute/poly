...
We call the lens $f$ the \emph{wrapper} and its codomain $q$ the \emph{wrapper interface}, which we \emph{wrap} around $p$ using $f$.
...
Express this construction as wrapping $q$ around $p$ by giving a lens $f\colon p\to q$ for which composing $\phi$ with $f$ yields $\psi$.
\begin{solution}
...
On directions, $f$ takes into account the current outputs of each of the constituent interfaces, as well as any new output given to the wrapper interface, then uses all of this to give input to each of the conssituent interfaces.
