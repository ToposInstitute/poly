...
We refer to the menus and options represented by the arena as a whole as the \emph{scenario}. %While the corollas $\1,\yon,$ and $\yon^\2$ are each interesting as decisions, the sum $\yon^\2+\2\yon+\1$ has very little theoretical interest; it's just an example.
...
In this chapter, we will set down the basic category-theoretic story of $\poly$, so that we can have a firm foundation from which to speak about interactive systems.
...
While much of the following theory applies to representable functors from any category, we will focus on representable functors $\smset\to\smset$.
...
The symbol $\yon$ stands for Yoneda, for reasons we will explain in \cref{lemma.yoneda} and \cref{exc.finish_proof_yoneda} \cref{exc.finish_proof_yoneda.embed}. For now, here are some pictures corresponding to various representables or pure powers:
...
In algebra, a polynomial is just a sum of pure powers.
...
To make sense of these products, we will define functor multiplication as well.
...
%---- Subsection ----%
\subsection{Sums and products of sets} \label{subsec.poly.func_nat.repr_sum.sum_prod_set}
...
The \emph{sum} $\sum_{i\in I}X_i$ of this indexed family is the set
\[
...
The set of all dependent functions whose signature is determined by a given indexed family of sets is the \textit{product} of that indexed family, as below.

\begin{definition}[Product of sets] \label{def.prod_sets}
Let $I$ be a set and $X\colon I\to\smset$ be an $I$-indexed family of sets.
The \emph{product} $\prod_{i\in I}X_i$ of this indexed family is the set of dependent functions
\[
...
Though we proved above explicitly that $\smset$ has all small products and coproducts, from here on out, we will assume the standard category-theoretic fact that $\smset$ is complete (has all small limits) and cocomplete (has all small colimits). % TODO: add ref for this??
...
\begin{definition}[Categorical sums and products of functions]
...
%---- Subsection ----%
\subsection{Sums and products of functors $\smset\to\smset$} \label{subsec.poly.func_nat.repr_sum.sum_prod_func}
...
We saw in \cref{prop.apply1} how to interpret the set $p(\1)$ for any polynomial $p$, e.g.\ $p\coloneqq\yon^\3+\3\yon^\2+\4$, as the sum of the coefficients of the original polynomial $p$: here $p(\1)\iso\1+\3+\4\iso\8$.
...
\]
That is, $p(\0)$ is the set of \emph{constant} representable summands of $p$.
...
% TODO: define positions & directions
...
Let $p$ be an arbitrary polynomial functor, which our notation lets us write as $p\coloneqq\sum_{i\in p(\1)} \yon^{p[i]}$, and let $f\colon X\to Y$ be an arbitrary function.
...
\]
Say we choose the function $f\colon\1\to\2$ that sends $1 \mapsto 2$.
Then $p(f)$ sends $(1, 1, 1) \mapsto (1, 2, 2)$ and $(2, 1) \mapsto (2, 2)$.
...
For reasons we will explain later, we call such a morphism a \emph{dependent lens}, or a \emph{lens} for short.

\begin{definition}[Dependent lens, $\poly$] \label{def.poly_cat}
Given polynomial functors $p$ and $q$, a \emph{dependent lens} (or simply \emph{lens}) is a natural transformation $p\to q$.
Then $\poly$ is the category whose objects are polynomial functors and whose morphisms are dependent lenses.
\end{definition}

In other words, $\poly$ is the full subcategory of $\smset^\smset$ spanned by the polynomial functors: we take the category $\smset^\smset$, throw out all the objects that are not (isomorphic to) polynomials, but keep all the same morphisms between any two polynomial functors.
...
\end{proposition}
\begin{proof}
By \cref{cor.sum_prod_set_endofuncs}, the category $\smset^\smset$ has arbitrary coproducts given by $\sum_{i \in I}$.
...
\end{equation}
<<<<<<< HEAD
which coincides with our notion of sums of functors $\smset \to \smset$ from \cref{cor.sum_prod_set_endofuncs}.
Of course, it also coincides with our notion of polynomial addition from high school algebra---just add all the terms together, combining like terms to simplify.
=======
by \cref{cor.sum_prod_set_endofuncs}.
This coincides with our notion of polynomial addition from high school algebra: just add all the terms together!
>>>>>>> e251022 (MEs)
...
%---- Subsection ----%
\subsection{Dependent lenses, concretely} \label{subsec.poly.func_nat.morph.concrete}

A dependent lens (i.e.\ natural transformation) between polynomial functors $p \to q$ consists of a function $p(X) \to q(X)$ for each set $X \in \smset$ such that the naturality squares commute.
That's a lot of data to keep track of!
Fortunately, there is a much simpler way to think about these lenses, which we will discover with some help from our old friend, the Yoneda lemma.

\begin{exercise} \label{exc.poly_morph_yoneda}
Given a set $S$ and a polynomial $q$, show that a lens $\yon^S \to q$ can be identified with an element of the set $q(S)$.
...
\begin{solution}
Recall that $\poly$ is the full subcategory of $\smset^\smset$ spanned by polynomial functors, including $\yon^S$ and $q$.
...
Before we present our alternative characterization of dependent lenses, note that every polynomial $p\coloneqq \sum_{i \in p(\1)}\yon^{p[i]}$ is uniquely associated to an indexed family of sets $(p[i])_{i \in p(\1)}$.
Alternatively, we could write such an indexed family as a functor $p[-]\colon p(\1)\to\smset$, where we view the set $p(\1)$ as a discrete category.
Below, we make use of this functor notation to express the indexed families associated with polynomials.
...
\end{tikzcd}
\end{equation}
where $f_\1 \colon p(\1) \to q(\1)$ is a function (or functor between discrete categories) and $f^\sharp \colon q[f_\1(-)] \to p[-]$ is a natural transformation: for each $i\in p(\1)$, there is a function $f^\sharp_i\colon q[f_\1(i)]\to p[i]$.
\end{proposition}
\begin{proof}
...
The expression on the right hand side of \eqref{eqn.main_formula} is the set of dependent functions $f\colon(i\in p(\1))\to\sum_{j\in q(\1)}p[i]^{q[j]}$, each of which is uniquely determined by its two projections, $(\pi_1\circ f)\colon(i\in p(\1))\to q(\1)$ and $(\pi_2\circ f)\colon(i\in p(\1))\to p[i]^{q[j]}$ with $j\coloneqq\pi_1(f(i))$.
These can be identified respectively with a (non-dependent) function $f_\1\coloneqq\pi_1\circ f$ with signature $p(\1)\to q(\1)$ and a natural transformation $f^\sharp\colon q[f_\1(-)]\to p[-]$.
\end{proof}

We have now greatly simplified our characterization of dependent lenses $f\colon p\to q$: rather than infinitely many functions satisfying infinitely many naturality conditions, they may simply be specified by a function $f_\1\colon p(\1)\to q(\1)$ and, for each $i\in p(\1)$, a function $f^\sharp_i\colon q[f_\1(i)]\to p[i]$, without any additional restrictions.
...
When we wish to emphasize the arena perspective, we will call the data of $(f_\1, f^\sharp)$ an \emph{arena morphism}, a \emph{dependent lens}, or simply a \emph{lens} between $p$ and $q$ (see \cref{subsec.poly.func_nat.morph.bimorphic_lens}); but since \cref{prop.poly_maps_prod_sum} tells us that polynomial morphisms and arena morphisms carry the same data, we may use these terms interchangeably.
In fact, in future chapters we will primarily refer to the morphisms of $\poly$ as \emph{lenses}.
If you're familiar with lens terminology from a functional programming context, we'll see in \cref{subsec.poly.func_nat.morph.bimorphic_lens} how our notion of dependent lenses is related.
...
%---- Subsection ----%
\subsection{Arena morphisms as dependent lenses}\label{subsec.poly.func_nat.morph.bimorphic_lens}
...
Then an arena morphism between any two arenas is a more generalized lens: a \emph{dependent lens}, where the direction-sets depend on the positions.
This is why we will henceforth refer to arena morphisms simply as \emph{lenses}.
...
\end{equation}
so in terms of arenas, a map $\gamma\in\Gamma(p)$ can be thought of as a dependent function that sends each $p$-position $i$ to a direction $\gamma(i)$ of $p$ at $i$.
...
In this chapter we explained how discrete dynamical systems can be expressed as certain maps of polynomial functors. For example, a Moore machine has an input set $A$, an output set $B$, a set of states $S$, a return function $S\to B$, and an update function $A\times S\to S$. All this is captured in a map of polynomials
\[S\yon^S\to B\yon^A.\]
We discussed a generalization $S\yon^S\to p$, where the output is an arbitrary polynomial $p\in\poly$. We also talked about how to wire machines in parallel by using the parallel product $\otimes$ and how to add wrapper interfaces by composing with maps $p\to q$.
