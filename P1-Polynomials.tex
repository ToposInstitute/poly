...
We now know that we can specify a morphism $p\to q$ in $\poly$ in two ways:
\begin{itemize}
...
We can visualize lens composition using polyboxes.
...
\end{tikzpicture}
\]
The position box for $q$, which would normally be blue as part of the polyboxes for $g\colon q\to r$, is instead filled in via $f_\1$; similarly, the direction box for $q$, which would normally be blue as part of the polyboxes for $f\colon p\to q$, is filled in via $g^\sharp$.
...
Similarly, the arrow $f^\sharp$ depends on both the position box of $p$ and the direction box of $q$.

On the other hand, the arrow $g_\1$ depends only on the position box of $q$, and not the position box of $p$ that came before it: $g_\1$ is the on-positions function for a lens $q\to r$ and therefore depends only on its domain. (Of course, changing the position box of $p$ may change the position box of $q$, thus affecting what $g_\1$ enters in the position box for $r$; what we mean is that if the position box of $p$ changes but the position box of $q$ does not, $g_\1$ will not change the position box of $r$.)
For the same reason, the arrow $g^\sharp$ does not depend on the position box of $p$, and the arrow $f^\sharp$ does not depend on either box of $r$.
...
\begin{tikzpicture}
  \node (1) {
    \begin{tikzpicture}[polybox, mapstos]
      \node[poly, dom, "$p$" below] (p) {$f^\sharp_ig^\sharp_{f_\1i}c$\at$i$};
  ...
  \node[right=1.8 of 1] (2) {
    \begin{tikzpicture}[polybox, mapstos]
      \node[poly, dom, "$p$" left] (p) {$h^\sharp_ic$\at$i\vphantom{h_\1}$};
      \node[poly, cod, "$r$" right, right=of p] (q) {$\vphantom{h^\sharp_i}c$\at$h_\1i$};
...
\begin{remark}
  ...
  \]
  corresponding to the lens $p\to r$ whose on-positions function $p(\1)\to r(\1)$ is equal to the composite of the on-positions functions $f_\1$ and $g_\1$, and whose on-directions function $r[g_\1f_\1i]\to p[i]$ for each $i\in p(\1)$ is equal to the composite of the on-directions functions $g^\sharp_{f_\1i}$ and $f^\sharp_i$.
...
%-------- Section --------%
\section{Relation to lenses from functional programming}\label{subsec.poly.cat.morph.bimorphic-lens}
...
\begin{equation}\label{eqn.bimorphic_lens}
  ...
  \end{aligned}
\end{equation}
Each position $b \in B_1$ of $B_1\yon^{A_1}$ ``gets'' a position $f_\1b \in B_2$ of $B_2\yon^{A_2}$, and given $b \in B_1$, every direction at $f_\1b$ in $A_2$ ``puts'' a direction back at $b$ in $A_1$.
