...
On the other hand, while \emph{arenas} and \emph{dependent sets} are exactly the same mathematical object, with the same syntax, we will stick to usng the term ``arena'' when referring to them as objects in our categories of interest that coincide with polynomials, while we will use the term ``dependent set'' for more general set-theoretic purposes.
...
\cref{prop.free_parallel_monoid} tells us that the carrier of the free $\otimes$-monoid on $p$ has as its positions elements of $\Set{List}(p(\1))$, i.e.\ lists of $p$-positions.
