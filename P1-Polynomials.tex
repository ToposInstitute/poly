...
\begin{example}
    ...
    We could draw them out instead---each element corresponds to one of the four corollas above with each of its leaves labeled with an element of $X$:
    \[
    \begin{tikzpicture}[trees, sibling distance=5mm]
        \node["$1$" below] (1) {$\bullet$}
            child {node {$a$}}
            child {node {$a$}};
        \node["$1$" below, right=of 1] (2) {$\bullet$}
            child {node {$a$}}
            child {node {$b$}};
        \node["$1$" below, right=of 2] (3) {$\bullet$}
            child {node {$b$}}
            child {node {$a$}};
        \node["$1$" below, right=of 3] (4) {$\bullet$}
            child {node {$b$}}
            child {node {$b$}};
        \node["$2$" below, right=of 4] (5) {$\bullet$}
            child {node {$a$}};
...
% TODO: Why define trees when we could've just defined corollas...? Decision trees?? built out of corollas? put above: Later we will study larger trees built out of these corollas. ???
