...
\begin{example}[Interacting with the coin jar]
  ...
  We can express this with an on-positions function $f_\1$ from the set of $p$-positions to the set of $q$-positions, as follows:

%  \[
%  \begin{tikzpicture}[short=-2pt]
%      \node (open) {`open'};
%      \node[below=1 of open] (closed) {`closed'};
%
%      \node[draw, ellipse, inner sep=0pt, fit=(open)(closed), label={[anchor=south,below]270:$p(\1)$}] (ppos) {};
%
%      \node[left=1 of ppos] (needy) {`open'};
%      \node[below=1 of open] (closed) {`closed'};
%
%%      \draw (1+3,.8-.4*1) node[label={[below right=0]:\tiny\x}] (Y1\x) {$\bullet$};
%%      \draw (1+3,.8-.4*2) node[label={[below right=0]:\tiny\x}] (Y1\x) {$\bullet$};
%%      \draw (1+3,.8-.4*3) node[label={[below right=0]:\tiny\x}] (Y1\x) {$\bullet$};
%%
%%      \draw[mapsto] (X10) to (Y11);
%%
%%      \node[draw, ellipse, inner sep=0pt, fit=(Y11) (Y13)] (Y1) {};
%  \end{tikzpicture}
%  \]
%
%
...
% TODO: finish/integrate above section
