...
\end{tikzpicture}
\]
This suggests yet another depiction of the polynomial \eqref{eqn.poly_example}: as a \emph{dependent set}\footnote{Perhaps better known as an \emph{indexed family of sets}.} $(p[i])_{i \in I}$.
...
The first row shows the algebraic terminology, as in \eqref{eqn.poly_example};
the second row shows the terminology we just introduced for polynomials as dependent sets, as in \eqref{eqn.arena_example};
the third shows the pictorial terminology of corolla forests, as in \eqref{eqn.forest_example};
the fourth shows dynamical systems terminology, which we will explore in \cref{ch.poly.dyn_sys};
and the fifth row shows decision-making terminology, which we will introduce in \cref{sec.poly.intro.dec}.% and further develop in \cref{**}.

\begin{equation}%\label{eqn.table_terminology}
\footnotesize
\begin{tabular}{l|ll}
\multicolumn{3}{c}{\normalsize Polynomial Terminology}\\[3pt]
$p \coloneqq \sum_{i \in p(\1)} \yon^{p[i]}$ & $i \in p(\1)$ & $d \in p[i]$ \\
\textbf{arena} & position & direction \\
\textbf{corolla forest} & root $\bullet$ & leaf $\uparrow$ \\
\textbf{(mode-dependent) interface} & output & input \\
\textbf{menu} & decision & option \\
...
\end{remark}
% ** We haven't really explained *how* a polynomial is a functor yet
