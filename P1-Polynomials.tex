...
\index{set!indexing}\index{category!discrete}
...
\index{coproduct}\index{polynomial functors!coproduct of|see{polynomial functors, sum of polynomials}}
...
Then by \cref{cor.poly-coprod-repr} and the universal property of the coproduct, we have a natural isomorphism\index{coproduct!universal property of}
\[
...
\index{interface}\index{dynamics!internal}\index{control}
...
\begin{enumerate}
    ...
    Since each reward value is a real number, it suffices to define the state set to be $S'\coloneqq\ord{n}\times\ord{n}\times\lst(\rr)$.\index{list}
