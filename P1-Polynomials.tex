...
Though we proved above explicitly that $\smset$ has all small products and coproducts, from here on out, we will assume the standard category-theoretic fact that $\smset$ is complete (has all small limits) and cocomplete (has all small coliimits). % TODO: add ref for this??
...
Given functors $F,G\colon\smset\to\smset$, their categorical coproduct or sum in $\smset^\smset$, denoted $F+G$, is the functor $\smset\to\smset$ defined for all $X\in\smset$ by
\[
	(F+G)(X)\coloneqq F(X)+G(X);
\]
while their categorical product in $\smset^\smset$, denoted $F\times G$ or $FG$, is the functor $\smset\to\smset$ defined for all $X\in\smset$ by
\[
...
\]
are respectively defined for all $X\in\smset$ by
\[
...
Given a set $I\in\smset$, we will also use $I$ to denote the constant functor $\smset\to\smset$ that assigns every set to $I$.
...
We are finally ready to define what a polynomial functor is.
