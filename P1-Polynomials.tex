...
The symbol $\yon$ stands for Yoneda, for reasons we will explain in \cref{lemma.yoneda} and \cref{exc.finish_proof_yoneda} \cref{exc.finish_proof_yoneda.embed}. For now, here are some pictures for your eyes to gaze at; they are the polynomials corresponding to various representables, namely the pure powers:
...
Now that we have introduced representable functors 

So far, we have used the symbol $\yon^A$ to denote the functor $\smset\to\smset$ represented by a set $A$, namely the functor $X\mapsto X^A$.
More generally, we may add these representable functors together to form polynomials.
In some sense, the name ``polynomial'' doesn't quite fit: in algebra, polynomials are generally finite sums, whereas we allow infinite sums.
However, we are not the first by far to use the term ``polynomial'' this way, and the name stuck.
...
The polynomials we have already introduced are the pure powers $\yon^A$ for all sets $A$: they are representable functors $\yon^A\colon\smset\to\smset.$
So to define general polynomials like $p$, we just need to introduce what the sums of functors are. It will be useful to discuss products of functors at the same time.
