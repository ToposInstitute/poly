...
\begin{enumerate}
    ...
    \item
    To have the robots listen for each others' signals only when they are sufficiently close, we must move away from monomial interfaces and Moore machines to leverage dependency.
    Let the robots' new interfaces be
    \[
        p\coloneqq \{\text{`close'}\}AX\yon^{A'X}+\{\text{`far'}\}AX\yon^X \qqand p'\coloneqq \{\text{`close'}\}A'X\yon^{AX}+\{\text{`far'}\}A'X\yon^X,
    \]
    so that their input sets are just $X$ when they are `far.'
    ...
    \[
        \{\text{`close'}\}AX\yon^{A'X}\otimes\{\text{`close'}\}A'X\yon^{AX}\to\yon^{[0,5]},
    \]
    given by the on-directions function
    \[
        ((`close',a,x),(`close',a',x'),s)\mapsto
          \begin{cases}
          	((`close',a',x'),(`close',a,x))&\tn{ if }d(x,x')<s\\
          	((`far',a'_0,x'),(`far',a_0,x))&\tn{ otherwise};
          \end{cases}
    \]
    the lens
    \[
    ...
    given by the on-directions function
    \[
        ((`far',a,x),(`far',a',x'),s)\mapsto
          \begin{cases}
          	((`close',x'),(`close',x))&\tn{ if }d(x,x')<s\\
          	((`far',x'),(`far',x))&\tn{ otherwise};
          \end{cases}
    \]
    and two other lenses that should never come up in practice but that we must specify anyway:
    \[
        \{\text{`close'}\}AX\yon^{A'X}\otimes\{\text{`close'}\}A'X\yon^{AX}\to\yon^{[0,5]},
    \]
    given by the on-directions function
...
\end{tikzpicture}
\]
	\item Use the above and a diagram of the following form to create a function that spits out the base-10 digits of $1/7$ (with the decimal point omitted).
\[
\begin{tikzpicture}[oriented WD]
	...
	\draw 
		...
		\node[right, font=\footnotesize] at (outer_out1) {$0142857142857142857\cdots$};
\end{tikzpicture}
\]
\end{enumerate}
\begin{solution}
\begin{enumerate}
    ...
    If the initial state is $(q,r,p)=(0,1,10)$, then the first output will be $0$, the next state will be $(10\bdiv7,10\bmod7,1*10)=(1,3,10)$.
    
    **
    %I think we need to add some sort of delay on this for it to work properly
\end{enumerate}
\end{solution}
\end{exercise}
...
Then an enclosure for the person and the units is a lens $\2\yon\otimes X\yon\otimes \yon^X\to\yon$. The morphism $\2X\yon^X\to\yon$, corresponding to a function $\2X\to X$ that maps $(1,x)\mapsto x_0$ and $(2,x)\mapsto x$.
\end{example}
