...
\begin{exercise}%\label{exc.changing_types} % the sort of information or 
...
% **transition? copower-hom-power two-variable adjunction
...
\begin{enumerate}
    \item Using \cref{prop.morph_arena_to_func} to translate $f$ from a morphism in $\poly$ to a natural transformation, characterize the pullback of $p(B)\To{f_B}q(B)\From{q(h)}q(A)$ in $\smset$.
...
\begin{enumerate}[resume]
    ...
    \end{equation}
    commutes.
    Hint: Use \cref{prop.morph_arena_to_func} and/or \cref{cor.morph_func_to_arena}.
...
\begin{enumerate}
    ...
    Meanwhile, an element of $q(A)$ is a pair comprised of a $q$-position $j$ and a function $\ell\colon q[j]\to A$, and $q(h)$ sends $(j,\ell)\mapsto(j,\ell\then h)$.
...
\begin{enumerate}[resume]
    \item By \cref{cor.morph_func_to_arena}, $f_{p[i]}$ sends $(i,\id_{p[i]})\mapsto(f_\1(i),f^\sharp_i)$, and $q(f^\sharp_i)$ sends $(f_\1(i),\id_{q[f_\1(i)]})\mapsto(f_\1(i),f^\sharp_i)$ as well, so \eqref{eqn.cart_nt_pullback_cone} commutes.
    ...
    It follows from the behavior of $p(f^\sharp_i)$ that $i'=i$ and $g\then f^\sharp_i=\id_{p[i]}$, and it follows from the behavior of $f_{q[f_\1(i)]}$ (by \cref{prop.morph_arena_to_func}) that $f^\sharp_i\then g=\id_{q[f_\1(i)}$.
