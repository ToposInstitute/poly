...
\begin{enumerate}
    \item every morphism $f$ in $\cat{C}$ factors $f = e\then m$ as a morphism $e \in E$ composed with a morphism $m\in M$;
    \item this factorization is unique up to unique isomorphism;
    \item $E$ and $M$ each contain every isomorphism; and
    \item $E$ and $M$ are each closed under composition.
\end{enumerate}
If $E$ is the class of epimorphisms and $M$ is the class of monomorphisms, we say that $\cat{C}$ has \emph{epi-mono factorization}.
\end{definition}
...
\end{proposition}
\begin{proof}
The class of epimorphisms (resp.\ monomorphisms) automatically contains every isomorphism and is closed under composition, so it remains to check conditions 1 and 2 from \cref{def.factor}.
\end{proof}
