...
In \cref{**}, we will show that $\circ$ is left coclosed, and in \cref{**}, we will show that $\circ$ preserves connected limits.
\end{proof}
...
We'll see how to express this in terms of lenses in \cref{exc.grid_reward_par}, once we explain how to juxtapose systems like this in general in \cref{sec.poly.dyn_sys.new.par}.
...
%---- Subsection ----%
\subsection{Situations as enclosures}\label{sec.poly.dyn_sys.new.sit_encl}
...
\begin{exercise}
Let $\phi\to S\yon^S\to B\yon^A$ be an $(A,B)$-Moore machine.
\begin{enumerate}
...
We saw in \cref{sec.poly.dyn_sys.new.sit_encl} that a situation (i.e.\ lens into $\yon$) for the interface of a dynamical system encloses that dynamical system in a closed system.
...
As for the chalk, it is either in your possession or not.
Either way, it also returns its height, which is either down on the table or up in the air.
...
The chalk has only four states $C\coloneqq \{\text{out}, \text{in}\} \times H \cong\4$: the $H$ coordinate is its current height, and the other coordinate is whether or not it is ``in your possession.'' We will give a dynamical system $C\yon^C\to\const{Chalk}$ with states $C$ and interface $\const{Chalk}$, i.e.\ a lens
...
So we can express the on-directions function of \cref{eqn.chalk_dynamics} at $(\text{out}, h_\const{Chalk})$ as
\begin{align*}
	\text{no press} &\mapsto (\text{out, down}) \\
	\text{press} &\mapsto (\text{in}, h_\const{Chalk})
\end{align*}
and the on-directions function of \cref{eqn.chalk_dynamics} at $(\text{in}, h_\const{Chalk})$ as
\begin{align*}
...
Equivalently, \cref{eqn.situations2} says that the interaction pattern tells us what values the first eye sees at any location when the second eye's location is fixed at $b'$, as well as what values the second eye sees at any location when the first eye's location is fixed at $b$.
...
\subsection{Simple interaction patterns as wiring diagrams}
