...
\begin{exercise}\label{exc.moore_lens}
...
When we run an $(A,B)$-Moore machine, an initial state and a list of $A$'s turns into a list of states, which then turns into a list of $B$'s. Indeed, when we receive a new state from the old state and an input, via the put map $S\times A\to S$, this becomes the new state. Somehow in $S\yon^S$, the $S$ in the exponent makes its way to the $S$ in the coefficient. But how? We'll discuss this in \cref{ex.do_nothing}.
...
As mentioned just after \cref{exc.moore_lens}, something seems to be missing in the way we model dynamical systems as lenses $\varphi\colon S\yon^S\to p$.
