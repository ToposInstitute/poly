...
In this chapter, we will set down the basic categorical story of $\poly$, so that we can have a firm foundation from which to speak about interactive systems.
...
Note that the position-set $p(\1)$ and the $p(\1)$-indexed family of direction-sets $p[-]\colon p(\1)\to\smset$ uniquely characterize a polynomial $p$ up to isomorphism.
...
\begin{exercise}
Using \cref{ex.comm_poly}, verify explicitly that, for $p, q \in \poly$, the polynomial $p+q$ given by the binary sum of $p$ and $q$ satisfies the universal property of the coproduct of $p$ and $q$.
...
To show that $p+q$ equipped with $\iota$ and $\kappa$ satisfies the universal property of the coproduct, we apply \cref{ex.comm_poly}.
