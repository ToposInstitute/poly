...
Much of \cref{part.comon} will focus on the astonishing features of another monoidal structure, an asymmetric one, which we will put off defining---we will save its surprises for when we can better savor them.
But here we introduce a third symmetric monoidal structure, given by an operation you probably weren't allowed to do to polynomials back in high school.
% This monoidal structure will prove useful when we apply $\poly$ to dynamics in the next chapter.
% TODO: explain parallel products in terms of interaction
