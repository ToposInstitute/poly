...
\end{tikzpicture}
\]
This suggests yet another depiction of the polynomial \eqref{eqn.poly_example}: as a \emph{dependent set}\footnote{Perhaps better known as an \emph{indexed family of sets}. But we refer to these as \emph{dependent sets} to compare them to the usual sets we are familiar with from set theory, the same way dependent types compare to the usual types that are familiar from type theory.} $(p[i])_{i \in I}$.
...
For instance, though a polynomial will turn out to be a \emph{functor}, while an arena is a \emph{dependent set}, they are so closely related that we often do not make a distinction between a polynomial $p$ and its arena $(p[i])_{i \in I}$; they are two different syntaxes for the same object.
...
To compromise, we will denote an indexed family of sets by $X\colon I\to\smset$ for a set $I$ viewed as a discrete category, but denote the set obtained by evaluating $X$ at each $i\in I$ by $X_i$ rather than $X(i)$.
...
\begin{definition}[Sum of sets] \label{def.dep_sums_sets}
...
\begin{definition}[Product of sets] \label{def.dep_prods_sets}
Let $I$ be a set and $X\colon I\to\smset$ be an $I$-indexed family of sets.
...
For a dependent function $f\colon(i\in I)\to X_i$, we may denote the element of $X_i$ that $f$ assigns to each $i\in I$ by $f(i), fi,$ or $f_i$.
...
The following standard fact describes the constructions from \cref{def.dep_sums_sets,def.dep_prods_sets} categorically and further justifies why we call them sums and products.
...
from \eqref{eqn.misc98339} specifies a fixed set. Here is a list of all eight of its elements:
\[
\left\{
\begin{gathered}
...
We will often encounter sums of sets nested within products, as in \eqref{eqn.prod_sum_prod}.
...
To be very precise and verbose, let's say
\[
I\coloneqq\4\qqand
  p[1]\coloneqq\2,\quad
  p[2]\coloneqq\1,\quad
...
\begin{exercise}
In the verbose style of \cref{ex.verbose_poly_eval}, write out all the elements of $p(X)$ for $p$ and $X$ as follows:
\begin{enumerate}
