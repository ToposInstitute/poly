...
%-------- Section --------%
\section{Decisions \& interaction} \label{sec.poly.intro.dec}
...
\end{proposition}
\begin{proof}
See \cref{thm.poly_limits,thm.poly_colimits}.
\end{proof}
...
For the following results, let $\com{C}$ be the comonoid in $(\poly,\yon,\circ)$ that corresponds to the category $\cat{C}$ under \cref{prop.ahman_uustalu1}, and similarly for $\com{D}$ and $\cat{D}$.
...
\begin{proposition}
For any category $\cat{C}$, the category of left $\com{C}$-comodules is equivalent to the category of functors $\cat{C}\to\poly$:
...
\]
between that of bicomodules over comonoids in $\poly$ and parametric right adjoints between copresheaf categories.
\end{proposition}
\begin{proof}
**
\end{proof}
...
\footnote{The material in this section is even more technical, and we won't use it again in the book, so the reader may wish to skip to \cref{part.comon}.}
