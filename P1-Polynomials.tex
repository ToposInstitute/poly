...
In this chapter, we will set down the basic categorical story of $\poly$, so that we have a firm foundation from which to speak about interactive systems.
...
%-------- Section --------%
\section{Introducing polynomial functors}
\label{sec.poly.func_nat.func}



%---- Subsection ----%
\subsection{What is a polynomial functor?} \label{subsec.poly.func_nat.func.what}
...
%---- Subsection ----%
\subsection{Interpreting positions and directions}

Let us make an informal digression on how we will think about the positions and directions of polynomials throughout this book.
While this section has very little mathematical content, the intuition we build here will guide us as we delve into the deeper theory of polynomials and their applications to modeling interaction.

The main idea is that a \emph{position} is some status that may be held, while the set of \emph{directions} at each position are the set of options available to one holding that status.
While these positions and directions may be imagined abstractly, here we give some concrete examples.

\begin{example}[Representables as menus] \label{ex.reps-as-menus}
    Consider a representable (and thus polynomial) functor $\yon^A$ for some set $A$.
    As a polynomial, it has $1$ position and the elements of $A$ as its directions.
    In this case, we can think of $A$ as a set of options to choose from, possibly as part of a menu.

    Perhaps the menu consists of dinner options available at a wedding, in which case the corresponding representable functor might be
    \[
    ...
    \]
    Of course, in each case, there are exactly $3$ directions, so there is an isomorphism of representable functors
    \[
        \yon^{\{\text{`chicken', `beef', `vegetarian'}\}} \iso \yon^{\text{`cut', `copy', `paste'}\}}.
    \]

    We may interpret the representable $\yon^\2$ as a $2$-option menu.
    ...
    Having no options, represented by $\yon^\0\iso\1$, is when you actually don't get through it: an impossible decision, a sort of ``dead end.''
...
For consistency, we will favor the term ``direction'' over ``option'' when referring to the elements of $A$ for a summand $\yon^A$ of some polynomial.
Nevertheless, when we think of a polynomial's directions, we may think of them as options to choose from.
...
\begin{example}[A coin jar]

\end{example}

% TODO: examples of interpreting positions and directions

%---- Subsection ----%
\subsection{Corolla forests}

% TODO: decision trees -> corolla forests

%---- Subsection ----%
\subsection{Polyboxes}

% TODO: spreadsheets/menus
...
If you're familiar with lenses from functional programming, we'll see in \cref{subsec.poly.func_nat.morph.bimorphic_lens} how our notion of a dependent lens is related.
...
Observe that our characterization of a dependent lens can be expressed entirely in the language of positions and directions: a lens $f\colon p \to q$ consists of a ``forward'' \emph{on-positions} function $f_\1$ from the position-set of $p$ to the position-set of $q$, along with, for every position $i$ of $p$, a ``backward'' \emph{on-directions} function $f^\sharp_i$ from the direction-set of $q$ at $f_\1(i)$ to the direction-set of $p$ at $i$.
We will refer to a morphism between polynomial functors as a ``lens'' rather than as a ``natural transformation'' when we wish to emphasize this concrete on-positions and on-directions interpretation.

% TODO: make the above a def? also maybe incorporate on-directions map as nat trans

% TODO: Give intuition via decisions, corollas, etc. each their own section
...
%---- Subsection ----%
\subsection{Relation to lenses from functional programming}\label{subsec.poly.func_nat.morph.bimorphic_lens}
...
The lemma itself is the crux of the proof of \cref{prop.poly_maps_prod_sum}, that these two formulations of morphisms between polynomials are equivalent; so unraveling this proof reveals the translation we seek.
...
\]
where the isomorphism follows from the universal property of coproducts, as in the proof of \cref{prop.poly_maps_prod_sum}.
