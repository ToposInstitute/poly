...
\begin{equation*}
    ...
    \end{tikzpicture}
\end{equation*}
Above, the roots are indicated by dots ($\bullet$), and the leaves are indicated by arrows ($\uparrow$).
...
\begin{definition}[Corolla forest]
    ...
    The \emph{corolla forest associated to a polynomial functor} $p\iso\sum_{i\in p(\1)}\yon^{p[i]}$ is the disjoint union of corollas associated to each representable summand $\yon^{p[i]}$ of $p$.
\end{definition}
