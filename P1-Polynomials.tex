...
In \cref{prop.comp_left_coclosed}, we will show that $\circ$ is left coclosed, and in \cref{**}, we will show that $\circ$ preserves connected limits.
\end{proof}
...
\begin{definition}[Polynomial morphisms, $\poly$] \label{def.poly_cat}
...
\]
with some other robots whose interfaces are $q_1,\ldots,q_n$; let $q\coloneqq(q_1\otimes\cdots\otimes q_n)$. The interaction pattern induces a morphism $f'\colon q\to \ihom{p,r}$ such that the original system $f$ factors through the evaluation $\ihom{p,r}\otimes p\to r$.
