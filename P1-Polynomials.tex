...
For example, in any rooted tree, the root has height $0$; every neighbor of the root has height $1$; every neighbor of a vertex of height $1$ either is the root or has height $2$; and so forth.

Now we can define a special kind of tree that will depict our representable functors.
...
Here are the corollas associated to various representables; the roots are indicated by dots ($\bullet$) and the leaves are indicated by arrows ($\uparrow$).
\begin{equation*}
...
% Why define trees when we could've just defined corollas...?
