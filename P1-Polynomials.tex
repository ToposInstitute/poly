...
\begin{example}
    ...
    \begin{tikzpicture}[trees, sibling distance=5mm]
        ...
        \node["$1$" below, right=1.2 of 3] (4) {$\bullet$}
            child {node {$b$}}
            child {node {$b$}};
        \node["$2$" below, right=1.2 of 4] (5) {$\bullet$}
            child {node {$a$}};
        ...
        \node["$3$" below, right=of 7] (8) {$\bullet$}
            child {node {$b$}};
        \node["$4$" below, right=of 8] (9) {$\bullet$};
    \end{tikzpicture}
    \]
\end{example}
...
\begin{equation} \label{eqn.polybox}
...
\begin{equation} \label{eqn.polybox}
  ...
  \end{tikzpicture}
\end{equation}
The top cell, or the \emph{direction box}, cannot be filled until the bottom cell is.
Once the bottom cell contains a $p$-position $i$, the top cell must be filled with a $p[i]$-direction, say $a\in p[i]$:
\begin{equation} \label{eqn.polybox}
  \begin{tikzpicture}[polybox, tos]
  ...
  \end{tikzpicture}
\end{equation}
The $p[-]$ label to the left of the top cell reminds us that the $a$ within it is an element of $p[i]$, where $i$ is the entry in the bottom cell.
Once we are accustomed to polyboxes, we will often drop these reminder labels.
