...
% Let $S'\coloneqq S+\1$, so that the initial state $\yon\to S'\yon^{S'}$ now is canonical: it's the new $\1$. We also have a canonical inclusion $S\To{i}S'$. We will give a morphism
% \[
% S'\yon^{S'}\to\yon+\yon^S
% \]
...
\begin{tikzpicture}
  ...
  \begin{tikzcd}[row sep=15pt]
  	...
  	\LMO{0}\ar[l, bend left=15pt, yellow!75!black]\ar[r, red]&
...
We'll introduce wrapper interfaces in the next section, before describing how they can be used in conjunction with parallel products to model general interaction in \cref{**}.
...
We call the lens $f$ the \emph{wrapper} and its codomain $q$ the \emph{wrapper interface}, which we \emph{wrap} around $\phi$ using $f$.
...
%** Give an example of this with both outputs and inputs changing
...
%---- Subsection ----%
\subsection{Situations as enclosures}
...
We call such a wrapper an \emph{enclosure} of an interface $p$, since it is a way of closing off $p$ to the outside world.
...
An enclosure of your interface dictates what you'll see (the input you receive) given anything you might do (the output you provide); there is no need for any further outside interference.

\begin{remark}
Although they both refer to lenses into $\yon$, we will use the term \emph{enclosure} in the context of dynamical systems and wrappers and \emph{situation} more generally.
\end{remark}

\begin{exercise}
The notation $\Gamma(p)$ for the set of situations for a polynomial $p$ comes from the common mathematical term ``global section.''
...
by taking their parallel product, then compose this system with a wrapper $f$ with wrapper interface $q$ to obtain a new dynamical system
...
Suppose we have a dependent dynamical system $\varphi$ that takes unchanging input and sometimes outputs an element of $A$ and sometimes outputs only silence. That is, its interface is $A\yon+\yon$. But what if some other system inputs only elements of $A$? We will combine $\varphi$ with another system---a repeater---to get an $A$-emitter, i.e.\ a system with interface $A\yon$. How do we construct the repeater, and how do the two systems fit together?
