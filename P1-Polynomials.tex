...
In \cref{**}, we will show that $\circ$ preserves connected limits, and in \cref{**}, we will show that $\circ$ is left coclosed.
\end{proof}
...
Much of the following theory is applicable for general functors into $\smset$; but for now, we are only interested in functors $\smset\to\smset$.
...
\begin{definition}[Dependent sums and products of sets] \label{def.dep_sums_prods_sets}
...
\end{proposition}
\begin{proof}
This is a special case of a more general fact, where $[\smset,\smset]$ is replaced by an arbitrary functor category $[\cat{D},\cat{C}]$, where $\cat{C}$ is a category that (like $\smset$) has limits and colimits; see \cite[page 22 -- 23, displays (24) and (25)]{macLane1992sheaves}.
\end{proof}
...
So the sum of these functors over $i\in p(\1)$ sends each $(i, g)\in p(X)$ to $(i, f^{p[i]}(g)) = (i, g\then f)\in p(Y)$, as desired.
\end{proof}
...
Binary coproducts are thus given by binary sums of functors, denoted by $+$, while the initial object of $\poly$ is the constant polynomial $\0$.
...
When we wish to emphasize the arena perspective, we will call the data of $(f_\1, f^\sharp)$ a \emph{morphism of arenas}, an \emph{arena morphism}, or a \emph{lens} between $p$ and $q$ (see \cref{subsec.poly.func_nat.prepare_dyn.bimorphic_lens}); but since \cref{prop.poly_maps_prod_sum} tells us that polynomial morphisms and arena morphisms carry the same data, we may use these terms interchangeably.
...
\end{align*}
where ($\ast$) follows from the fact that for any function $f_\1 \colon p(\1) \to q(\1)$, the set $p(\1)$ can be written as the disjoint union of sets of the form $f_\1\inv(j) = \{i \in p(1) \ | \ f_\1(i) = j\}$ for each $j \in q(\1)$.
...
The result above will allow us to apply \cref{eqn.cat_completely_distributive}, or sometimes specifically \cref{eqn.push_prod_sum_obj_indep}, to push $\prod$'s past $\sum$'s of polynomials whenever we so desire.
...
The on-positions function $\mu_\1 \colon m(\1)^n \to m(\1)$ sends every $n$-tuple $i\coloneqq (i_1,\ldots,i_n)$ of positions to a single position $i'\coloneqq\mu_\1(i)$.
...
Let $m\coloneqq\rr_{\geq0}\yon^{\rr_{\geq0}}$. If we take $\eta\coloneqq0$ and take $\mu_\1(a,b) \coloneqq a+b$ and $\mu^\sharp_{(a,b)}(t) \coloneqq \left(\frac{at}{a+b},\frac{bt}{a+b}\right)$, then $(m, \eta, \mu)$ is a $\otimes$-monoid.
\end{example}
...
\]
for all $k\in\nn$.
% definition a little clumsy; we'll see this sequence pop out of the theory in \cref{**}.
...
\end{enumerate}
\begin{solution}
For each of the following constructs, we find $A, B \in \smset$ such that the construct can be identified with a morphism $S\yon^S \to B\yon^A$, i.e.\ a function $\text{return} \colon S \to B$ and a function $\text{update} \colon S \times A \to S$.
\begin{enumerate}
...
\]
For any input, if it is of the form $(s,t)$, then the robot should go to entry $s$ in the file and read the character at that entry.
If the input is ``continue,'' the robot should move to the next entry (i.e.\ from $s$ to $s+1$) and read that character—unless the new entry would be greater than $t$, in which case the robot should continually output ``done'' until it receives another $(s,t)$ pair.
\begin{solution}
Given a file $f \colon \ord{n} \to \Set{ascii}$, we construct our file-reading robot as a Moore machine as follows.
There are many options for what states the machine should record, but we will use pairs of values $(i, t) \in \ord{n}^\2$, where $i$ is the entry where the robot is current located, while $t$ is the entry where the robot should stop.
But we will also include a ``stopped'' state to record when the robot has already stopped.
...
\]
If the robot's current state is ``stopped,'' then the robot should output ``done.'' Otherwise, the robot should output the character at the entry where the robot is currently located.
...
While \cref{exc.file_reader} gives us a functioning file-reading robot, it is a little strange that we are still able to give the input ``continue'' even when the output is ``done,'' or input a new range of entries before the robot has finished reading from the previous range.
...
In particular, we will be able to let the robot ``close its port,'' so that it can't receive signals while it's busy reading, but open its port again once it's ``done''; see \cref{ex.generalized_file_reader}.

\paragraph{Deterministic state automata}
...
The picture in \cref{ex.Moore_three} may look familiar to those who have studied automata theory; in fact, a deterministic state automaton can be expressed as a Moore machine (with a preset initial state).
...
\end{enumerate}
Given a sequence (or ``word'') $(a_1,\ldots,a_n)$ of elements from $A$, we say that the automaton \emph{accepts} this word if starting at the initial state and following the elements in the sequence leads us to an accept state---or, more formally, if the sequence $(s_0,s_1,\ldots,s_n)$ defined
\[
    s_{k+1}\coloneqq u(s_k,a_{k+1})
\]
for all $k\in\nn$ is such that $s_n$ is an accept state: $s_n\in F$.

We call any set of words comprised of elements in $A$ a \emph{language}, and we say that the set of all words that the automaton accepts is the language \emph{recognized} by the automaton.
\end{definition}
...
\section{Dependent systems}\label{sec.poly.dyn_sys.depend_sys}
%again, to do this requires polynomials (in this case, $\yon^A+1$), which we'll get to in \cref{sec.poly.dyn_sys.depend_sys}.
...
A \emph{dependent system} (or a \emph{dependent Moore machine}) is a lens
\[S\yon^S\to p\]
...
The output of a dependent system with interface $p$ is a $p$-position: an element of the set $p(\1)$.
Then the input must come from the direction-set $p[i]$.
...
\begin{remark}
It may seem limiting that the set of possible inputs of a dependent system should depend on the current \emph{output}, rather than the current \emph{state}.
...
Philosophically, the answer to this question is ``no'' as long as we think about the interface of a dependent system as capturing everything about how the system interacts with the outside world.
...
But however you think of dependent systems, we need to get a feel for how they work mathematically.
We'll do this by going through many examples.
...
\]
To give such a map, we provide an on-positions function $\phi_\1\colon S\to\2$ (here we are thinking of $\2$ as the position-set of $\yon^A+\1$).
...
In other words, the on-directions function $\phi^\sharp_s$ is trivial when $\phi_\1(s)=2$.

On the other hand, $\phi^\sharp_s$ is not trivial on those elements $s\in S$ for which $\phi_\1(s)=1$.
Instead, these on-directions functions $\phi^\sharp_s\colon A\to S$ specify how the machine updates its state on each input from $A$, if the current state is $s$.
This is in line with the way the automaton's update function behaves.

When equipped with an initial state $s_0\in S$ specified by a map $\yon\to S\yon^S$, we call these dependent systems \emph{halting deterministic state automata}.
Given a sequence (or ``word'') $(a_1,\ldots,a_n)$ of elements from $A$, we say that the automaton \emph{accepts} this word if starting at the initial state and following the elements in the sequence leads us to an accept state, \emph{without hitting an accept state and stopping too early}---or, more formally, if there exists a sequence $(s_0,s_1,\ldots,s_n)$ such that $\phi_\1(s_n)=2$ and, for all $k\in[0,n-1]$, we have $\phi_\1(s_k)=1$ and
\[
...
\begin{enumerate}
	\item What is $S$?
	\item What is $A$?
	\item Based on the picture, which states are accept states, and which are not?
...
\begin{enumerate}
    ...
    According to the previous part, $\phi$ has an on-positions function $\phi_\1\colon S\to\2$ sending states 1 and 2, as non-accept states, to 1, and sending state 3, as an accept state, to 2.
    Then the two nontrivial on-directions functions behave as follows, according to the \eqref{eqn.hdsa}:
    \begin{align*}
...
The graph itself can be seen as a dynamical system $f \colon V\yon^V \to g$, where $f_\1 = \id_V$ and $f^\sharp(v, e) = t(e)$.
\end{example}
...
\begin{exercise}
Find what you think is an interesting generalization of deterministic finite state automata that you can model using dependent Moore machines.
\begin{solution}
**
\end{solution}
\end{exercise}
...
\end{tikzpicture}
\]
Then a dependent Moore machine of the form $S\yon^S\to p$, where
\[
...
\begin{enumerate}
	...
	\item Define a lens $S\yon^S\to p$ in a way that respects positions and properly updates the robots list of rewards, but otherwise does anything you want.
\qedhere
\end{enumerate}
\begin{solution}
**
...
two modes, each of which returns an ascii character:
...
return the ascii character at the current position.
\begin{align*}
  \return{FileReader}(s, t) = \begin{cases} \fun{Accepting}(f(s)) &\mbox{if $s = t$}\\ \fun{Busy}((f(s)))  \end{cases}
\end{align*}
...
to be the input. 
\begin{align*}
  \update{FileReader}(s, t) = \begin{cases} \_ \mapsto (s + 1, t) &\mbox{if $\return{FileReader}(s, t)$ is $\fun{Busy}$} \\ (s', t') \mapsto (s', t') &\mbox{if $\return{FileReader}(s, t)$ is $\fun{Accepting}$}  \end{cases}
\end{align*}
...
\[
(r_\1,r^\sharp)\colon A\yon^A\to \Set{ascii}\mdot\yon^A+\Set{ascii}\mdot\yon^\1
\]
as follows.
...
\end{proposition}
\begin{proof}
We are given lenses $S\yon^S\to B_1\yon^{A_1}$ and $S\yon^S\to B_2\yon^{A_2}$. Hence, by the universal property of products, we have a map
\[
  S\yon^S\to
...
By \cref{exc.poly_prod}, if each $(A_i,B_i)$-Moore machine has $\text{return}_i$ as its return function and $\text{update}_i$ as its update function, then the induced $\left(\sum_{i \in \ord{n}} A_i, \prod_{i \in \ord{n}} B_i\right)$-Moore machine has a return function that sends $s \in S$ to the $n$-tuple $(\text{return}_i(s))_{i \in \ord{n}}$ and an update function that sends $(s, a) \in S \times \sum_{i \in \ord{n}} A_i$ to $\text{update}_i(s, a)$ if $a \in A_i$.
...
This single Moore machine can receive inputs from any one of the original machines' input sets and update its state accordingly; it then returns output in all of the original machines at once. It's as though each of the machines can see where the combined system is at any time, but only one of them can actually operate it at any given time.
...
If $\yon$ were the only interface acting on the system, we would have a Moore machine $g \colon S\yon^S \to \yon$ whose return function is the unique function $S \to \1$ and whose update function is the identity function on $S$, since the input never changes the system.
...
In particular, $p \iso B\yon^{A + \1}$, while $f'$ consists of a return function $S \to B$ that is the same as the return function of $f$ and an update function $S \times (A + \1) \to S$ that behaves like the update function of $f$ when the input is from $A$ but does not change the state when the input is from $\1$.
\end{solution}
\end{exercise}
...
\end{proposition}
\begin{proof}
We are given lenses $S_1\yon^{S_1}\to B_1\yon^{A_1}$ and $S_2\yon^{S_2}\to B_2\yon^{A_2}$, so we can take their parallel product to get a map
\[
...
By \cref{prop.dirichlet_monoidal}, if each $(A_i,B_i)$-Moore machine has $\text{return}_i$ as its return function and $\text{update}_i$ as its update function, then the induced $\left(\prod_{i \in \ord{n}} A_i, \prod_{i \in \ord{n}} B_i\right)$-Moore machine has a return function that sends the $n$-tuple of states $(s_i)_{i \in \ord{n}} \in \prod_{i \in \ord{n}} S$ to the $n$-tuple of outputs $(\text{return}_i(s))_{i \in \ord{n}}$ and an update function that sends $((s_i)_{i \in \ord{n}}, (a_i)_{i \in \ord{n}}) \in S \times \prod_{i \in \ord{n}} A_i$ to $(\text{update}_i(s_i, a_i))_{i \in \ord{n}}$.
...
This single Moore machine can receive inputs from any one of the original machines' input sets and update its state accordingly; it then returns output in all of the original machines at once.
...
\begin{enumerate}
	...
	\item What is the lens corresponding to this diagram?
\qedhere
\end{enumerate}
\begin{solution}
**
...
\]
In other words we have a lens $ST\yon^{ST}\to \const{Plant}\otimes\const{Controller}$; that's a new dynamical system with state space $ST=(S\times T)$; a state in it is just a pair of states, one in $S$ and one in $T$. Furthermore our wiring diagram already gave us a map $\const{Plant}\otimes\const{Controller}\to\const{System}$, so combining, we have a new system
\[
...
Let's use these vectors to define the internal dynamics of each system. Each system will hold as its internal state its current position and velocity, i.e.\ $S=\rr^\2\times\rr^\2$. To define a lens $S\yon^S\to\rr^\2\yon^{\rr^\2-(0,0)}$ we simply output the current position and update the current velocity by adding a vector pointing to the other system and having appropriate magnitude:
\begin{align*}
...
Let $p,q\coloneqq\nn\yon^\nn$.
\begin{enumerate}
	\item Write a lens $p\otimes q\to\yon$ that corresponds to the function $(a,b)\mapsto (b,a+b)$.
...
So what is a map $p_1\otimes\cdots\otimes p_k\to q$ in general? It's a protocol by which the $k$-many participants $p_i$ together decide what decision $q$ must make, as well as how $q$'s choice among its options (the decision once made) is passed back and distributed as an option at each $p_i$.
...
To do so, we need to create a contract $p_1\otimes p_2\to p$ of the external arena $p$ around (the arenas of) the internal systems. That is, we need to give a lens
\[
...
\footnote{And if you want the generic way for $p$ to interact with other systems in a box $r$, just use $\ihom{p,r}$.}
...
\begin{example}[Chu $\&$]
Suppose we are given polynomials $p_1,p_2,q_1,q_2,r\in\poly$ and morphisms
\[
...
Let $f \colon p \to q$ be a morphism in $\poly$. It is a monomorphism if and only if the on-positions function $f_\1 \colon p(\1) \to q(\1)$ is a monomorphism in $\smset$ and, for each $i \in p(\1)$, the on-directions function $f_i^\sharp \colon q[f_\1(i)]\to p[i]$ is an epimorphism in $\smset$.
\end{proposition}
\begin{proof}
...
We now need to show that for any $i\in p(\1)$, the on-directions function $f_i^\sharp \colon q[f_\1(i)] \to p[i]$ is an epimorphism.
...
Let $f \colon p \to q$ be a morphism in $\poly$. It is an epimorphism if and only if the function $f_\1 \colon p(\1) \to q(\1)$ is an epimorphism in $\smset$ and, for each $j\in q(\1)$, the induced function
\[
...
\begin{solution}
Given sets $A$ and $B$, by \cref{prop.epis_in_poly}, a morphism $f \colon \yon^A + \yon^B \to \yon^{AB}$ is an epimorphism if its on-positions function $f_\1 \colon \2 \to \1$ is an epimorphism (which must be true) and if the induced function
\[
...
\[
    p'(\1) \coloneqq \{i \in p(\1) \ | \ f_\1(i) = g_\1(i)\}.
\]
...
Here $p \in \poly$.
\begin{enumerate}
    \item The canonical morphism $\eta \colon p \to p(\1)$ is the identity $\eta_\1 \colon p(\1) \to p(\1)$ on positions and the empty function on directions.
	...
	\end{tikzcd}
	\]
	So $p_i(\1) \coloneqq \{(a, i') \in \1 \times p(\1) \ | \ i = i' \} = \{(1, i)\}$, with $f_\1$ uniquely determined and $g_1$ picking out $i \in p(\1)$.
...
\begin{enumerate}
    ...
    \[
        p(\1) \coloneqq \{(i, i') \in \2 \times \3 \ | \ f_\1(i) = f'_\1(i)\} = \{(1,1), (2,2), (2,3)\}.
    \]
...
The category of elements $\int G_c$ has objects $E_c+V_c$ and two kinds of (non-identity) morphisms, $e \to s_\1(e)$ and $e \to t_\1(e)$, associated to each $e \in E_c$, all pointing from an object in $E_c$ to an object in $V_c$. %**This is a cofunctor into graph category, right?
...
\begin{definition}[Vertical, cartesian] \label{def.vert_cart}
...
%**orthogonal factorization system? some "up to iso" subtleties
\begin{proposition}\label{prop.vert_cart_factorization}
Vertical and cartesian morphisms form a factorization system of $\poly$.
\end{proposition}
\begin{proof}
...
It remains to show that every morphism in $\poly$ can be uniquely (up to unique isomorphism) factored as a vertical morphism composed with a cartesian morphism.
...
\end{tikzcd}
\]
The intermediary object $\sum_{i\in p(\1)} \yon^{q[f_\1(i)]}$ is clearly unique up to unique isomorphism.
\end{proof}
...
Here is an alternative characterization of a cartesian morphism in $\poly$.
...
Show that a morphism $f\colon p\to q$ in $\poly$ is cartesian if and only if the square on the left hand side of \eqref{eqn.poly_map_usu} is also a pullback:
\[
\begin{tikzcd}
	\dot{p}(\1)\ar[d, "\pi_p"']&
	\bullet\ar[l, "f^\sharp"']\ar[r]\ar[d]&
...
\]
\begin{solution}
We wish to show that a morphism $f\colon p\to q$ in $\poly$ is cartesian if and only if the square on the left hand side of \eqref{eqn.poly_map_usu} is a pullback.
...
Let $f\colon p\to q$ be a morphism in $\poly$. The following are equivalent:
	\begin{enumerate}
		\item viewed as a lens, $f$ is a cartesian morphism in the sense of \cref{def.vert_cart}: for each $i\in p(\1)$, the on-directions function $f^\sharp_i$ is a bijection;
...
In the following, let $f\colon p\to q$ be a cartesian morphism in $\poly$ and $h\colon A\to B$ be a function.
\begin{enumerate}
    \item Using \cref{prop.morph_arena_to_func} to translate $f$ from a morphism in $\poly$ to a natural transformation and \cref{prop.poly_on_functions} to interpret $q(h)$, characterize the pullback of $p(B)\To{f_B}q(B)\From{q(h)}q(A)$ in $\smset$.
...
In the following, let $f\colon p\to q$ be a morphism in $\poly$ that is cartesian when viewed as a natural transformation, so that \eqref{eqn.cart_nt_pullback} is a pullback for any function $h\colon A\to B$.
Also fix $i\in p(\1)$.
\begin{enumerate}[resume]
...
Here $f\colon p\to q$ is a cartesian morphism in $\poly$ and $h\colon A\to B$ is a function.
\begin{enumerate}
...
\end{enumerate}
Next, we will show that $3\Rightarrow1$ in \cref{prop.cart_as_nt}, with $f\colon p\to q$ as a morphism in $\poly$ that is a cartesian natural transformation and $i\in p(\1)$.
\begin{enumerate}[resume]
...
We can obtain the on-positions function of a morphism by passing it through the functor $\poly\To{p(\1)}\smset$ from \cref{thm.adjoint_quadruple}.
...
A position of $p\times q$ (resp.\ of $p\otimes q$) is a pair $(i,j)\in p(\1)\times q(\1)$. The morphism $(f\times g)^\sharp_{(i,j)}$ (resp.\ $(f\otimes g)^\sharp_{(i,j)}$) is $f^\sharp_i+g^\sharp_j$ (resp.\ $f^\sharp_i\times g^\sharp_j$) which is again an isomorphism if $f^\sharp_i$ and $g^\sharp_j$ are. Hence $f\times g$ (resp.\ $f\otimes g$) is cartesian, completing the proof.
\end{proof}

\begin{proposition}\label{prop.pullback_vert_cart}
Pullbacks preserve vertical (resp.\ cartesian) morphisms.
In other words, if $f\colon p\to q$ is a morphism and $g\colon q'\to q$ a vertical (resp.\ cartesian) morphism, then the pullback $g'$ of $g$ along $p$
\[
\begin{tikzcd}
...
For example, suppose one has a set $A$ and a function $f\colon A\to p(\1)$, which we can also think of as a cartesian morphism between constant polynomials.
...
\end{tikzcd}
\end{equation}
Here $\eta_p$ is the unit of the adjunction $\adjr{\smset}{A}{p(\1)}{\poly}$; it is a vertical morphism.
...
\]
We'll see this as part of a bigger picture in \cref{prop.basechange,thm.triple_adjoint_basechange}, but first we need the following definition and result about cartesian morphisms.
...
The functor $p\mapsto p(\1)$ is strong monoidal with respect to $\otimes$ and strict monoidal if we choose the lens construction as our model of $\poly$. By \cref{prop.monoidal_pres_vert_cart}, the monoidal product $\otimes$ preserves cartesian morphisms; thus we will have established the desired monoidal $*$-bifibration in the sense of \cite[Definition 12.1]{shulman2008framed} as soon as we know that $\otimes$ preserves op-cartesian morphisms.
