...
It turns out that bimodules between comonoids in $\poly$ (categories) are also important objects of study in category theory. If $\cat{C}$ and $\cat{D}$ are categories, Richard Garner showed that a bimodule between them can be identified with what's known as a \emph{parametric right adjoint} between the associated copresheaf categories $[\cat{C},\smset]$ and $[\cat{D},\smset]$. Parametric right adjoints, or \emph{pra}'s come up in $\infty$-category theory, but they also have a much more practical usage: they are the so-called \emph{data migration functors}.
...
\[
[\cat{C},\smset]\coloneqq\Cat{Fun}(\cat{C},\smset)
\]
...
\end{enumerate}
	\[
	\elts^\cat{C}\colon[\cat{C},\smset]\to\Cat{dopf}(\cat{C})
\qedhere	
	\]
\end{exercise}
...
\begin{enumerate}
	\item the category $[\cat{C},\smset]$ of functors $\cat{C}\to\smset$,
...
\end{proposition}
\begin{proof}
By \cref{exc.elts_functor} we have a functor $\elts^\cat{C}\colon[\cat{C},\smset]\to\Cat{dopf}(\cat{C})$. There is a functor going back: given a discrete opfibration $\pi\colon\cat{S}\to\cat{C}$, we define $(\partial\pi)\colon\cat{C}\to\smset$ on $c\in\Ob(\cat{C})$ and $f\colon c\to c'$ by 
\begin{align*}
	(\partial\pi)(c)&\coloneqq\{s\in\cat{S}\mid\pi(s)=c\}\\
	(\partial\pi)(f)(s)&\coloneqq\bar{f}(s).
\end{align*}
On objects, the roundtrip $[\cat{C},\smset]\to[\cat{C},\smset]$ sends $I\colon\cat{C}\to\smset$ to the functor
\begin{align*}
	c&
...
\begin{example}
In \cref{ex.cofree_dyn_sys} we had a dynamical system with $S\coloneqq\{\bul[dgreen],\bul[dyellow],\bul[red]\}$ and $p\coloneqq\yon^\2+\1$, and  $f\colon S\yon^S\to p$ from \cref{exc.det_fsa_misc_398}, depicted here again for your convenience:
...
Let $\cat{C}=(\car{c},\epsilon,\delta)$ and $\cat{D}=(\car{d},\epsilon,\delta)$ be comonoids. A \emph{$(\cat{C},\cat{D})$-bimodule} $(m,\lambda,\rho)$, denoted $\cat{C}\bimodfrom[m]\cat{D}$ or in shorthand $\car{c}\bimodfrom[m]\car{d}$,%
%
...
\end{tikzcd}
\end{gather}
Just $(m,\lambda)$ and the first line of diagrams is called a left $\cat{C}$-module, and similarly just $(m,\rho)$ and the second line of diagrams is called a right $\car{D}$-module.
...
\begin{exercise}
\begin{enumerate}
	\item Draw the equations of \eqref{eqn.bimod_left} using polyboxes.
	\item Draw the equations of \eqref{eqn.bimod_right} using polyboxes.
\qedhere
\end{enumerate}
\end{exercise}
...
\end{tikzcd}
\]
In other words, it is a left $\car{c}$-comodule whose carrier is constant on a set. A morphism is a function $S\to T$ commuting with $\alpha$, just as for comodules; see \cref{def.bimodule}.
\end{definition}
...
\begin{description}
 ...
 We give the duplicator $\delta\colon\car{s}\to\car{s}\tri\car{s}$ using $\alpha_2$ for the codomain, and using the composite $\then$ from $\car{c}$:
  ...
  In \cref{exc.tfae_c_sets} we check that $(\car{s},\epsilon,\delta)$ really is a comonoid, that $(\alpha_1,\id)\colon\car{s}\cof\car{c}$ is a cofunctor, that the roundtrips between cartesian cofunctors and coalgebras are identities, and that these assignments are functorial.
	...
	One checks easily that if $S\yon\to\car{c}\tri S\yon)$ corresponds to $S\to\car{c}\tri S$ under the above isomorphism, then one is a left module iff the other is.
...
Let $\com{C}=(\car{c},\epsilon,\delta)$ be a comonoid and suppose that $\rho\colon m\to m\tri\car{c}$ is a right $\com{C}$-module. Then for any set $A$ and function $f\colon A\to m\tri\1$, the polynomial $f^*m$ has an induced right module structure $\rho_f$ fitting into the commutative square below:
\[
\begin{tikzcd}
  f^*m\ar[d]\ar[r, "\rho_f"]&
  f^*m\tri\car{c}\ar[d]\\
...
Let $\com{C}=(\car{c},\epsilon,\delta)$ be a comonoid in $\poly$. For any set $G$, the polynomial $\yon^G\tri\car{c}$ has a natural right $\com{C}$-module structure.
\end{proposition}
\begin{proof}
...
\begin{proposition}
Functions $i'\colon G\to\car{c}\tri\1$ are in bijection with positions $i\in\yon^G\tri\car{c}\tri\1$. Let $m\coloneqq i^*(\yon^G\tri\car{c})$ and let $\rho_i$ be the induced right $\com{C}$-module structure from \cref{prop.right_modules_as_sums}. Then $\rho_i$ corresponds to the free $\cat{C}$-set generated by $i'$. 
\end{proposition}
\begin{proof}
...
\begin{enumerate}
    ...
    \item Show that $\delta^i$ makes $\yon^{\car{c}[i]}$ a right $\com{C}$-module.
    \qedhere
\end{enumerate}
\end{exercise}
...
\begin{proposition}
For any two categories $\car{c},\car{d}$, the category $\bimod{\car{d}}{\car{c}}$ of bimodules between them is a rig category: it has a terminal object (carried by $\car{d}\tri\1$), an initial object (carried by $\0$), as well as products that distribute over coproducts.
\end{proposition}
\begin{proof}
For bimodules $\car{d}\bimodfrom[m]\car{c}$ and $\car{d}\bimodfrom[n]\car{c}$, the coproduct has carrier $m+n$, and the product has carrier $m\times_{d\tri\1}n$. **Finish**
\end{proof}
...
Let $\cat{C}$ and $\cat{D}$ be categories; the following conditions on a functor $F\colon[\cat{C},\smset]\to[\cat{D},\smset]$ are equivalent.
\begin{enumerate}
...
Consider a category $\cat{C}$ and a bimodule $\cat{C}\bimodfrom[p]\cat{C}$ from it to itself. Since $[\cat{C},\smset]$ is locally cartesian closed, one could ask whether $p$ arises from the bridge diagram
\[
\begin{tikzcd}
	&
	E\ar[r]\ar[dl]&
...
Let $\cat{C}$ and $\cat{D}$ be categories. A \emph{pra-functor} (also called a \emph{parametric right adjoint functor}) $[\cat{C},\smset]\to[\cat{D},\smset]$ is one satisfying any of the conditions of \cref{prop.prafunctor}
\end{definition}
...
\end{theorem}
\begin{proof}
The functor $\Fun{Copsh}$ sends $\cat{C}$ to the corresponding copresheaf category $[\cat{C},\smset]$ by \cref{thm.tfae_c_sets}.
...
\begin{proposition}
If $\car{c}\bimodfrom[m_1]\car{d}$ and $\car{c}\bimodfrom[m_2]\car{d}$ are bimodules, then so are
\[
...
\begin{proposition}
If $\car{c}_1\bimodfrom[m_1]\car{d}_1$ and $\car{c}_2\bimodfrom[m_2]\car{d}_2$ are bimodules, then so are
\[
...
\begin{proposition}
For any category $\cat{C}$, the category $\smset[\cat{C}]$ is the free coproduct completion of $[\cat{C},\smset]\op$.
\end{proposition}
\begin{proof}
**
\end{proof}
...
\smset[\cat{C}]\to\poly
\]
sending $m\to m\tri\car{c}$ to $m\in\poly\cong\smset[\yon]$ is given by composition with the principal $\cat{C}$-module $\cat{C}\bimodfrom[\widecheck{\car{c}}]\yon$.
\end{proposition}
\begin{proof}
**
\end{proof}
\begin{proposition}
The functor $\eta_{\cat{C}} \colon \cat{C} \to \smset[\cat{C}]$ corresponding to the unit bimodule $\car{c} \bimodfrom[\car{c}] \car{c}$ sends each object $i \in \car{c}(\1)$ to the bimodule $\yon \bimodfrom[\yon ^{\car{c}[i]}]\car{c}$.
\end{proposition}

\begin{proposition}
The functor $[\cat{C},\smset]\op \to \smset[\cat{C}]$ has a left adjoint, 
\[
\adjr{[\cat{C},\smset]\op}{\yon ^ {-}}{\Gamma}{\smset[\cat{C}]}.
\]
\end{proposition}
\begin{proof}
**
...
\begin{proposition}
For any categories $\cat{C}$ and $\cat{D}$ and $(\cat{D},\cat{C})$-bimodule $\car{d}\bimodfrom[m]\car{c}$, we have
\[
m\tri_\car{c}\car{c}\cong m
\]
where $\car{c}\bimodfrom[\car{c}]\yon$ is the regular left $\car{c}$-module.
\end{proposition}
...
\begin{proposition}
A functor $F\colon\cat{C}\to\cat{D}$ gives rise to a right adjoint bimodule (= left adjoint pra) $\car{c}\bimodfrom[\Delta_F]\car{d}$.
\end{proposition}
\begin{proof}
...
\begin{proposition}
A cofunctor $\varphi\colon\cat{C}\cof\cat{D}$ gives rise to a left adjoint bimodule (= right adjoint pra) $\car{c}\bimodfrom[\wh{\varphi}]\car{d}$; we denote its right adjoint bimodule (= left adjoint pra) by $\wc{\varphi}$,
...
\begin{proposition}
Every bimodule $\car{c}\bimodfrom[S]\0$, is isomorphic to the composite
\[
\car{c}\bimodfrom[S\yon]\yon\bimodfrom[\1]\0
\]
for a bimodule $\car{c}\bimodfrom[S\yon]\yon$ with carrier $S\yon\in\poly$.

Moreover, $S\yon$ is adjoint to a bimodule $\yon\bimodfrom[\yon^S]\car{c}$ with carrier $\yon^S\in\poly$.
\end{proposition}
\begin{proof}
...
For any bimodules $\car{c}_1\bimodfrom[m_1]\car{d}_1$ and $\car{c_2}\bimodfrom[m_2]\car{d_2}$, we can construct a new bimodule in two ways:
\[
	\car{c}_1+\car{c}_2\bimodfrom[m_1+m_2]\car{d}_1+\car{d}_2
	\qqand
	\car{c}_1\otimes\car{c}_2\bimodfrom[m_1\otimes m_2]\car{d}_1\otimes \car{d}_2
