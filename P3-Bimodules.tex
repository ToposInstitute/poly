...
\begin{description}
  ...
  \end{tikzpicture}
	...
	\end{align*}
	Going backward, if we're given a coalgebra $\alpha\colon S\to\ema{c}\tri S$, we obtain a function $\alpha_1\colon S\to\ema{c}\tri\1$ and we define $\ema{s}\coloneqq \alpha_1^*\ema{c}$ and the cartesian map $\varphi\coloneqq(\alpha_1,\id)\colon\ema{s}\to\ema{c}$ to be the base change from \cref{prop.basechange}. We need to show $\ema{s}$ has a comonoid structure $(\ema{s},\epsilon,\delta)$ and that $\varphi$ is a cofunctor. We simply define the eraser $\epsilon\colon\ema{s}\to\yon$ using $\alpha_1$ and the eraser on $\ema{c}$:
	\[
	\begin{tikzpicture}[polybox, mapstos]
	...
	\end{tikzpicture}
	\]
 We give the duplicator $\delta\colon\ema{s}\to\ema{s}\tri\ema{s}$ using $\alpha_2$ for the codomain, and using the composite $\then$ from $\ema{c}$:
	\[
  \begin{tikzpicture}[polybox, mapstos]
...
\smset[\cat{C}]\to\poly
\]
sending $m\to m\tri\ema{c}$ to $m\in\poly\cong\smset[\yon]$ is given by composition with the principal $\cat{C}$-module $\cat{C}\bimodfrom[\widecheck{\ema{c}}]\yon$.
\end{proposition}
\begin{proof}
**
\end{proof}
