...
In this part we'll consider another kind of morphism between comonoids in $\poly$, i.e.\ categories, and one that is a bit more familiar than cofunctors. Namely, we'll consider the bicomodules between comonoids. So why do I say they're more familiar?
...
\[
\smset^{\cat{C}}\coloneqq\Cat{Fun}(\cat{C},\smset)
\]
...
In a monoidal category, one can not only consider monoids and comonoids, but also modules between these. For example, in the monoidal category of abelian groups under bilinear product, the monoid objects are rings and the bicomodules are bicomodules in the usual sense. Here we are interested in comonoids rather than monoids, so we should be interested in bi-comodules; we will just call these bicomodules for linguistic convenience.

We will see that bicomodules in $\poly$ are equivalent to data-migration functors. Given comonoids/categories $\cat{C}$ and $\cat{D}$, a bicomodule $\cat{C}\bimodfrom[m]\cat{D}$ is a way of moving database instances on $\cat{D}$ to database instances on $\cat{C}$. One could call it a ``$\cat{D}$-indexed union of conjunctive queries.''

\begin{definition}[Bicomodule]\label{def.bicomodule}
Let $\cat{C}=(\car{c},\epsilon,\delta)$ and $\cat{D}=(\car{d},\epsilon,\delta)$ be comonoids. A \emph{$(\cat{C},\cat{D})$-bicomodule} $(m,\lambda,\rho)$, denoted $\cat{C}\bimodfrom[m]\cat{D}$ or in shorthand $\car{c}\bimodfrom[m]\car{d}$,%
%
\footnote{Note that the symbol $\car{c}\bimodfrom\car{d}$ looks like it's going backward, from $\car{d}$ to $\car{c}$; this is good because we'll see that this is the direction of data migration for a $(\cat{C},\cat{D})$-bicomodule. But the symbology is also mnemonically good because the $\tri$'s go in the correct direction $\car{c}\tri m\from m$ and $m\to m\tri\car{d}$.}
%
consists of
%
\begin{enumerate}
	\item a polynomial $m\in\poly$,
	\item a morphism $\car{c}\tri m\From{\lambda}m$, and
	\item a morphism $m\To{\rho}m\tri\car{d}$,
\end{enumerate}
%
...
\begin{tikzcd}[ampersand replacement=\&]
	...
	m\tri\car{d}\ar[d, "\lambda\tri\car{d}"]\\
	\car{c}\tri m\ar[r, "\car{c}\tri\rho"']\&
	\car{c}\tri m\tri\car{d}
\end{tikzcd}
\end{gather}
Just $(m,\lambda)$ and the first line of diagrams is called a left $\cat{C}$-comodule, and similarly just $(m,\rho)$ and the second line of diagrams is called a right $\car{D}$-comodule.

A \emph{morphism} of $(\cat{C},\cat{D})$-bicomodules is a map $m\to n$ making the following diagram commute:
\[
\begin{tikzcd}
	\car{c}\tri m\ar[d]&
	m\ar[l]\ar[r]\ar[d]&
...
\end{tikzcd}
\]
We denote the category of $(\cat{C},\cat{D})$-bicomodules by $\bimod{\cat{C}}{\cat{D}}$.
\end{definition}
...
Note that \eqref{eqn.bimod_coherence_polybox} means that we can unambiguously write
\[
\begin{tikzpicture}[polybox, tos]
	\node[poly, dom, "$m$" left] (m) {};
...
\begin{enumerate}
	\item Show that a left $\cat{C}$-comodule is the same thing as a $(\cat{C},\yon)$-bicomodule.
	\item Show that a right $\cat{C}$-comodule is the same thing as a $(\yon,\cat{C})$-bicomodule.
	\item Show that every polynomial $p\in\poly$ has a unique $(\yon,\yon)$-bicomodule structure.
	\item Show that there is an isomorphism of categories $\poly\cong\bimod{\yon}{\yon}$.
\qedhere
\end{enumerate}
\end{exercise}

\begin{definition}[$\com{C}$-coalgebra]
Let $\com{C}=(\car{c},\epsilon,\delta)$ be a comonoid. A \emph{$\com{C}$-coalgebra} consists of a set $S$ and a function $\alpha\colon S\to\car{c}\tri S$, making the two diagrams below commute:
\[
\begin{tikzcd}
	\car{c}\tri S\ar[d, "\epsilon\tri "']&
...
\end{tikzcd}
\]
In other words, it is a left $\car{c}$-comodule whose carrier is constant on a set. A morphism is a function $S\to T$ commuting with $\alpha$, just as for comodules; see \cref{def.bicomodule}.
\end{definition}

In order to as quickly as possible orient the reader to bicomodules, we begin by proving the following.

\begin{theorem}\label{thm.tfae_c_sets}
For a comonoid $\com{C}=(\car{c},\epsilon,\delta)$ (category $\cat{C}$), the following categories are equivalent:
\begin{enumerate}
	\item functors $\cat{C}\to\smset$;
	\item discrete opfibrations over $\cat{C}$;
	\item cartesian cofunctors to $\com{C}$;
...
Let $\com{C}=(\car{c},\epsilon,\delta)$ be a comonoid and suppose that $\rho\colon m\to m\tri\car{c}$ is a right $\com{C}$-comodule. Then for any set $A$ and function $f\colon A\to m\tri\1$, the polynomial $f^*m$ has an induced right comodule structure $\rho_f$ fitting into the commutative square below:
\[
\begin{tikzcd}
  f^*m\ar[d]\ar[r, "\rho_f"]&
  f^*m\tri\car{c}\ar[d]\\
...
\end{proposition}
\begin{proof}
The pullback diagram to the left defines $f^*(m)$ and that to the right is its composition with $\car{c}$
\[
\begin{tikzcd}
	f^*m\ar[r]\ar[d]&
	m\ar[d]\\
...
\hspace{.7in}
\begin{tikzcd}
	f^*m\tri\car{c}\ar[r]\ar[d]&
	m\tri\car{c}\ar[d]\\
	A\ar[r, "f"']&
	m\tri\1\ar[ul, phantom, very near end, "\lrcorner"]
\end{tikzcd}
\]
which is again a pullback by \cref{thm.connected_limits,exc.composing_with_constants}. Now the map $\rho\colon m\to m\tri\car{c}$ induces a map $\rho_f\colon f^*(m)\to f^*(m)\tri c$; we claim it is a right comodule. It suffices to check that $\rho_f$ interacts properly with $\epsilon$ and $\delta$, which we leave to \cref{exc.right_modules_as_sums}.
\end{proof}

\begin{exercise}\label{exc.right_modules_as_sums}
Let $\car{c},\epsilon,\delta)$, $\rho\colon m\to m\tri\car{c}$, and $f\colon A\to m\tri\1$ be as in \cref{prop.right_modules_as_sums}. Complete the proof of that proposition as follows:
\begin{enumerate}
	\item Show that $\rho_f\then\epsilon=\id_m$
	\item Show that $\rho_f\then(f^*m\tri\delta)=\rho_f\then(\rho_f\tri\car{c})$.
\qedhere
\end{enumerate}
\end{exercise}
...
\begin{proposition}
The bicategory of polynomial functors in the sense of \cite{kock2012polynomial} is precisely that of bicomodules between discrete categories.
\end{proposition}
\begin{proof}
**
\end{proof}
...
\begin{exercise}
\begin{enumerate}
    \item Show that $\yon^{\car{c}[f]}$ is a map of right $\com{C}$-modules.
...
Let $\com{C}$ be a comonoid. For any set $I$ and right $\com{C}$-modules $(m_i)_{i\in I}$, the coproduct $m\coloneqq \sum_{i\in I}m_i$ has a natural right-comodule structure. Moreover, each representable summand in the carrier $m$ of a right $\com{C}$-comodule is itself a right-$\com{C}$ comodule and $m$ is their sum.
\end{proposition}
\begin{proof}
**
\end{proof}
...
%-------- Section --------%
\section{Morphisms between bicomodules}

\begin{proposition}
For any two categories $\car{c},\car{d}$, the category $\bimod{\car{d}}{\car{c}}$ of bicomodules between them is a rig category: it has a terminal object (carried by $\car{d}\tri\1$), an initial object (carried by $\0$), as well as products that distribute over coproducts.
\end{proposition}
\begin{proof}
For bicomodules $\car{d}\bimodfrom[m]\car{c}$ and $\car{d}\bimodfrom[n]\car{c}$, the coproduct has carrier $m+n$, and the product has carrier $m\times_{d\tri\1}n$. **Finish**
\end{proof}

\begin{proposition}
For any two categories $\car{c},\car{d}$, the category $\bimod{\car{d}}{\car{c}}$ has all limits and is extensive.
\end{proposition}
\begin{proof}
**
\end{proof}
...
%---- Subsection ----%
\subsection{Adjoint bicomodules}

\begin{exercise}
Recall that there is a unique $(\0,\cat{C})$-bicomodule, namely $\0\bimodfrom[\0]\cat{C}$.
\begin{enumerate}
...
Consider a category $\cat{C}$ and a bicomodule $\cat{C}\bimodfrom[p]\cat{C}$ from it to itself. Since $\smset^{\cat{C}}$ is locally cartesian closed, one could ask whether $p$ arises from the bridge diagram
\[
\begin{tikzcd}
	&
	E\ar[r]\ar[dl]&
...
Let $\cat{C}$ and $\cat{D}$ be categories. A \emph{pra-functor} (also called a \emph{parametric right adjoint functor}) $\smset^{\cat{C}}\to\smset^{\cat{D}}$ is one satisfying any of the conditions of \cref{prop.prafunctor}
\end{definition}
...
\end{theorem}
\begin{proof}
The functor $\Fun{Copsh}$ sends $\cat{C}$ to the corresponding copresheaf category $\smset^{\cat{C}}$ by \cref{thm.tfae_c_sets}.
...
%\end{array}
%\]
%Now why would we want to do this? The answer is that the maps of bicomodules from this instance to the Deck are exactly what are called the \emph{suggestions} in the original Clue game. There are $\3\0$ of them:
%\[
%\left\{
%\begin{array}{lllll}
%  (\circ,O,\cdot),&
...
%
%\begin{exercise}
%Show that the $\3\3$-card deck from \cref{ex.clue_suggestions} factors---up to isomorphism---through a bicomodule of the form $\yon^\4+\2\yon^\2+\yon\bimodfrom\yon^\2+\yon$, where $\yon^\2+\yon$ carries the walking arrow.
%\end{exercise}
...
%-------- Section --------%
\section{Composing bicomodules}

We denote the composite of $\car{e}\bimodfrom[n]\car{d}\bimodfrom[m]\car{c}$ by 
\[\car{e}\bimodfrom[n\tri_\car{d}m]\car{c}.\]

Recall the Yoneda functor $\yon^{\car{c}[-]} \colon \cat{C} \to \bimod{\yon}{\cat{C}}$ from \cref{def.yoneda_functor}.
Using bicomodule composition, we obtain the composite functor
\[
    \cat{C} \times \bimod{\cat{C}}{\cat{D}} \To{\yon^{\car{c}[-]} \times \bimod{\cat{C}}{\cat{D}}} \bimod{\yon}{\cat{C}} \times \bimod{\cat{C}}{\cat{D}} \To[\tri_\cat{C}] \bimod{\yon}{\cat{D}}.
\]
...
\begin{proposition}
For any category $\cat{C}$, the category $\smset[\cat{C}]$ is the free coproduct completion of $\left(\smset^{\cat{C}}\right)\op$.
\end{proposition}
\begin{proof}
**
\end{proof}
...
\smset[\cat{C}]\to\poly
\]
sending $m\to m\tri\car{c}$ to $m\in\poly\cong\smset[\yon]$ is given by composition with the principal $\cat{C}$-comodule $\cat{C}\bimodfrom[\widecheck{\car{c}}]\yon$.
\end{proposition}
\begin{proof}
**
\end{proof}
\begin{proposition}
The functor $\eta_{\cat{C}} \colon \cat{C} \to \smset[\cat{C}]$ corresponding to the unit bicomodule $\car{c} \bimodfrom[\car{c}] \car{c}$ sends each object $i \in \car{c}(\1)$ to the bicomodule $\yon \bimodfrom[\yon ^{\car{c}[i]}]\car{c}$.
\end{proposition}

\begin{proposition}
The functor $\left(\smset^{\cat{C}}\right)\op \to \smset[\cat{C}]$ has a left adjoint, 
\[
\adjr{\left(\smset^{\cat{C}}\right)\op}{\yon ^ {-}}{\Gamma}{\smset[\cat{C}]}.
\]
\end{proposition}
\begin{proof}
**
...
\]
such that the left adjoint is a fully faithful inclusion of profunctors into bicomodules.
\end{theorem}
...
\begin{theorem}
For any $(\cat{C}, \cat{D})$-bicomodule $m$, the functor $\smset[\cat{C}] \to \smset[\cat{D}]$ defined by composition with $m$ preserves constants, coproducts, and all small limits; in particular, it's a map of rig categories.
\end{theorem}
\begin{proof}
**
\end{proof}
...
The composite of a linear left $\cat{C}$-comodule and a representable right $\cat{C}$-comodule is the set of natural transformations between the corresponding copresheaves.
\end{proposition}



\begin{proposition}
For any categories $\cat{C}$ and $\cat{D}$ and $(\cat{D},\cat{C})$-bicomodule $\car{d}\bimodfrom[m]\car{c}$, we have
\[
m\tri_\car{c}\car{c}\cong m
\]
where $\car{c}\bimodfrom[\car{c}]\yon$ is the regular left $\car{c}$-comodule.
\end{proposition}

%\begin{example}
%Using \cref{prop.composite_for_nats} we can obtain the $\3\0$ clue suggestions from \cref{ex.clue_suggestions} as a composite of bicomodules.
%(finish)
%\end{example}

\begin{theorem}\label{thm.elements}
For any category $\car{c}$, there is an adjunction
\[
\adj{\smcat^\sharp/\car{c}}{}{\int}{\bimod{\car{c}}{\0}}
\]
...
\Opensolutionfile{solutions}[solution-file10]

%------------ Chapter ------------%
\chapter{The framed bicategory}

A type of categorical structure called framed bicategory---also known as a proarrow equipment---gives a nice way to organize what's going on with comonoids, cofunctors, bicomodules, and maps between them. A framed bicategory is the type of double category, so it has objects, vertical morphisms, horizontal morphisms, and 2-cells. Thus we will discuss a framed bicategory $\pp$ for which the objects are categories, the vertical morphisms are cofunctors, and the horizontal morphisms are parametric right adjoints.

%-------- Section --------%
\section{Adjoint bicomodules}

\begin{proposition}
A functor $F\colon\cat{C}\to\cat{D}$ gives rise to a right adjoint bicomodule (= left adjoint pra) $\car{c}\bimodfrom[\Delta_F]\car{d}$.
\end{proposition}
\begin{proof}
The carriers of $\car{c}\bimodfrom[\Delta_F]\car{d}$ and its right adjoint $\car{d}\bimodfrom[\Pi_F]\car{c}$ are the polynomials
\[
  \sum_{i\in\car{c}(\1)}\yon^{Fi}
  \qqand
  \sum_{j\in\car{d}(\1)}\yon^{\sum_{j\to j'}(Fi=j')}
\]
...
\begin{proposition}
A cofunctor $\varphi\colon\cat{C}\cof\cat{D}$ gives rise to a left adjoint bicomodule (= right adjoint pra) $\car{c}\bimodfrom[\wh{\varphi}]\car{d}$; we denote its right adjoint bicomodule (= left adjoint pra) by $\wc{\varphi}$,
\begin{equation}\label{eqn.cof_adj_bimod}
\begin{tikzcd}
	\car{c}\ar[r, biml-bimr, shift left=7pt, "\wc{\varphi}"]\ar[r, phantom, "\Rightarrow"]&
	\car{d}\ar[l, biml-bimr, shift left=7pt, "\wh{\varphi}"]
\end{tikzcd}
\end{equation}
\end{proposition}
Note that the adjunction notation in \eqref{eqn.cof_adj_bimod} is unambiguous: if you read $\car{c}\bimodfrom\car{d}$ in the bicomodule direction---i.e.\ as a map from $\car{c}$ to $\car{d}$---then the notation indicates that $\wh{\varphi}$ is the left adjoint. If instead you read $\car{c}\bimodfrom\car{d}$ in the pra direction---i.e.\ as a map from $\car{d}$ to $\car{c}$---then the notation indicates that $\wh{\varphi}$ is the right adjoint.
\begin{proof}
**
\end{proof}
...
\begin{proposition}
A pra $\smset^{\cat{C}}\to\smset^{\cat{D}}$ is a left adjoint if it is of the form $\Sigma_G\circ\Delta_F$ for functors $\cat{C}\From{F}\cat{X}\To{G}\cat{D}$, where $G$ is a discrete opfibration.
\end{proposition}
\begin{proof}
**
\end{proof}
...
\begin{proposition}
Every bicomodule $\car{c}\bimodfrom[S]\0$, is isomorphic to the composite
\[
\car{c}\bimodfrom[S\yon]\yon\bimodfrom[\1]\0
\]
for a bicomodule $\car{c}\bimodfrom[S\yon]\yon$ with carrier $S\yon\in\poly$.

Moreover, $S\yon$ is adjoint to a bicomodule $\yon\bimodfrom[\yon^S]\car{c}$ with carrier $\yon^S\in\poly$.
\end{proposition}
\begin{proof}
...
\[
\begin{tikzcd}
	\car{c}\ar[r, "\car{c}"]\ar[d, "f"']&
	\car{c}\ar[d, "g"]\\
	\car{d}\ar[r, "\car{d}"']&
	\car{d}\ar[ul, phantom, "\Downarrow"]
\end{tikzcd}
\]
\begin{definition}[Natural co-transformations]
...
For any bicomodules $\car{c}_1\bimodfrom[m_1]\car{d}_1$ and $\car{c_2}\bimodfrom[m_2]\car{d_2}$, we can construct a new bicomodule in two ways:
\[
	\car{c}_1+\car{c}_2\bimodfrom[m_1+m_2]\car{d}_1+\car{d}_2
	\qqand
	\car{c}_1\otimes\car{c}_2\bimodfrom[m_1\otimes m_2]\car{d}_1\otimes \car{d}_2
\]
One can think of this quite easily in terms of prafunctors: given prafunctors $[\cat{D}_1,\smset]\to[\cat{C}_1,\smset]$ and $[\cat{D}_2,\smset]\to[\cat{C}_2,\smset]$, one obtains prafunctors between the coproduct categories and the product categories
\[
	[(\cat{D_1}+\cat{D_2}),\smset]\to[(\cat{C_1}+\cat{C_2}),\smset]
\]
In this section we will explain how this looks in terms of bicomodules, e.g.\ the respective bicomodule structures on $m_1+m_2$ and $m_1\otimes m_2$, and show that each of these constructions satisfies all the conditions to be a monoidal structure on the framed bicategory $\pp$.
...
\begin{enumerate}
  ...
  \item Both functors and cofunctors give left adjoint bicomodules: for functors $F\colon\cat{D}\to\cat{C}$ we use the pullback $\Delta_F$ and for cofunctors $G\colon\cat{C}\cof\cat{D}$ we use the companion as in \cref{**}. Can we characterize left adjoint bicomodules in general?
...
%\end{figure}
%
%A distilled version of the story so far---up to the end of the last paragraph---can be formalized mathematically, and we do so in this part. That is, we'll discuss the sense in which \emph{bicomodules} $m$ (playing the role of ``us'' in the above story) sit between two toposes (``realms''). From one realm (playing the role of ``reality''), the bicomodule $m$ ``picks up'' patterns. The other realm (playing the role of ``mind'') operates and moves through these patterns. Though the mind realm need not be any smaller than the reality realm, the two sides of this story are not symmetric in terms of how they interact with $m$.
%
%We saw in \cref{part.comon} that the comonoids $\com{C}$ in the monoidal category $(\poly,\yon,\circ)$ can be identified with categories $\cat{C}$.  The cofree comonoid $\cofree{p}$ of $p$-trees for arbitrary polynomial $p$ were seen to be particularly interesting from a dynamics point of view, lending themselves to much of the language of ``patterns'' used in the story above. 
%
%The story of this part really gets started when we realize that copresheaves $\Phi\colon\cat{C}_p\to\smset$ on these cofree categories can be identified with dependent dynamical systems in the sense of \cref{def.gen_moore}. But more generally, copresheaves on any category have a number of pleasant characterizations in terms of comonoids in $\poly$. 
%
%Moreover, the bicomodules $\cofree{q}\tickar[m]\cofree{p}$ between comonoids $\cofree{q}$ and $\cofree{p}$ correspond to parametric right adjoints $[\cofree{p},\smset]\to[\cofree{q},\smset]$ between the associated copresheaf categories, allowing us to migrate a dynamical system $\Phi$ of type $p$ to one of type $q$. One can describe this migration, this parametric right adjoint, in terms of receptors---or what database theorists call ``frozen instances''---picking up patterns in $\Phi$ and then sorting them into copresheaves on the target category $\cofree{q}$.
%
%Our goal in this part is to say the above carefully and with plenty of examples.
%
...
%
%\begin{proposition}
%Let $\cat{C}$ be a category and $\eta_\cat{C}\colon\cat{C}\to\Ob(\cat{C})$ the unit cofunctor. An attribute structure on $\cat{C}$ can be identified with a $(\Ob(\cat{C}),\0)$-bicomodule $\Ob(\cat{C})\bimodfrom[V]0$. An instance on $(\cat{C},V,\alpha)$ can be identified with a square of the form
%\[
