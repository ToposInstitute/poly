...
Continuing from \cref{ex.nothing_trees}, we draw the transition lens $\delta\colon\ema{s}\to\ema{s}\tri\ema{s}$ of $\ema{s}\coloneqq\3\yon^\3\iso\{\bul[red],\bul[dgreen],\bul[blue]\}\yon^{\{\bul[red],\bul[dgreen],\bul[blue]\}}$ (where directions are labeled with their targets) in tree pictures as well, recalling that the trees of $\ema{s}\tri\ema{s}$ are obtained by taking a corolla of $\ema{s}$ and gluing more corollas of $\ema{s}$ to each of its leaves:
\[
\begin{tikzpicture}[trees, 
  level 1/.style={sibling distance=5mm},
...
\subsection{Running dynamical systems}
...
%
		...
		\node[blue] at ($(S.east)!.33!(S2.west)$) {$\delta^2$};
...
So we want $\text{Run}_0(\phi)=\epsilon$ and $\text{Run}_1(\phi)=\phi$; we can achieve this by setting $\delta^{-1}\coloneqq\epsilon$ and $\delta^0\coloneqq\id_\ema{s}$.
...
The polynomial $\yon^\2+\yon$ can be given a comonoid structure as follows.
We'll go a little quickly here, because we'll soon discover a much more familiar way to think about comonoids.
...
\end{tikzpicture}
\]
Then on directions, $\delta$ should tell us how to run two directions together into one.
...
\subsubsection{The duplicator assigns codomains and composition}
...
\begin{tikzpicture}[polybox, tos]
    ...
    \node[poly, cod, right=1.8 of yX.south, yshift=-1ex, "$\ema{c}$" right] (p) {};
    \node[poly, cod, above=of p, "$\ema{c}$" right] (p') {};
...
\begin{tikzpicture}[polybox, mapstos]
    ...
    \node[poly, cod, right=1.8 of yX.south, yshift=-1ex, "$\ema{c}$" right] (p) {$f$\at$i$};
    \node[poly, cod, above=of p, "$\ema{c}$" right] (p') {$g$\at$\cod f$};
...
So $\delta$ takes an object $i\in\Ob\cat{C}$ and a morphism $f\colon i\to\_$ and assigns another object $\cod f\in\Ob\cat{C}$ to be the codomain of $f$.
It then takes another morphism $g\colon\cod f\to\_$ and assigns a morphism $f\then g\colon i\to\_$ to be the composite of $f$ and $g$.
...
In the picture below, the left equality depicts the left erasure law, while the right equality depicts the right erasure law:
\[
\begin{tikzpicture}
	\node (1) {
        \begin{tikzpicture}[polybox, mapstos]
	...
	\node[right=.5 of 2] (3) {
        \begin{tikzpicture}[polybox, mapstos]
        	...
        	\node[poly, cod, right=2 of r.south, yshift=-2.5ex, "$\ema{c}$" below] (p) {$f$\at$i$};
...
\end{tikzpicture}
\]
We find that on directions, the erasure laws state that for all $i\in\Ob\cat{C}$ and $f\colon i\to\_$,
\[
...
\begin{tikzpicture}
    \node (p1) {
        \begin{tikzpicture}[polybox, tos, font=\tiny]
            \node[poly, dom, "$\ema{s}$" left] (m') {$i\to s_3$\at$i$};
            \node[poly, right= of m'.south, yshift=-1ex, "$\ema{s}$" below] (mm') {$i\to s_2$\at$i$};
            \node[poly, above=of mm', "$\ema{s}$" above] (C') {$s_2\to s_3$\at$s_2$};
            \node[poly, cod, right= of mm'.south, yshift=-1ex, "$\ema{s}$" right] (D') {$i\to s_1$\at$i$};
            \node[poly, cod, above=of D', "$\ema{s}$" right] (mmm') {$s_1\to s_2$\at$s_1$};
            \node[poly, cod, above=of mmm', "$\ema{s}$" right] (CC') {$s_2\to s_3$\at$s_2$};
            %
            \draw[double] (m'_pos) to[first] (mm'_pos);
...
%
	\node (p2) [right=of p1] {
	    \begin{tikzpicture}[polybox, tos, font=\tiny]
            \node[poly, dom, "$\ema{s}$" left] (m) {$i\to s_3$\at$i$};
            \node[poly, right= of m.south, yshift=-1ex, "$\ema{s}$" below] (D) {$i\to s_1$\at$i$};
            \node[poly, above=of D, "$\ema{s}$" above] (mm) {$s_1\to s_3$\at$s_1$};
            \node[poly, cod, right= of D.south, yshift=-1ex, "$\ema{s}$" right] (DD) {$i\to s_1$\at$i$};
            \node[poly, cod, above=of DD, "$\ema{s}$" right] (mmm) {$s_1\to s_2$\at$s_1$};
            \node[poly, cod, above=of mmm, "$\ema{s}$" right] (C) {$s_2\to s_3$\at$s_2$};
            %
            \draw[double] (m_pos) to[first] (D_pos);
...
There's a lot going on here, so let's break it down---we'll focus on the run functions first.
On the left hand side, we run together $i\to s_1$ and $s_1\to s_2$ to obtain $i\to s_2$, before running that together with $s_2\to s_3$ to obtain $i\to s_3$, as we see in the upper left box.
Meanwhile, on the right, we run together $s_1\to s_2$ and $s_2\to s_3$ to obtain $s_1\to s_3$, before running $i\to s_1$ together with our newly obtained $s_1\to s_3$ to again obtain $i\to s_3$ in the upper left box.
We could write this all out equationally, but all this is saying is that ``running together'' the directions of a state system is an associative operation.
When running together three directions, it doesn't matter whether we run the first two together or the last two together to start.
Not only is this guaranteed by the way in which we constructed $\delta$, it also makes intuitive sense.
...
\begin{solution}
Following the arrows on either side of \eqref{eqn.trans_lens_coassoc_polybox} all the way to the domain's direction box, we obtain an expression for each box's contents that we can then set equal to each other.
The easiest way to actually write down these expressions is probably to start at the end with $i\to s_3$ and follow the arrows backward, unpacking each term until only the contents of the blue boxes remain (namely $i, i\to s_1, s_1\to s_2,$ and $s_2\to s_3$).
Here's what we get when we follow this process for the left hand side of \eqref{eqn.trans_lens_coassoc_polybox} (remember where to look for the three inputs to the run function):
\begin{align*}
    i\to s_3&=\text{run}(i,i\to s_2,s_2\to s_3)\\
    &=\text{run}(i,\text{run}(i,i\to s_1,s_1\to s_2),s_2\to s_3)\\
    &=\text{run}(i,\text{run}(i,i\to s_1,s_1\to s_2),s_2\to s_3);
\end{align*}
and here's what we get for the right (also remember where to look for the two inputs to the target function):
\begin{align*}
    i\to s_3&=\text{run}(i,i\to s_1,s_1\to s_3)\\
    &=\text{run}(i,i\to s_1,\text{run}(s_1,s_1\to s_2,s_2\to s_3))\\
    &=\text{run}(i,i\to s_1,\text{run}(\text{$\cod$}(i,i\to s_1),s_1\to s_2,s_2\to s_3)).
\end{align*}
Setting these equal yields our desired associativity equation:
\[
    \text{run}(i,\text{run}(i,i\to s_1,s_1\to s_2),s_2\to s_3)=\text{run}(i,i\to s_1,\text{run}(\text{$\cod$}(i,i\to s_1),s_1\to s_2,s_2\to s_3)).
\]
\end{solution}
\end{exercise}
...
On the other hand, the fact that both the upper position boxes in the codomain contain $s_2$ implies that
\begin{align*}
    \text{$\cod$}(i,\text{run}(i,i\to s_1,s_1\to s_2))&=\text{$\cod$}(i,i\to s_2)\\
    &=s_2\\
    &=\text{$\cod$}(s_1,s_1\to s_2)\\
    &=\text{$\cod$}(\text{$\cod$}(i,i\to s_1),s_1\to s_2),
\end{align*}
which is exactly what we wanted in \eqref{eqn.state_run_tgt}.
In English, this says that when we run together $i\to s_1$ and $s_1\to s_2$, the new direction's target is the same as the direction of $s_1\to s_2$, the latter of the two directions that we ran together.
Again, this coincides with our intuition: if we follow two directions in order, we should end up at wherever the latter direction leads us.
...
\end{tikzpicture}
\]
Ah, it's saying that it wants $\cod(m')=\cod(m\then m')$; well yeah, that's how codomains should work. And it wants $m\then(m'\then m'')=(m\then m')\then m''$, classic associativity.

We've seen that all of the data and equations of categories correspond to the data and equations of polynomial comonoids.
...
\begin{example}[A simple comonoid that's not $S\yon^S$]\label{ex.walking_arrow_cat}
...
% ** There's something tricky going on with the codomains here...check this
...
Whereas a functor $\cat{C}\to\cat{D}$ can be thought of as a \emph{diagram}---a picture or model of the category $\cat{C}$ that can be found within another category $\cat{D}$---cofunctors are much more like the dynamical systems of \cref{ch.poly.dyn_sys}.\footnote{In fact, we will see in \cref{ch.comon.cofree} that cofunctors generalize our dynamical systems.}
A cofunctor $F\colon\cat{C}\coto\cat{D}$ is a way of interacting with the states (objects) and transitions (morphisms) within $\cat{C}$ by way of $\cat{D}$.
Imagine the cofunctor as a box, with $\cat{C}$ on the inside and $\cat{D}$ on the outside.
Some $c\in\cat{C}$ might be the current state inside the box, but all anyone outside the box can see is the object $Fc\in\cat{D}$ that the box chooses to display in lieu of $c$.
Still, any morphism $g$ coming out of $Fc$ can be selected from the outside; the box guarantees that whatever $c$ is on the inside, there is a morphism $F^\sharp_c g$ coming out of that $c$.
Then the new state on the inside is $\cod F^\sharp_c g$, the new object on the outside is $F\cod F^\sharp_c g=Fg$, and the process repeats in a manner compatible with identities and composition.
...
%-------- Section --------%
\section{Introduction: Cofree comonoids and dynamical systems}
