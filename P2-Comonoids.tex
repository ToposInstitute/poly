...
Back in \cref{subsec.poly.func_nat.repr_sum.sum_prod_set}, we saw how to write the instructions for choosing an element of a dependent sum or product of sets.
...
We won't give the entire definition of what it means for two monoidal structures to be duoidal here---there are a few commutative diagrams to verify for technical reasons---but the key condition is that there is a natural map
...
\end{enumerate}
\begin{solution}
The comonoid structure on $\yon$ has $\epsilon\colon\yon\to\yon$ and $\delta\colon\yon\to\yon\tri\yon=\yon$ the unique maps.
\begin{enumerate}
	\item We first show that for any polynomial $m$, there is a unique $\yon$-comodule structure on $m$. By \eqref{eqn.left_comod_laws}, a $\yon$-comodule structure is a map $\lambda\colon m\to\yon\tri m=m$ such that $\lambda\then(!\tri m)=\id_m$, but $!\tri m=\id_m$, so $\lambda=\id_m$ is forced. A morphism $(m,\id)\to(n,\id_n)$ of $\yon$-comodules is also easily seen to be just a polynomial map $m\to n$.
	\item Similar.
\end{enumerate}
\end{solution}
\end{exercise}
...
\begin{enumerate}
    \item We need a map $\sum_{v\in V}\yon^{A_v}\to V\sum_{v\in V}\yon^{A_v}$, and we use the obvious ``diagonal'' map, which on directions sends $v\mapsto (v,v)$ and on directions is the identity.
    \item We need a map $\sum_{v\in V}\yon^{A_v}\to\sum_{v\in V}\prod_{a\in A_v}\sum_{v\in V}\yon$, which is the same as an element of
    \[
