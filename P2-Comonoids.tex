...
Before we move onto examples, one more note about the theory: notice that both sides of our coassociative law are given by $\delta^2\colon\ema{c}\to\ema{c}\tripow3$, as defined in \cref{prop.n_duplication}.
On directions, $\delta^2$ tells us how to compose three morphisms $i\To{f}\_\To{g}\_\To{h}\_$ in $\cat{C}$ all at once to obtain $i\To{f\:\then\:g\:\then\:h}\_$, and (co)associativity ensures this is well-defined.

In general, $\delta^{n-1}\colon\ema{c}\to\ema{c}\tripow{n}$ on directions tells us how to compose $n$ morphisms in $\cat{C}$ for each $n\in\nn$.
After all, we have already seen that $\delta^1=\delta$ performs binary composition, that $\delta^0=\id_\ema{c}$ performs ``unary'' composition (the ``unary composite'' of a single morphism $f$ is just $f$ itself), and that $\delta^{-1}=\epsilon$ performs ``nullary'' composition (the ``nullary composite'' at any object is just its identity).
The directions of $\ema{c}\tripow{n}$ at positions in the image of $\delta^{n-1}$ are exactly the sequences of composable morphisms of length $n$, and $\delta^{n-1}$ sends each sequence to the single direction that is its composite.
...
Let $F\colon\cat{C}\cof\cat{D}$ be a cofunctor, $c,c'\in\cat{C}$ be objects, and $g\colon Fc\to Fc'$ be an isomorphism in $\cat{D}$. Then $F^\sharp_{c}g$ is also an isomorphism in $\cat{C}$.
\end{proposition}
\begin{proof}
...
\begin{align*}
	...
	F^\sharp_{c'}\left(g^{-1}\right)\then F^\sharp_{\cod F^\sharp_{c'}g}g \tag*{\eqref{eqn.pres_comp}}
\end{align*}
...
That is, a cofunctor $F\colon\cat{C}\coto\cat{D}$ is a way of interacting with the states (objects) and transitions (morphisms) within $\cat{C}$ by way of $\cat{D}$.
