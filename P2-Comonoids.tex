...
The composition of polynomials will be extremely important in the story that follows. However, we only sometimes think of it as functor composition; more often we think of it as a certain operation on arenas or corolla forests.

\subsection{Composite arenas}
...
While this description completely characterizes $p \tri q$ as an arena, it is a bit of a mouthful.
But it turns out to have a very natural interpretation when we translate from arenas to corolla forests.

\subsection{Composite corolla forests}

In fact, the forest of $p\tri q$ is given by gluing corollas from the forest of $q$ onto the leaves of corollas from the forest of $p$ in every possible way.
...
% ** Move next example & exercise til after morphs into composites/polyboxes discussed
% \begin{example}
% For any $p$ and $q$ there is an interesting cartesian map $o_{p,q}\colon p\otimes q\to p\tri q$ that orders the operation. It looks like this:
% \[
...
% Both $\otimes$ and $\tri$ have the same monoidal unit, the identity functor $\yon$, whose identity is the unique map $\yon\to\yon$.
% In fact the maps $o_{p,q}$ constitute a lax monoidal functor $(\poly,\yon,\otimes)\to(\poly,\yon,\tri)$.
...
\end{tikzpicture}
\]
Now here are pictures of $p\tri\yon$, obtained by gluing the one-leaf corolla of $\yon$ to all the leaves of each of the corollas of $p$ in turn, and $\yon\tri p$, obtained by gluing each of the corollas of $p$ to the single leaf of $\yon$:
\[
\begin{tikzpicture}[rounded corners]
  ...
  \end{tikzpicture}
	...
	\node (p2) [draw, right=2 of p1, "$\yon\:\tri\:${\color{blue!50!black}$p$}" above] {
	\begin{tikzpicture}[trees, sibling distance=2.5mm]
    \node (1) {$\bullet$}
      child {
...
Recall from \cref{ex.practice_with_poly_morphisms} that we can think of $f=(f_\1,f^\sharp)$ as a way to delegate decisions from $p$ to $p'$. Every decision (i.e.\ position) $i\in p(\1)$ is assigned a decision $f_\1(i)\in p'(\1)$. Then every option $d\in p'[f_\1(i)]$ there is passed back to an option $f^\sharp(d)\in p[i]$. Let's start with an example and then give the general method.
...
%
	...
	\begin{tikzpicture}[trees, sibling distance=2.5mm]
    \node[red!75!black, "\tiny 2" below] (1) {$\bullet$} 
      child {coordinate (11)};
...
\[
	...
	\begin{scope}[
	  ...
	  level 2/.style={sibling distance=2.5mm}]
    \node[blue!50!black] (1) {$\bullet$} 
      child {node[red!75!black] (11') {$\bullet$} 
      	child {coordinate (11)}
				child {coordinate (12)}
			}
      child {node[red!75!black] (12') {$\bullet$} 
      	child {coordinate (13)}
				child {coordinate (14)}
			};
%
    \node[blue!50!black, right=5 of 1] (2) {$\bullet$} 
      child {node[red!75!black] (21') {$\bullet$} 
      	child {coordinate (21)}
				child {coordinate (22)}
			}
      child {node[red!75!black] (22') {$\bullet$} 
      	child {coordinate (23)}
			};
%
    \node[blue!50!black, below=1.3 of 1] (3) {$\bullet$} 
      child {node[red!75!black] (31') {$\bullet$} 
      	child {coordinate (31)}
			}
      child {node[red!75!black] (32') {$\bullet$} 
      	child {coordinate (32)}
				child {coordinate (33)}
			};
%
    \node[blue!50!black] at (2|-3) (4) {$\bullet$} 
      child {node[red!75!black] (41') {$\bullet$} 
      	child {coordinate (41)}
			}
      child {node[red!75!black] (42') {$\bullet$} 
      	child {coordinate (42)}
			};
%
    \node[blue!50!black, below=1.3 of 3] (5) {$\bullet$} 
      child {node[red!75!black] (51') {$\bullet$} 
      	child {coordinate (51)}
				child {coordinate (52)}
			};
%
    \node[blue!50!black] at (4|-5) (6) {$\bullet$} 
      child {node[red!75!black] (61') {$\bullet$} 
      	child {coordinate (61)}
			};
		\end{scope}
%%
...
%
	\node (q) [draw, red!75!black, right=2 of p, "$q=$" left] {
	\begin{tikzpicture}[trees, sibling distance=4mm]
...
\[
    ...
    \node["\tiny $A$" below] (1) {$\bullet$} 
      child {coordinate (11')}
      child {coordinate (12')};
    \node[right=2 of 1, red!75!black,"\tiny $C$" below] (1') {$\bullet$}
    	child {node[red] {$\bullet$}
				child {coordinate (1'1)}
				child {coordinate (1'2)}
			}
...
%
    \node (2) [right=3 of 1',"\tiny $B$" below] {$\bullet$} 
      child {coordinate (21')};
    \node[right=2 of 2, red!75!black,"\tiny $D$" below] (2') {$\bullet$}
			child{node[red] {$\bullet$}
				child {coordinate (2'1)}
