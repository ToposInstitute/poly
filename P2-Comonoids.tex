...
\begin{align*}
    ...
    &\iso\poly\left(\chom{q}{p}, r\right) \tag*{\eqref{eqn.chom_def}}.
\end{align*}
\end{proof}

\begin{corollary}[Left preservation of limits]
Fix $r\in\poly$.
...
\begin{enumerate}
    ...
    \item \label{exc.comp_left_pres_lim.prod} Deduce \eqref{eqn.comp_left_pres_prod} using \eqref{eqn.comp_left_pres_lim}.
\end{enumerate}
\begin{solution}
\begin{enumerate}
...
The claim for the left variable follows as in the proof of \cref{prop.left_dist_prod}: limits of functors $\smset\to\smset$ are computed pointwise and $\poly$ is a full subcategory of $\smset^\smset$ closed under limits. The claim for the right-hand variable comes down to the fact that polynomials are sums of representables; representable functors commute with all limits and sums commute with connected limits in $\smset$. See \cite[Proposition 1.6]{kock2012polynomial} for details.
\end{proof}
