...
\part{A different category of categories}\label{chapter.comon}

\Opensolutionfile{solutions}[solution-file5]

%------------ Chapter ------------%
\chapter{The composition product}\label{sec.composition_prod}

We have seen that the category of polynomial functors---sums of representables $\smset\to\smset$ and the natural transformations between them---has quite a bit of well-interoperating mathematical structure. Further, it is an expressive way to talk about dynamical systems that can change their interfaces and wiring patterns based on their internal states.

In this part we will discuss a monoidal structure on $\poly$ that is quite easy from the mathematical point of view---it is simply composition---but which is again remarkable both in terms of its semantics and the phenomena that emerge mathematically. 
...
\end{proposition}
\begin{proof}
We can rewrite \eqref{eqn.sum_p1_again} for $p$ and $q$ as
\[
p\cong\sum_{i\in p(\1)}\prod_{d\in p[i]}\yon
\qqand
q\cong\sum_{j\in q(\1)}\prod_{e\in q[j]}\yon.
\]
For any set $X$ we have $(p\circ q)(X)=p(q(X))=p(\sum_j\prod_e X)=\sum_i\prod_d\sum_j\prod_eX$, so \eqref{eqn.composite_formula_circ} is indeed the formula for their composite.
...
\begin{align}\label{eqn.composition_formula_sums_first}
  p\circ q\cong
...
But $\tri$ highlights the asymmetry of composition, in contrast with the other monoidal structures on $\poly$ we've encountered, and we'll soon see that it is quite evocative in terms of trees, and again it leaves $\circ$ for other uses.
...
\begin{enumerate}
	...
	\item How many functions $j_2\colon p[2]\to q(\1)$ are there?
...
\begin{enumerate}
    \item By standard polynomial multiplication, we have that $\yon^\2 \tri q \iso q^2 \iso \yon^\6 + 2\yon^\3 + \1$.
    \item We have that $\yon^\1 \tri q \iso q \iso \yon^\3 + \1$.
    \item Combining the previous parts, we have that $(\yon^\2 + \yon^\1) \tri q \iso q^2 + q^1 \iso \yon^\6 + \3\yon^\3 + \2$.
    \item Since $p[1] \iso \2$ and $q(\1) \iso \2$, there are $2^2 = 4$ functions $p[1] \to q(\1)$.
    \item When $j_1 \colon p[1] \to q(\1)$ is one of the two possible bijections, we have that
    \[
        \sum_{d \in p[1]} q[j_1(d)] \iso q[1] + q[2] \iso \3 + \0 \iso \3.
    \]
    When $j_1 \colon p[1] \to q(\1)$ sends everything to $1 \in q(\1)$, we have that
    \[
        \sum_{d \in p[1]} q[j_1(d)] \iso q[1] + q[1] \iso \3 + \3 \iso \6.
    \]
    Finally, when $j_1 \colon p[1] \to q(\1)$ sends everything to $2 \in q(\1)$, we have that
    \[
        \sum_{d \in p[1]} q[j_1(d)] \iso q[2] + q[2] \iso \0 + \0 \iso \0.
    \]
    \item Since $p[2] \iso \1$ and $q(\1) \iso \2$, there are $2^1 = 2$ functions $p[2] \to q(\1)$.
    \item When $j_2 \colon p[2] \to q(\1)$ maps to $1 \in q(\1)$, we have that $\sum_{d \in p[2]} q[j_2(d)] \iso q[1] \iso \3$, and when $j_2 \colon p[2] \to q(\1)$ maps to $2 \in q(\1)$, we have that $\sum_{d \in p[2]} q[j_2(d)] \iso q[2] \iso \0$.
    \item From the previous parts, we have that
    \[
        \sum_{i\in p(\1)}\sum_{j_i\colon p[i]\to q(\1)}\yon^{\sum_{d\in p[i]}q[j_i(d)]} \iso (\2\yon^\3 + \yon^\6 + \yon^\0) + (\yon^\3 + \yon^\0) \iso \yon^\6 + \3\yon^\3 + \2,
    \]
...
In terms of corollas, the forest of the composition product $p\tri q$ is given by gluing corollas from the forest of $q$ onto the leaves of corollas from the forest of $p$ in every possible way.
...
\begin{exercise}
Use $p,q$ as in \eqref{eqn.pq_misc39} and $r\coloneqq \yon^\2+\1$ in the following.
\begin{enumerate}
	\item Draw $q\tri p$.
	\item Draw $p\tri p$.
...
\end{enumerate}
\begin{solution}
We have $p,q$ as in \eqref{eqn.pq_misc39} and $r\coloneqq \yon^\2+\1$.
\begin{enumerate}
    \item **
    \item 
    \item 
...
\begin{enumerate}
	\item The isomorphism exists: we have that $(A\yon)\otimes(B\yon) \iso AB\yon \iso (A\yon)\tri (B\yon)$.
	\item The isomorphism exists: we have that $\yon^A\otimes\yon^B \iso \yon^{AB} \iso \yon^A\tri\yon^B$.
	\item The isomorphism does not necessarily exist: while $A\otimes B \iso AB$, we have that $A\tri B \iso A$.
	\item The isomorphism exists: we have that $A\yon \otimes p \iso \sum_{i \in p(\1)} A\yon \otimes \yon^{p[i]} \iso \sum_{i \in p(\1)} A\yon^{p[i]} \iso Ap \iso A\yon\tri p$.
	\item The isomorphism does not necessarily exist: if, say, $p = B$, then $\yon^A \otimes B \iso B$, while $\yon^A \tri B \iso B^A$.
	\item The isomorphism does not necessarily exist: if, say, $p = \yon^B$, then $\yon^B \otimes A\yon \iso A\yon^B$, while $\yon^B \otimes A\yon \iso (A\yon)^B \iso A^B\yon^B$.
	\item The isomorphism exists: we have that $p \otimes \yon^A \iso \sum_{i \in p(\1)} \yon^{p[i]} \otimes \yon^A \iso \sum_{i \in p(\1)} \yon^{Ap[i]} \iso p \tri \yon^A$.
\end{enumerate}
\end{solution}
\end{exercise}
...
\begin{enumerate}
	...
	\item Show that if $X$ is a set (considered as a constant polynomial) and $p$ is any polynomial, then $X\tri p\cong X$.
	\item \label{exc.composing_with_constants.appl} Show that if $X$ is a set and $p$ is a polynomial, then $p\tri X\cong p(X)$, where $p(X)$ is the set given by applying $p$ as a functor to $X$.
\qedhere
\end{enumerate}
\begin{solution}
\begin{enumerate}
...
\begin{enumerate}[resume]
    ...
    We can also verify this using \cref{eqn.composite_formula}:
    \[
...
\begin{enumerate}
	...
	\item Why is $p\tri\yon$ isomorphic to $p$?
...
\begin{enumerate}
    ...
    \item Composing any functor with the identity functor yields the original functor, so $p \tri \yon \iso p$.
    \item **
\end{enumerate}
\end{solution}
\end{exercise}
...
%-------- Section --------%
\section{The monoidal structure $(\poly, \tri, \yon)$}\label{subsec.monoidal_struc_tri}

The technical claim is that $\tri$ is a monoidal product, which means that it's well-behaved; in particular, it's functorial, associative, and unital. In fact, all of this comes from general theory: for any category $\cat{C}$, the category whose objects are functors $\cat{C}\to\cat{C}$ and whose morphisms are natural transformations is a monoidal category, with functor composition as the monoidal product. For us, $\cat{C}\coloneqq\smset$; yet we are only using polynomial functors, not all functors, so there is a tiny bit more to check, but it's accomplished by \cref{prop.poly_closed_comp} and the fact that the identity functor $\smset\to\smset$ is a polynomial (it's $\yon$).
...
For any $f\colon p\to p'$ and $g\colon q\to q'$, we should obtain a morphism $(f\tri g)\colon(p\tri q)\to(p'\tri q')$. This is actually quite an impressive operation! It threads back and forth in a fascinating way. 

Recall from \cref{ex.practice_with_poly_morphisms} that we can think of $f=(f_1,f^\sharp)$ as a way to delegate decisions from $p$ to $p'$. Every decision (i.e.\ position) $i\in p(\1)$ is assigned a decision $f_1(i)\in p'(\1)$. Then every option $d\in p'[f_1(i)]$ there is passed back to an option $f^\sharp(d)\in p[i]$. Let's start with an example and then give the general method.
...
Okay, now suppose someone gives us delegations (i.e.\ morphisms) $f\colon p\to p'$ and $g\colon q\to q'$. Let's just pick something relatively at random:
\[
\begin{tikzpicture}
...
%---- Subsection ----%
\subsection{Pronouncing polynomial composites}
...
%-------- Section --------%
\section{Working with composites}\label{subsec.working_with_composites}
...
%-------- Section --------%
\section{Mathematical aspects of $\tri$}
...
Prove \cref{prop.left_dist} in terms of the formula for $\tri$ given in \cref{eqn.composite_formula}.
\begin{solution}
Given $p,q,r \in \poly$, recall that the positions of the sum $p+q$ form the set $p(\1)+q(\1)$, while the directions at each position $i \in p(\1) + q(\1)$ correspond to the directions in $p[i]$ if $i \in p(\1)$ and the directions in $q[i]$ if $i \in q(\1)$.
So by \cref{eqn.composite_formula}, we have that
\[
\begin{split}
    (p + q) \tri r &\iso \sum_{i \in p(\1)+q(\1)} \prod_{d \in (p+q)[i]} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon \\
    &\iso \sum_{i \in p(\1)} \prod_{d \in p[i]} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon + \sum_{i \in q(\1)} \prod_{d \in q[i]} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon \\
    &\iso p \tri r + q \tri r.
\end{split}
\]
We can also recall that the positions of the product $pq$ form the set $p(\1) \times q(\1)$, while the directions at each position $(i, i') \in p(\1) \times q(\1)$ correspond to the directions in $p[i] + q[i']$.
So by \cref{eqn.composite_formula}, we have that
\[
\begin{split}
    (pq) \tri r &\iso \sum_{(i,i') \in p(\1) \times q(\1)} \ \prod_{d \in p[i] + q[i']} \ \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon \\
    &\iso \sum_{(i,i') \in p(\1) \times q(\1)} \left(\prod_{d \in p[i]} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon\right) \left(\prod_{d \in q[i']} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon\right) \\
    &\iso \left(\sum_{i \in p(\1)} \prod_{d \in p[i]} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon\right)\left(\sum_{i' \in q(\1)} \prod_{d \in q[i']} \sum_{j \in r(\1)} \prod_{e \in r[j]} \yon\right) \\
    &\iso (p \tri r)(q \tri r).
\end{split}
\]
\end{solution}
...
\begin{exercise}
Show that the distributivities of \cref{prop.left_dist} do not hold on the other side:
\begin{enumerate}
...
\begin{solution}
\begin{enumerate}
    \item Given a polynomial functor $p \colon \smset \to \smset$, we wish to show that $p$ preserves connected limits of sets; that is, for a connected category $J$ and a functor $X \colon J \to \smset$, we have
    \[
        p\left(\lim_{j \in J} X_j\right) \iso \lim_{j \in J} p(X_j).
    \]
    ...
    Then by \cref{exc.composing_with_constants} \cref{exc.composing_with_constants.appl}, the left hand side of the isomorphism we seek is isomorphic to $p\tri\left(\lim_{j \in J} X_j\right)$, while the right hand side is isomorphic to $\lim_{j \in J} \left(p \tri X_j\right)$.
    These are isomorphic by \cref{thm.connected_limits}.
    \item Given $p,q,r \in \poly$, we wish to show that $p \tri (qr) \iso (p \tri q) \times_{(p \tri \1)} (p \tri r)$.
    ...
    \item As in the previous part, we can write $pq$ as the pullback $p \times_\1 q$.
    By \cref{thm.connected_limits}, this connected limit is preserved by composition with $r$, so $(pq) \tri r \iso (p \tri r) \times_{\1 \tri r} (q \tri r) \iso (p \tri r) \times_\1 (q \tri r) \iso (p \tri r)(q \tri r)$. (Here we use the fact that $\1 \tri r \iso \1$ by \cref{exc.composing_with_constants} \cref{exc.composing_with_constants.appl}.)
    \item Given a set $A$ and polynomials $p,q$, the left-distributivity of $\tri$ over products implies that $(Ap)\tri q \iso (A\tri q)(p \tri q)$, while \cref{exc.composing_with_constants} \cref{exc.composing_with_constants.appl} implies that $A \tri q \iso A$.
