...
\end{tikzpicture}
\]
Then the do-nothing enclosure $\epsilon\colon\car{s}\to\yon$ can be drawn like so:
\[
...
Armed with this new lens, we can model three steps through a system $\phi\colon\car{s}\to p$ with interface $p\in\poly$ as the composite lens
\[
    \car{s}\To{\delta^{(3)}}\car{s}\tripow3\To{\phi\tripow3}p\tripow3.
\]
...
Given a dynamical system $\phi\colon\car{s}\to p$ with interface $p\in\poly$, we can construct a new dynamical system that we call $\text{Run}_n(\phi)$, with the same state system $\car{s}$ but a new interface $p\tripow{n}$, by defining $\text{Run}_n(\phi)\coloneqq\delta^{(n)}\then\phi\tripow{n}$.
Visually, we define $\text{Run}_n(\phi)$ so that the following diagram commutes:
\begin{equation}\label{eqn.speedup}
\begin{tikzcd}
	\car{s}\ar[r, "\delta^{(n)}"]\ar[rr, bend right, "{\text{Run}_n(\phi)}"']&
	\car{s}\tripow{n}\ar[r, "\phi\tripow{n}"]&
	p\tripow n
\end{tikzcd}
\end{equation}
...
%
  ...
  \end{tikzpicture}	
	...
	\node at (given|-obtain_lab) {Given $\phi\colon \car{s}\to p$,};
\end{tikzpicture}
\]
Notice that we have only defined $\delta^{(n)}$, and thus $\text{Run}_n(\phi)$, for integers $n\geq2$.
But $n=0$ runs through $n$ is doing nothing, modeled by the do-nothing enclosure $\epsilon\colon\car{s}\to\yon$, while $n=1$ run through $\phi$ is modeled by $\phi\colon\car{s}\to\car{s}$ itself.
So we want $\text{Run}_0(\phi)=\epsilon$ and $\text{Run}_1(\phi)=\phi$; we can achieve this by setting $\delta^{(0)}\coloneqq\epsilon$ and $\delta^{(1)}\coloneqq\id_\car{s}$.
...
\begin{exercise}
Verify that when $\delta^{(0)}=\epsilon$ and $\delta^{(1)}=\id_\car{s}$, if $\text{Run}_n(\phi)$ is defined as $\delta^{(n)}\then\phi\tripow{n}$ for all $n\in\nn$, then $\text{Run}_0(\phi)=\epsilon$ and $\text{Run}_1(\phi)=\id_\car{s}$.
\begin{solution}
We have $\text{Run}_n(\phi)=\delta^{(n)}\then\phi\tripow{n}$ for all $n\in\nn$, as well as $\delta^{(0)}=\epsilon$ and $\delta^{(1)}=\id_\car{s}$.
Then $\text{Run}_0(\phi)=\epsilon\then\phi\tripow0=\epsilon\then\id_\yon=\epsilon$ and $\text{Run}_1(\phi)=\id_\car{s}\then\phi\tripow1=\phi\tripow1=\phi$.
\end{solution}
\end{exercise}
...
We can then model $n$ steps through a dynamical system $\phi\colon\car{s}\to p$ with interface $p\in\poly$ by composing this canonical lens with $\phi\tripow{n}$ to obtain a ``sped-up'' dynamical system $\text{Run}_n(\phi)$.
This new system has the same state system $\car{s}$, but its interface is now $p\tripow{n}$.

The canonicity of $\car{s}\to\car{s}\tripow{n}$ is due to the following standard result about comonoids, which can be proved inductively. %**remove this last part if you end up including the proof
\end{example}
% The fact that a state system $\car{s}$ has a comonoid structure is well-known: it is called the \emph{state comonad} in computer science literature. **Check that this is actually true...
...
% \begin{align*}
%     \delta\then\left(\delta^{(0)}\tri\delta^{-1}\right)&=\delta\then\left(\car{c}\tri\epsilon\right) \tag{b}\\
%     &=\id_\car{c} \tag*{\eqref{eqn.erasure_law}}\\
%     &=\delta^{(0)} \tag{b}.
% \end{align*}
...
% We will check unitality only for $s$; it is easier for $B$.
% \[
% \begin{tikzpicture}[trees, sibling distance=5mm,	bend right=60]
% 	\node (1A) [red] {$\bullet$} 
%   	child  {coordinate (1A1) \idchild}
...
% \end{tikzpicture}
% \]
% In both pictures, one can see that the composite map is the identity. We would do associativity here, but because the category $\cat{WA}$ is so simple, associativity is guaranteed; this makes the pictures too trivial.
\end{solution}
\end{exercise}
...
% \begin{exercise}
% Let $\cat{C}$ be a category, $\car{c}$ its carrier, and $i\in\Ob\cat{C}$ an object. This exercise is for people who know the definition of the coslice category $i/\cat{C}$ of objects under $i$. Is it true that there is an isomorphism
% \[\Ob(c/\cat{C})i\cong^?\car{c}[i]\]
...
\begin{center}
    $\cat{WA}\coloneqq\:$\boxCD{examplecolor}{
    $s\Too{a}t$}
\end{center}
...
Often we will denote a category by its carrier when its comonoid structure can be deduced from context, and $\cat{C}$ will be a category throughout with carrier $\car{c}$.
...
\begin{enumerate}
	...
	\item What does a cofunctor from $\yon$ to a poset represent? %**replace poset with preorder??
\qedhere
\end{enumerate}
\begin{solution}
\begin{enumerate}
...
\begin{example}[Cofunctors from $\cat{C}$ to state categories are $\cat{C}$-coalgebras]
...
% ** coalgebras...discrete opfibrations...copresheaves
...
Recall that we have a category $\cat{C}$ with carrier $\car{c}$.
...
\begin{exercise} %** label the arrows
How many cofunctors
\[
...
Consider a dynamical system $\phi\colon\car{s}\to p$ with state system $\car{s}$ and interface $p$.
In \cref{subsec.comon.sharp.state.run}, we posed the question of whether there was a single morphism that could capture all the information encoded by the family of lenses $\text{Run}_n(\phi)\colon\car{s}\to p\tripow{n}$ for all $n\in\nn$, defined as the composite
\[
    \car{s}\To{\delta^{(n)}}\car{s}\tripow{n}\To{\phi\tripow{n}}p\tripow{n}
\]
...
\begin{definition}[Trees on polynomials] \label{def.poly_tree}
...
\begin{definition}[Pretrees on polynomials] \label{def.pretree}
...
\begin{itemize}
    ...
    \tablefootnote{Both the morphisms-as-vertices and the morphisms-as-rooted-paths perspectives are helpful for interpreting the category $\cofree{p}$, so we include both; as we have seen, it is not terribly difficult to translate between them.}
    \begin{itemize}
...
\begin{proof}
To show that $\cofree{-}$ is the right adjoint of $U$, it is enough to show that for every lens $\phi\colon\car{c}\to p$, there exists a unique cofunctor $F\colon\cat{C}\cof\cofree{p}$ for which
\begin{equation} \label{eqn.cofree_universal_lens_counit}
\begin{tikzcd}
    \car{c} \ar[d,dashed,"F"'] \ar[dr,"\phi"] \\
...
First, we construct $F$ from $\phi$ as follows.
If we let $\epsilon$ and $\delta$ be the eraser and duplicator of $\cat{C}$, the diagram
\begin{equation} \label{eqn.phi_seed}
\begin{tikzcd}[column sep=large]
    \car{c} \ar[d, "\epsilon"'] \ar[r, "\delta"] \ar[dr, "\phi"'] &
    \car{c}\tri\car{c} \ar[d, "\epsilon\:\tri\:\phi"] \\
    \yon \ar[d] & p \ar[d] \\
    \1 & p\tri\1 \ar[l]
\end{tikzcd}
\end{equation}
...
\begin{align*}
    \delta\then\left(\epsilon\tri\phi\right)&=
    \delta\then\left(\epsilon\tri\car{c}\right)\then\phi \\ &=
    \id_\car{c}\then\phi \tag{Left erasure law} \\ &=
    \phi.
\end{align*}
Then by induction, the larger diagram
\begin{equation} \label{eqn.phi_expand}
\begin{tikzcd}[column sep=large]
    \car{c} \ar[d,"\epsilon"'] \ar[r,"\delta"] \ar[dr,"\phi"'] & \car{c}\tri\car{c} \ar[d,"\epsilon\:\tri\:\phi"] \ar[r,"\delta\:\tri\:\car{c}"] & \car{c}\tripow3 \ar[d, "\epsilon\:\tri\:\phi\tripow2"] \ar[r,"\delta\:\tri\:\car{c}\tripow2"] & \car{c}\tripow4 \ar[d,"\epsilon\:\tri\:\phi\tripow{3}"] \ar[r] & \cdots \\
...
\[
\begin{tikzcd}[column sep=large]
    \car{c} \ar[d, "\phi"'] \ar[r, equals] & \car{c} \ar[d, "\phi"] \\
...
This lens is the composite of the lens $\car{c}\to\car{c}\tripow{(n+1)}$ in the top row, which by \cref{prop.n_duplication} is the canonical lens $\delta^{(n+1)}$ associated with the comonoid $\cat{C}$, composed with the lens $\epsilon\tri\phi\tripow{n}\colon\car{c}\tripow{(n+1)}\to p\tripow{n}$.
...
\[
    \poly(\car{c},p)\iso\smcat^\sharp(\cat{C},\cofree{p}).
\]
So every lens $\phi\colon\car{c}\to p$ has a corresponding cofunctor $F\colon\cat{C}\cof\cofree{p}$ that we call its \emph{mate}.

We can view $\phi$ as a dynamical system with an interface $p$ and a generalized state system $\car{c}$, carrying an arbitrary category $\cat{C}$ of states and transitions.
Then the lens $\text{Run}_n(\phi)\colon\car{c}\to p\tripow{n}$ for $n\in\nn$, defined as the composite
\[
    \car{c}\To{\delta^{(n)}}\car{c}\tripow{n}\To{\phi\tripow{n}}p\tripow{n},
\]
...
As the cofunctor $F\colon\cat{C}\cof\cofree{p}$ has an underlying lens between carriers $f\coloneqq UF\colon\car{c}\to\car{t}_p$, we can obtain another lens $\car{c}\to\tripow{n}$ as the composite
\[
...
% ** add examples...deterministic state automata
...
% ** turn below into example?
\begin{proposition}[Coproducts in $\smcat^\sharp$]
...
Let $I$ be a set and $(\cat{C}_i)_{i\in I}$ be categories with carriers $(\car{c}_i)_{i\in I}$.
...
\begin{enumerate}
	\item Give a category $\cat{D}$ carried by the polynomial $\sum_{i\in\car{c}(\1)}\car{c}[i]\yon^{\car{c}[i]}$ and a cofunctor $\cat{D}\cof\cat{C}$.
...
\end{proposition}
\begin{proof}
Given $(\car{c},\epsilon_{\car c},\delta_{\car c}),(\car{d},\epsilon_{\car d},\delta_{\car d})\in\smcat^\sharp$, 

be categories corresponding to comonoids $(\car{c},\epsilon_{\car c},\delta_{\car c})$ and $(\car{d},\epsilon_{\car d},\delta_{\car d})$.
