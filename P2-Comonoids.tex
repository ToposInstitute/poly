...
% ** do the reading off equations thing
...
\begin{example}[Dynamical systems as coalgebras]
...
The easiest way to actually write down these expressions is probably to start at the end with $s_0\to s_3$ and follow the arrows backwards, unpacking each term until only the contents of the blue boxes remain (namely $s_0, s_0\to s_1, s_1\to s_2,$ and $s_2\to s_3$).
...
\[
    \boxed{\text{run}(s_0,\text{run}(s_0,s_0\to s_1,s_1\to s_2),s_2\to s_3)=\text{run}(s_0,s_0\to s_1,\text{run}(\text{tgt}(s_0,s_0\to s_1),s_1\to s_2,s_2\to s_3))}.
\]
\end{solution}
\end{exercise}
...
\begin{enumerate}
    ...
    From there, we can get to $\ema{s}\tripow4$ directly by composing with $\delta\tri\delta$ to obtain $\boxed{\delta\then(\delta\tri\delta)}$.
    ...
    This gives us $4$ more ways to write a lens $\ema{s}\to\ema{s}\tripow4$: either $\boxed{\delta\then(\ema{s}\tri(\delta\then(\delta\tri\ema{s})))}$ or $\boxed{\delta\then(\ema{s}\tri(\delta\then(\ema{s}\tri\delta)))}$ if we chose to preserve the first $\ema{s}$, and either $\boxed{\delta\then((\delta\then(\delta\tri\ema{s}))\tri\ema{s})}$ or $\boxed{\delta\then((\delta\then(\ema{s}\tri\delta))\tri\ema{s})}$ if we chose to preserve the second.
...
\begin{example} %**polish this; distinguish between steps...?
...
% ** This is good, but update all language to current. Maybe divide it up, too, and intersperse? Do-nothing enclosure, then transition lens, then...
\begin{example}[Picturing state systems as trees] \label{ex.picturing_SyS}
...
\begin{enumerate}
	...
	\item We know $\ema{s}$ is supposed to be the carrier of a comonoid $(\ema{s},\epsilon,\delta)$. Which two maps $\ema{s}\to \ema{s}\tripow3$ are supposed to be equal by associativity? %**update this so it doesn't use comonoid language yet
...
(We'll talk more about monoids in $\smset$ in \cref{**}.)
...
\slogan{
    every state system is a polynomial comonoid, whose eraser is the do-nothing enclosure and whose duplicator is the transition lens.
}
...
\begin{example}[Not all polynomial comonoids are state systems]
...
What is the carrier of each of the following categories?
\begin{enumerate}
...
\begin{solution}
\begin{enumerate}
	\item The category \boxCD{white}{$A\Too{f}B$} has $2$ morphisms out of $A$ and $1$ morphism out of $B$, so its carrier is $\yon^\2 + \yon$.
	\item The category \boxCD{white}{$B\From{f}A\To{g}C$} has $3$ morphisms out of $A$ and $1$ morphism out of each of $B$ and $C$, so its carrier is $\yon^\3 + 2\yon$.
	\item The empty category has no objects, so its carrier is the empty sum $\0$.
	\item The monoid $(\nn,0,+)$ has $1$ object, and its morphisms form the set $\nn$, so its carrier is $\yon^\nn$.
	\item The monoid $(M, e, *)$ has $1$ object, and its morphisms form the set $M$, so its carrier is $\yon^M$.
	\item The poset $(\nn,\leq)$ has $\nn$ as its set of objects, and there is exactly one morphism from every $n \in \nn$ to each element of $\{n' \in \nn \ | \ n \leq n'\} \iso \nn$ (and no other morphisms from $n$), so the carrier of the poset is $\nn\yon^\nn$.
	\item The poset $(\nn,\geq)$ has $\nn$ as its set of objects, and there is exactly one morphism from every $n \in \nn$ to each element of $\{0, 1, \ldots, n\} \iso \ord{n}+\1$ (and no other morphisms from $n$), so the carrier of the poset is $\sum_{n \in \nn} \yon^{\ord{n}+\1} \iso \yon^\1 + \yon^\2 + \yon^\3 + \cdots$.
\end{enumerate}
\end{solution}
\end{exercise}
...
\begin{tabular}{cc|cc}
  ...
  & 
  \rule{0pt}{2ex} $\delta_i^\sharp\colon(\ema{c}\tri\ema{c})[\delta_\1(i)]\to\ema{c}[i]$ & composition* & $\displaystyle\comp\colon\sum_{f\in\cat{C}[i]}\cat{C}[\cod(f)]\to\cat{C}[i]$ \\
  \hline
  \multicolumn2{c|}{\multirow{2}{*}{right erasure law}} & \multicolumn2{c}{$\ast$} \\
  \multicolumn2{c|}{} & \multicolumn2{c}{right identity law} \\
  \hline
  \multicolumn2{c|}{\multirow{2}{*}{left erasure law}} & \multicolumn2{c}{codomains of identities} \\
  \multicolumn2{c|}{} & \multicolumn2{c}{left identity law} \\
  \hline
  \multicolumn2{c|}{\multirow{2}{*}{coassociative law}} & \multicolumn2{c}{codomains of composites} \\
...
The identities of $\cat{C}$ are all captured by the counit $\epsilon\colon\ema{c}\to\yon$ and the codomain and composition information of $\cat{C}$ are all captured by the comultiplication map $\delta\colon\ema{c}\to\ema{c}\tri\ema{c}$.
...
% update to highlight categorical structure
\begin{example}[A simple comonoid that's not $S\yon^S$]\label{ex.walking_arrow}
The polynomial $\yon^\2+\yon$ can be given a comonoid structure. Let's first associate names to its positions and directions. 
...
\end{enumerate}
\begin{solution}
We are given a comonoid $(\ema{c},\epsilon,\delta)$ corresponding to the category \boxCD{white}{$B\From{f}A\To{g}C$}.
\begin{enumerate}
...
\begin{exercise}
Finish \cref{ex.monoids} by showing that if $(M,e,*)$ satisfies the unitality and associativity requirements of a monoid in $(\smset,\1,\times)$ then $(\yon^M,\epsilon,\delta)$ satisfies the erasure and coassociativity requirements of a comonoid in $(\poly,\yon,\tri)$.
\begin{solution} %** Do we want to write it out??
This is a direct consequence of the fully faithful Yoneda embedding $\smset\op\to\poly$ sending $M\mapsto\yon^M$, and the fact that $\yon^A\tri\yon^B\iso\yon^{A\times B}$ for all $A,B\in\smset$.
\end{solution}
\end{exercise}

% ** displaced content begins
...
% ** displaced content ends

\begin{example}[Monoid action]\label{ex.monoid_action}
Suppose that $(M,e,*)$ is a monoid, $S$ is a set, and $\alpha \colon M \times S \to S$ is an action. There is an associated category $\cat{A}$ with carrier $S \yon ^ M$. In other words, it has objects set $S$ and for every $s \in S$ there is an outgoing morphism for each $m \in M$, namely $s \To {m} \alpha (m , s)$. 
\end{example}
...
\end{enumerate}
\begin{solution}
Here $(M,e,*)$ is a monoid, $S$ is a set, $\alpha \colon M \times S \to S$ is an action, and $\cat{A}$ is the associated category with carrier $S\yon^M$; in particular, for each $s \in S$ and $m \in M$, there is a morphism $s \To{m} \alpha(m,s)$.
\begin{enumerate}
...
% ** define comonoid morphisms

% ** define category of comonoids

Note that we were very careful in how we stated \cref{thm.ahman_uustalu}: while we claimed an isomorphism-preserving one-to-one correspondence between objects in $\comon$ and objects in $\smcat^\sharp$, we never claimed that these two categories are isomorphic or even equivalent.
The strange truth of the matter is that they are not: polynomial comonoid morphisms correspond not to functors, but to a different map of categories called a \emph{cofunctor}.
Specifying \cref{def.**} to the case of $(\poly,\yon,\tri)$, we derive the following definition of a cofunctor.
...
Then \cref{thm.ahman_uustalu} can be restated as follows.
\begin{corollary}[Ahman-Uustalu]
There is an isomorphism of categories
\[
    \comon\iso\smcat^\sharp.
\]
\end{corollary}
...
% ** displaced stuff begins

\begin{example}[Admissible sections]\label{ex.admissible_section}
Consider the monoid $\cat{N}\coloneqq (\nn,0,+)$, considered as a category with one object. For any category $\cat{C}$, a cofunctor $\varphi\colon\cat{C}\cof\cat{N}$ is called an \emph{admissible section} \cite{aguiar1997internal}. We'll have more to say about these in \cref{thm.catsharp_to_mon}, but our goal here is simply to unpack the definition.
...
\begin{definition}[Morphism of comonoids]\label{def.morphism_comonoids}
...
Let's see the two laws of comonoid morphisms using polyboxes. First the counit law:
...
% **Not this, but the fact that this is here in the text means...
...
We saw in \cref{ex.cofunctors_comon_homs} that cofunctors $\cat{C}\cof\cat{D}$ are the same thing as morphisms of comonoids $\com{C}\to\com{D}$, so we elide the difference. The question we're interested in now is: how do we think about cofunctors? What is a map of polynomial comonoids like?
...
There is a fully faithful functor $\Cat{Mon}\op\to\smcat^\sharp$, whose image is precisely those categories whose carrier is representable.
\end{proposition}
\begin{proof}
...
\begin{enumerate}
	\item Write out the comultiplication law from \eqref{eqn.unit_law_draw} in terms of polyboxes.
