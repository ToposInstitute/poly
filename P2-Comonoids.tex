...
We can express a dynamical system $\phi\colon S\yon^S\to p$ in polyboxes as
\begin{equation*} \label{eqn.polybox_lens}
\begin{tikzpicture}
  \node (f) {
    \begin{tikzpicture}[polybox, mapstos]
  	  ...
  	  \node[poly, cod, right=of S, "$p$" right] (p) {$a$\at$i$};
	  
  	  \draw (S_pos) -- node[below] {$\phi_\1$} (p_pos);
  	  \draw (p_dir) -- node[above] {$\phi^\sharp$} (S_dir);
    \end{tikzpicture}
  };
\end{tikzpicture}
\end{equation*}
