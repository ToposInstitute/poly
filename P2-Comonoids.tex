...
%**Adjust, say we want it to be connected
...
Suppose that $(M,e,*)$ is a monoid, $S$ is a set, and $\alpha\colon M\times S\to S$ is a \emph{(left) monoid action}.
That is, for all $s\in S$ we have $\alpha(e,s)=s$ and $\alpha(n*m,s)=\alpha(n,\alpha(m,s))$ for $m,n\in M$; equivalently, the diagrams
\[
\begin{tikzcd}
	1\times S\ar[rd,equals]\ar[r,"e\:\times\:S"] & M\times S\ar[d,"\alpha"]\\
	& S
\end{tikzcd}
\qqand
\begin{tikzcd}[row sep=large]
    M\times M\times S\ar[d,"*\:\times\:S"]\ar[r,"M\:\times\:\alpha"] & M\times S\ar[d,"\alpha"]\\
    M\times S\ar[r,"\alpha"] & S
\end{tikzcd}
\]
commute.

Then there is an associated category $\cat{A}$ with objects in $S$ and morphisms $s\To{m}\alpha(m,s)$ for each $s\in S$ and $m\in M$.
...
\begin{enumerate}
	...
	\item $M$ always acts on itself by multiplication. Is the associated comonoid structure on $M\yon^M$ the same or different from the one coming from \cref{ex.state_comonad_1}? %**fix this reference
\qedhere
\end{enumerate}
\begin{solution}
Here $(M,e,*)$ is a monoid, $S$ is a set, $\alpha\colon M\times S\to S$ is a monoid action, and $\cat{A}$ is the associated category, whose corresponding comonoid is $(S\yon^M,\epsilon,\delta)$.
We also know that for each $s\in S$ and $m\in M$, there is a morphism $s\To{m} \alpha(m,s)$ in $\cat{A}$.
\begin{enumerate}
    ...
    Since we want the codomain of the morphism $m$ out of $s$ to be $\alpha(m,s)$, we can take $m=e$ to guarantee that its codomain will be $\alpha(e,s)=s$.
    ...
    We already know that we want the morphism $m\in M$ out of each $s\in S$ to have the codomain $\alpha(m,s)$.
    If we then have another morphism $n\in M$ out of $\alpha(m,s)$, its codomain will be $\alpha(n,\alpha(m,s))=\alpha(n*m,s)$, the same as the codomain of the morphism $n*m$ out of $s$.
    So it makes sense for the composite $s \To{m} \alpha(m,s) \To{n} \alpha(n,\alpha(m,s))$ to be the morphism $s \To{n*m} \alpha(n*m,s)$.
    Thus, we can define $\delta$ in polyboxes as
    \[
    \begin{tikzpicture}[polybox, mapstos, font=\tiny]
        \node[poly, dom] (p) {$n*m$\at$s$};
        \node[poly, cod, right=1.8 of p.south, yshift=-1ex] (q) {$m$\at$s$};
        \node[poly, cod, above=of q, xshift=2] (r) {$n$\at$\alpha(m,s)$};
        \draw[double] (p_pos) to[first] (q_pos);
        \draw (q_dir) to[climb] node[right] {$\cod$} (r_pos);
...
%**Why is the above example not a preorder?
...
\begin{exercise}
Let $S$ be a countable set. Is there any comonoid structure on $S\yon^S$ other than that of the state category?
\begin{solution}
In the case of $S=\0$, the only comonoid structure on $S\yon^S\iso\0$ is given by the empty category; and in the case of $S=\1$, the only comonoid structure on $S\yon^S\iso\yon$ is given by the category with $1$ object and no nonidentity morphisms.
So in those cases, the comonoid structure on $S\yon^S$ is unique.

Now assume $|S|\geq2$.
The state category is always connected, but we can always find a comonoid structure on $S\yon^S$ given by a category that is not connected as follows.
Consider a category whose object set is $S$ that has no morphisms between distinct objects.
...
But there is always a monoid structure on a countable set $S$: take for example the cyclic group $C_{|S|}$ of order $|S|$ (either $\zz/|S|\zz$ if $S$ is finite, or $\zz$ if $S$ is countable).
The resulting category will have carrier $S\yon^{C_{|S|}}\iso S\yon^S$.
\end{solution}
\end{exercise}
...
\begin{enumerate}[itemsep=0pt]
  ...
  \item a function $F^\sharp\colon C_0\times_{D_0}D_1\to C_1$ \emph{backward on morphisms},
\end{enumerate}
