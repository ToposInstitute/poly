...
\subsection{Polyboxes in action}
...
% \begin{exercise}
% Let $A,B\in\smset$ be sets, and let $p\in\poly$ be a polynomial. Is it true that the lenses $A\yon^B\to p$ can be identified with the lenses $A\to p\tri B$, i.e.\ that there is a bijection:
...
\begin{tikzpicture}[polybox, mapstos]
    ...
    \node[poly, cod, "$q$" right, right=of h] (r) {$d$};
