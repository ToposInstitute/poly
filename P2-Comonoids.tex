...
\begin{enumerate}
    ...
    \end{equation} %**Something about polyboxes??
    \qedhere
\end{enumerate}
\begin{solution}
...
\begin{enumerate}
    ...
    \item We prove \eqref{eqn.flip_reps_lins} in two pieces: that
    ...
    \end{equation}
    For \eqref{eqn.flipping1}, we have that $A\yon \tri p \iso Ap$, an $A$-fold coproduct of $p$.
    Similarly, $\yon^A \tri q \iso q^A$, an $A$-fold product of $q$.
    So the isomorphism follows from their respective universal properties.
...
By \cref{thm.poly_limits}, $\poly$ is complete, so by \cite[Theorem~4.3]{nlab:connected-limit}, it suffices to show that $(q\tri-)\colon\poly\to\poly$ preserves wide pullbacks on the right.
...
Thus $(q\tri-)$ sends a wide pullback over $r\in\poly$ to a wide pullback over the canonical lens $q\tri r\to q\tri\1$ in $\poly/q\tri\1$, corresponding to a limit in $\poly$ of a diagram where every arrow to $q\tri\1$ factors through $q\tri r$.
