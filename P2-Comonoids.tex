...
\]
We'll see that this property falls out naturally when we we generalize the transition lens to more than two steps in \cref{subsec.comon.sharp.state.coassoc}.
...
To make this interpretation more clear, we could be a little more verbose with our polybox picture if we wanted to:
\[
\begin{tikzpicture}
    \node (1) {
        \begin{tikzpicture}[polybox, mapstos]
	...
	\node[right=1.4 of 1] (2) {
	    \begin{tikzpicture}[polybox, mapstos]
            \node[poly, dom, "$\ema{s}$" left] (yX) {\at$s$};
            \node[poly, right=1.8 of yX.south, yshift=-1ex, "$\ema{s}$" below] (p) {$\id_s$\at$s$};
            \node[poly, above=of p, "$\ema{s}$" above] (p') {tgt$(s,\id_s)\to t$\at tgt$(s,\id_s)$};
            \node[poly, cod, identity, right=of p, "$\ema{s}$" below] (q) {};
            \node[poly, cod, above=of q, xshift=1ex, "$\ema{s}$" above] (q') {tgt$(s,\id_s)\to t$\at tgt$(s,\id_s)$};
            ...
            \draw (p_dir) to[climb] node[right] {tgt} (p'_pos);
            \draw (p'_dir) to[last] node[above=.4] {run} (yX_dir);
        \end{tikzpicture}
	};
	\node at ($(1.east)!.5!(2.west)$) {=};
\end{tikzpicture}
...
If following the direction $\id_s$ from the state $s$ is really akin to doing nothing, then its target state should be the same state $s$ that it emerged from.
...
\begin{tikzpicture}
	\node (1) {
        \begin{tikzpicture}[polybox, mapstos]
        	\node[poly, dom, "$\ema{s}$" left] (r) {\at$s$};
        	\node[poly, right=2 of r.south, yshift=-2.5ex, "$\ema{s}$" below] (p) {$\id_s$\at$s$};
        	\node[poly, cod, above=.8 of p, xshift=2.5ex, "$\ema{s}$" right] (p') {tgt$(s,\id_s)\to t$\at tgt$(s,\id_s)$};
	...
	\node[right=1.8 of 1] (2) {
        \begin{tikzpicture}[polybox, mapstos]
          	\node[poly, dom] (c) {$s\to t$\at$s$};
          	\node[poly, cod, right=of c] (c') {$s\to t$\at$s$};
...
\begin{tikzcd}
    ...
    & \ema{s}\tri\ema{s}\ar[ul, "\epsilon\:\tri\: c"]
\end{tikzcd}
\]
...
A \emph{comonoid} in a monoidal category $(\cat{C},\yon,\tri)$
