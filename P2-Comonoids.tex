...
\begin{example}
In \cref{ex.cofree_dyn_sys} we had a dynamical system with $S\coloneqq\{\bul[dgreen],\bul[dyellow],\bul[red]\}$ and $p\coloneqq\yon^\2+\1$, and $\varphi\colon S\yon^S\to p$ from \cref{exc.halt_dsa}, depicted here again for your convenience:
...
\end{tikzcd}
\end{equation}
The polynomial map $\varphi$ induces a cofunctor $F\colon S\yon^S\cof\cofree{p}$ from the state category on $S$ to the category of $p$-trees. We can now see this as a copresheaf on the category $\cofree{p}$ itself.
...
\[
\begin{tikzcd}[column sep=small]
	\bul[dgreen]\ar[rr, bend left]\ar[loop left]&&
	\bul[dyellow]\ar[dl, bend left]\ar[ll, bend left]\\&
	\bul[red]
\end{tikzcd}
\]
The polynomial map $f$ is supposed to induce a cofunctor $F\colon S\yon^S\cof\cofree{p}$ from the state category on $S$ to the category of $p$-trees. Thus to each of the three states $S=\{\bul[dgreen],\bul[dyellow],\bul[red]\}$, we need to associate a tree; they are shown here:
\[
...
\begin{solution}
\begin{enumerate}
    \item By the adjunction from \cref{thm.cofree}, cofunctors $\cofree{p} \to \cofree{p\tripow{n}}$ are in bijection with polynomial maps $\car{t}_{p}\to p\tripow{n}$, where $\car{t}_{p}=U\cofree{p}$ is the carrier. 
    
    From \cref{prop.n_duplication} we have a map $\delta^{(n)}\colon\cofree{p}\to\cofree{p}\tripow{n}$, and we also have the counit of the cofree adjunction which we'll temporarily call $r\colon\cofree{p}\to p$. Then the desired map is $(\delta^{(n)}\then r\tripow{n})\colon\car{t}_{p}\to p\tripow{n}$.
...
\[V_p\coloneqq\tr_p(\1).\]
For arrows we use the counit map $\pi\colon\tr_p\to p$ from \cref{thm.cofree} to define
\[
...
\begin{example}[Coproducts in $\smcat^\sharp$] \label{ex.comon_coprod}
...
\begin{enumerate}
    ...
    \item The map $\yon\to\car{t}\yon$ is the unique map.
...
\[
\begin{tikzpicture}[polybox, mapstos]
  \node[poly, dom, "$m$" left] (l) {$x$\at$i\vphantom{i}$};
  \node[poly, cod, "$m$" right, right=of l] (r) {$x$\at$i$};
  \draw[double,-] (l_pos) -- (r_pos);
  \draw[double,-] (r_dir) -- (l_dir);
%
	\node[poly, dom, right=2.5 of r, "$m$" left] (p) {$x'$\at$i$};
	\node[poly, right=1.5cm of p.south, yshift=-1.25ex] (r') {$\id_{|i|}$\at$|i|$};
	\node[poly, cod, above=of r', "$m$" right] (q) {$x$\at$i.\id_{|i|}$};
...
\end{tikzpicture}
\]
The equation says that for any $i\in m(1)$ we have $i=i.\id_{|i|}\coloneqq\lambda(i,\id_{|i|})$ and that for any $x\in m[i]$ we have $x=x'\coloneqq\lambda^\sharp_i(\id_{|i|},x)$.
...
\begin{tikzpicture}
    \node (p1) {
        \begin{tikzpicture}[polybox, mapstos]
            \node[poly, dom, "$m$" left] (m') {$\lambda^\sharp_i((f\then f'),x)$\at$i$};
            \node[poly, right=2.3 of m'.south, yshift=-1ex, "$\car{c}$" below] (mm') {$f\then f'$\at$|i|$};
            \node[poly, above=of mm', "$m$" above] (C') {$x$\at$i.(f\then f')$};
            \node[poly, cod, right=2 of mm'.south, yshift=-1ex, "$\car{c}$" right] (D') {$f$\at$|i|$};
            \node[poly, cod, above=of D', "$\car{c}$" right] (mmm') {$f'$\at$\cod f$};
            \node[poly, cod, above=of mmm', "$m$" right] (CC') {$x$\at$i.(f\then f')$};
            %
...
%
	\node (p2) [below=1 of p1] {
	    \begin{tikzpicture}[polybox, tos]
            \node[poly, dom, "$m$" left] (m) {$\lambda^\sharp_i(f,\lambda^\sharp_{i.f}(f',x))$\at$i$};
            \node[poly, right=3 of m.south, yshift=-1ex, "$\car{c}$" below] (D) {$f$\at$|i|$};
            \node[poly, above=of D, "$m$" above] (mm) {$\lambda^\sharp_{i.f}(f',x)$\at$i.f$};
            \node[poly, cod, right=2 of D.south, yshift=-1ex, "$\car{c}$" right] (DD) {$f$\at$|i|$};
            \node[poly, cod, above=of DD, "$\car{c}$" right] (mmm) {$f'$\at$|i.f|$};
            \node[poly, cod, above=of mmm, "$m$" right] (C) {$x$\at$(i.f).f'$};
            %
...
\end{tikzpicture}
\]
These say that $\cod(f)=|i.f|$, that $i.(f\then f')=(i.f).f'$, and that $\lambda^\sharp_i((f\then f'),x)=\lambda^\sharp_i(f,\lambda^\sharp_{i.f}(f',x))$.
	\item
\[
\begin{tikzpicture}[polybox, mapstos]
  \node[poly, dom, "$m$" left] (l) {$f$\at$i\vphantom{i}$};
  \node[poly, cod, "$m$" right, right=of l] (r) {$f$\at$i$};
  \draw[double,-] (l_pos) -- (r_pos);
  \draw[double,-] (r_dir) -- (l_dir);
%
	\node[poly, dom, right=2.5 of r, "$m$" left] (p) {$\rho^\sharp_i(f,\id_{|i'.f|})$\at$i$};
	\node[poly, cod, right=2.5cm of p.south, yshift=-1.25ex, "$m$" right] (r') {$f$\at$i'$};
	\node[poly, above=of r'] (q) {$\id_{|i'.f|}$\at$|i'.f|$};
  	\draw (p_pos) to[first] (r'_pos);
  	\draw (r'_dir) to[climb] node[left] {$\rho$} (q_pos);
		\draw (q_dir) to[last] (p_dir);
		\draw (q_pos) to[climb'] node[right] {$\epsilon_\car{d}$} (q_dir);
	\node at ($(r.west)!.5!(p.east)$) {$=$};
\end{tikzpicture}
\]
The equation says that for any $i\in m(1)$ we have $i=i'\coloneqq\rho_1(i)$ and that for any $f\in m[i]$ we have $f=\rho^\sharp_i(f,\id_{|i'.f|})$.
...
\begin{tikzpicture}
    \node (p1) {
        \begin{tikzpicture}[polybox, mapstos]
            \node[poly, dom, "$m$" left] (m') {$\rho^\sharp_i(\rho_i^\sharp(x,g),g')$\at$i$};
            \node[poly, right=2.7 of m'.south, yshift=-1ex, "$m$" below] (mm') {$\rho^\sharp_i(x,g)$\at$i$};
            \node[poly, above=of mm', "$\car{d}$" above] (C') {$g'$\at$|i.\rho^\sharp_i(x,g)|$};
            \node[poly, cod, right=2 of mm'.south, yshift=-1ex, "$m$" right] (D') {$x$\at$i$};
            \node[poly, cod, above=of D', "$\car{d}$" right] (mmm') {$g$\at$|i.x|$};
            \node[poly, cod, above=of mmm', "$\car{d}$" right] (CC') {$g'$\at$|i.\rho^\sharp_i(x,g)|$};
            %
            \draw[double,-] (m'_pos) to[first] (mm'_pos);
...
%
	\node (p2) [below=1 of p1] {
	    \begin{tikzpicture}[polybox, tos]
            \node[poly, dom, "$m$" left] (m) {$\rho^\sharp_i(x,(g\then g'))$\at$i$};
            \node[poly, right=2.5 of m.south, yshift=-1ex, "$m$" below] (D) {$x$\at$i$};
            \node[poly, above=of D, "$\car{d}$" above] (mm) {$g\then g'$\at$|i.x|$};
            \node[poly, cod, right=2 of D.south, yshift=-1ex, "$m$" right] (DD) {$x$\at$i$};
            \node[poly, cod, above=of DD, "$\car{d}$" right] (mmm) {$g$\at$|i.x|$};
...
\end{tikzpicture}
\]
These equations say that $\cod(g)=|i.\rho^\sharp_i(x,g)$ and that $\rho_i^\sharp(\rho_i^\sharp(x,g),g')=\rho^\sharp_i(x,(g\then g'))$.
\end{enumerate}
\end{solution}
\end{exercise}
