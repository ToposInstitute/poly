...
%-------- Section --------%
\section{Categorical properties of the composition product}
...
\subsection{Interaction with products and coproducts}
...
Fix $r\in\poly$.
The functor $(-\tri r)\colon\poly\to\poly$ that sends each $p\in\poly$ to $p\tri r$ commutes with coproducts and products (up to natural isomorphism):
\[
...
Prove \cref{prop.left_dist} using the explicit formula for $\tri$ given in \eqref{eqn.composite_formula} by manipulating sums and products.
\begin{solution}
...
We want an intuitive understanding of this left distributivity.
...
Follow \cref{ex.picturing_dist} with coproducts $(+)$ in place of products $(\times)$: use pictures to give an intuitive understanding of the left distributivity $(p+q)\tri r\cong (p\tri r)+(q\tri r)$.
\begin{solution}
...
\subsection{Interaction with limits}
...
\subsection{Interaction with parallel products}
...
\subsection{Interaction with vertical and cartesian lenses}
