...
\begin{exercise}
Suppose $p$, $q$, and $r$ are polynomials and you're given arbitrary lenses $f\colon q\to p\tri q$ and $g\colon q\to q\tri r$. Does the following diagram necessarily commute?\tablefootnote{When the name of an object is used in place of a morphism, we refer to the identity morphism on that object.
...
%\]
%commute,%
%\tablefootnote{Note that such a morphism $\fl{\delta}$ must be unique since $\phi_c$ is an epimorphism.}
...
% In cases where it does not matter whether we send morphisms forward or backward, functors and cofunctors effectively coincide.
% The following exercise formalizes this idea.

% \begin{exercise} %** this isn't actually true: needs to be stronger than fully faithful functor, actually a coproduct 
% Let $F\colon\cat{C}\to\cat{D}$ be a fully faithful functor; that is, for every $c,c'\in\cat{C}$, the induced map of hom-sets $F\colon\cat{C}(c,c')\to\cat{D}(Fc,Fc')$ is a bijection.
% Show that there exists a unique cofunctor $\Tilde{F}\colon\cat{C}\cof\cat{D}$ that agrees with $F$ on objects such that for all morphisms $f\colon c\to\_$ in $\cat{C}$ and $g\colon Fc\to\_$ in $\cat{D}$,
% \[
%     \Tilde{F}^\sharp_c(Ff)=f \qqand F\left(\Tilde{F}^\sharp_cg\right)=g.
% \]
% Then show that $F$ can be uniquely recovered from $\Tilde{F}$; further characterize all cofunctors $\Tilde{F}$ that arise from fully faithful functors $F$ in this way.
% \begin{solution}
% Given a fully faithful functor $F\colon\cat{C}\to\cat{D}$, we seek a cofunctor $\Tilde{F}\colon\cat{C}\cof\cat{D}$ that agrees with $F$ on objects and satisfies $\Tilde{F}^\sharp_c(Ff)=f$ and $F\left(\Tilde{F}^\sharp_cg\right)=g$ for all $f\colon c\to\_$ and $g\colon Fc\to\_$.

% **
% % , the fact that $\Tilde{F}\colon\cat{C}\cof\cat{D}$ agrees with $F$ on objects and satisfies $\Tilde{F}^\sharp_c(Ff)=f$ for all $f\colon c\to\_$ in $\cat{C}$ uniquely characterizes $\Tilde{F}$: as $F$ is full, every morphism $g\colon Fc\to\_$ in $\cat{D}$ is the image $Ff$ of some morphism $f\colon c\to\_$, 
% \end{solution}
% \end{exercise}
...
% \begin{exercise}
% Is it true that for any two categories $\cat{C},\cat{D}$, there is a bijection between the set of isomorphisms $\cat{C}\To{\iso}\cat{D}$ in $\smcat$ and the set of isomorphisms $\cat{C}\overset{\iso}{\cof}\cat{D}$ between them in $\smcat^\sharp$?
...
\subsubsection{Cofunctors and state categories}

% ** add intro about how we've worked with state categories, so...
...
% Adjust the statement...more like each prod proj gives rise to unique...??
\begin{solution}
...
\begin{example} %**what name: [Canonical cofunctors from state categories]??
%** fill out this example; check using polyboxes
...
% ** do we want this exercise if we haven't intro'd coproducts yet? Maybe move way later?
\begin{exercise}
...
%** Do we actually want cofunctors from state categories? Or both? What's worth discussing? What's the universal property...always (Ob C)y^(Ob C)-/->C
\begin{example}[Cofunctors to state categories]
...
% ** use iso-pres
% So a cofunctor $F\colon\cat C\cof S\yon^S$ 
% is completely determined by its behavior on objects: to preserve identities, it can only send the morphisms in $S\yon$ back to the identity morphisms in $\cat C$.
...
\subsubsection{Cofunctors to other preorders}
...
Cofunctors $\cat{T}\cof\cat{C}$ are somewhat strange beasts: because they must preserve codomains, they can be identified with objects $c\in\cat{C}$ for which the codomain of every emanating morphism $c\to c'$ is $c'=c$ itself. %** any more?
\end{example}
...
%** Or: Show that there is a cofunctor $S\yon^M\cof\yon^M$.
\begin{solution}
...
\begin{exercise} %** label the arrows, and maybe the polys to
How many cofunctors
\[
...
\begin{exercise}
\begin{enumerate}
	\item What is the finite set $\{\com{Q}_1,\ldots,\com{Q}_n\}$ of (isomorphism classes of) categories whose carrier is $\yon^\2+\yon$? %** did we do this already?
...
We'll go through this construction and its implications in this section, culminating in a purely formal proof of the following.

%** restructure so this is later, so that dependencies are clear?
...
\]
where the carrier $\ema{t}_p\coloneqq U\cofree{p}$ of the category $\cofree{p}$ for $p\in\poly$ is given by the limit of the following diagram:
...
\end{tikzcd}
\end{equation}
That is, given a category $\cat{C}\in\smcat^\sharp$ with carrier $\ema{c}\coloneqq U\cat{C}$ and a polynomial $p\in\poly$, there is a natural isomorphism
\[
    \poly(\ema{c},p)\iso\smcat^\sharp(\cat{C},\cofree{p}).
\]
\end{theorem}
As $\cofree{-}$ is the right adjoint of a forgetful functor sending comonoids to their carriers, this is a forgetful-cofree adjunction, and $\cofree{p}$ is called the \emph{cofree comonoid} on $p$.

But before we prove \cref{thm.cofree}, let us investigate the implications of this theorem and give a concrete characterization of the cofree comonoid in terms of its positions, directions, and categorical structure.

%---- Subsection ----%
\subsection{The carrier of the cofree comonoid} \label{subsec.comon.cofree.cons.car}

We begin by characterizing the carrier $\ema{t}_p\in\poly$ of the cofree comonoid on $p$.
We can compute it directly as the limit of \eqref{eqn.cofree_diagram}, but we will be able to describe it more concretely in terms of what we call \emph{$p$-trees}: trees comprised of $p$-corollas.
...
\begin{enumerate}
    ...
    Such a tree has $1$ root and $|A|$ times as many vertices in one level than the level below it, so its height-$n$ vertices for each $n\in\nn$ are in bijection with the set $A^\ord{n}$: each $n$-tuple in $A^\ord{n}$ gives a length-$n$ path of directions in $A$ to follow up the tree, uniquely specifying a height-$n$ vertex.
...
So $\tr_p$ is the position-set of the cofree comonoid on $p$: we write $\ema{t}_p(\1)\iso\tr_p$.
...
\end{enumerate}
\begin{solution}
For the given values of $p$, we have already characterized the position-set $\ema{t}_p(\1)=\tr_p$ of $\ema{t}$ in \cref{exc.p_tree_sets}, so it remains to characterize the directions at each position, i.e.\ the vertices of each $p$-tree.
\begin{enumerate}
    ...
    \item We saw that $\tr_\yon\iso\1$ and that the unique $\yon$-tree is an infinite ray extending from the root, where every vertex has exactly $1$ child.
    ...
    \item We saw that $\tr_{\2\yon}\iso\2^\nn$, and that every $2\yon$-tree is an infinite ray, whose vertices are in bijection with $\nn$.
...
Any such length-$4$ rooted path up the tree ends at a height-$4$ vertex, so such a sequence of $4$ directions of $p$ comprising a direction of $p\tripow4$ is just another way to specify a height-$4$ vertex.

Similarly, the contents of position-boxes and the arrows going up on the codomain side carry all the data of the bottom $4$ levels of $T$: namely the label $i_0$ of the root, the label $i_1$ of the vertex at the end of every direction $a_1$ out of the root, and so on until $i_3$.
...
There, the position could be represented with a single $p\tripow{n}$-pretree $\pi^{(n)}T$, and the direction $(a_1,\ldots,a_n)$ could be represented with a single height-$n$ vertex from $\vtx_n(T)$.
So here, too, we can package the $2$ lower copies of $p$ into a single pair of polyboxes for $p\tripow2$, if we let $u$ be the height-$2$ vertex of $T$ at the end of the path $(a_1,a_2)$:
...
\begin{tikzpicture}[polybox, mapstos]
    ...
    \node[poly, cod, right=1.8 of r.south, yshift=-1ex, "$p\tripow2$" right] (q) {$u$\at$\pi^{(2)}T$};
    \node[poly, cod, above=of q, "$p\tripow2$" right] (q') {$w$\at$\pi^{(2)}T_u$};
...
\end{tikzpicture}
\end{equation}
Here $T_u$ denotes the $p$-tree equal to the subtree of $T$ rooted at its vertex $u$, and $w$ is one of $T_u$'s height-$2$ vertices.
When viewed as the subtree of $T$ rooted at its the height-$2$ vertex $u$, the $p$-tree $T_u$ has its height-$2$ vertex $w$ identified with a height-$4$ vertex of $T$, a descendent of $u$ that we denote by $u\leadsto w$.
Alternatively, we can think of $u$ as representing the rooted path $(a_1,a_2)$ of $T$, while $w$ represents the rooted path $(a_3,a_4)$ of $T_u$, so $u\leadsto w$ represents the concatenated rooted path $(a_1,a_2,a_3,a_4)$ of $T$.
As concatenation is associative, $\leadsto$ is associative as well. 

Note that both the notation $T_u$, for what we will call the $p$-\emph{subtree} of the $p$-tree $T$ rooted at $u\in\vtx(T)$, and the notation $u\leadsto w$, for the vertex in $\vtx(T)$ that $w\in\vtx(T_u)$ coincides with when $T_u$ is identified with the subtree of $p$ rooted at $u$, will turn out to be temporary---we will soon justify why we can express these concepts in much more familiar terms.%
...
As a sneak peek, they will be crucial to defining the categorical structure of the cofree comonoid on $p$.
...
\begin{tikzpicture}[polybox, mapstos]
    ...
    \node[poly, cod, above=of q, "$p\tripow1$" right] (q') {$t$\at$\pi^{(1)}T_r$};
...
\begin{tikzpicture}[polybox, mapstos]
	...
	\node[poly, cod, right=of S, "$p\tripow2$" right] (S2) {$s$\at$\pi^{(2)}T_x$};
	\node[poly, cod, below=of S2, "$p\tripow1$" right] (S1) {$x$\at$\pi^{(1)}T$};
	\node[poly, cod, above=of S2, "$p\tripow1$" right] (S3) {$t$\at$\pi^{(1)}T_{x\leadsto s}$};
%
	\draw (S_pos) to[first] (S1_pos);
	\draw (S1_dir) to[climb] (S2_pos);
	\draw (S2_dir) to[climb] (S3_pos);
...
\begin{tikzpicture}[polybox, mapstos]
	\node[poly, dom, "$=\qquad\ema{t}_p$" left] (p) {$x\leadsto y\leadsto z\leadsto t$\at$T$};
  	...
  	\node[poly, cod, "$p\tripow1$" right] (q2) at (4,1.5*2-3.25) {$y$\at$\pi^{(1)}T_x$};
  	\node[poly, cod, "$p\tripow1$" right] (q3) at (4,1.5*3-3.25) {$z$\at$\pi^{(1)}T_{x\leadsto y}$};
  	\node[poly, cod, "$p\tripow1$" right] (q4) at (4,1.5*4-3.25) {$t$\at$\pi^{(1)}T_{x\leadsto y\leadsto z}$};
...
\end{tikzpicture}
\]
Notice that $r=x\leadsto s$ and that $s=y\leadsto z$.
Also notice that this last depiction is just another way to write \eqref{eqn.eps_1_1_1_1_polybox}, with each $p\tripow1$-pretree in place of the $p$-position that labels its root and height-$1$ vertices in place of their corresponding directions.
...
\begin{tikzpicture}[polybox, mapstos]
    ...
    \node[poly, cod, above=of q, "$p\tripow{m}$" right] (q') {$w$\at$\pi^{(m)}T_u$};
...
\begin{tikzpicture}[polybox, mapstos]
	\node[poly, dom, "$\ema{t}_p$" left] (S) {$x\leadsto s\leadsto t$\at$T$};
	\node[poly, cod, right=of S, "$p\tripow{j}$" right] (S2) {$s$\at$\pi^{(j)}T_x$};
	\node[poly, cod, below=of S2, "$p\tripow{i}$" right] (S1) {$x$\at$\pi^{(i)}T$};
	\node[poly, cod, above=of S2, "$p\tripow{k}$" right] (S3) {$t$\at$\pi^{(k)}T_{x\leadsto s}$};
%
	\draw (S_pos) to[first] (S1_pos);
	\draw (S1_dir) to[climb] (S2_pos);
	\draw (S2_dir) to[climb] (S3_pos);
...
We now the carrier $\ema{t}_p$ of the cofree comonoid $\cofree{p}$, but we have yet to describe its comonoid structure.
This comonoid structure will allow us to interpret $\cofree{p}$ as a category, whose objects will be $p$-trees and whose morphisms will be the vertices of each $p$-tree.
...
The eraser for a comonoid $\cofree{p}$ carried by $\ema{t}_p$ should be a lens of the form $\epsilon_p\colon\ema{t}_p\to\yon$.
...
As for the rest of its categorical structure, that will be determined by its duplicator.


\subsubsection{The duplicator of the cofree comonoid}

The duplicator for a comonoid $\cofree{p}$ carried by $\ema{t}_p$ should be a lens of the form $\delta_p\colon\ema{t}_p\to\ema{t}_p\tri\ema{t}_p$.
...
Each projection from $\ema{t}_p\tri\ema{t}_p$ to $p\tripow\ell\tri p\tripow{m}$ in \eqref{eqn.cofree_tri2_diagram} is then the composition product of the projections $\epsilon^{(\ell)}_p\colon\ema{t}_p\to p\tripow\ell$ and $\epsilon^{(m)}_p\colon\ema{t}_p\to p\tripow{m}$, which we can draw in polyboxes like so:
\[
\begin{tikzpicture}[polybox, mapstos]
	\node[poly, dom, "$\ema{t}_p$" below] (p) {$v$\at$T$};
        \node[left=0pt of p_pos] {$\tr_p$};
        \node[left=0pt of p_dir] {$\vtx(-)$};
	\node[poly, dom, above=.8 of p, "$\ema{t}_p$" above] (p') {$w$\at$U(v)$};
	...
	\node[poly, cod, above=.8 of q, "$p\tripow{m}$" above] (q') {$w$\at$\pi^{(m)}U(v)$};
...
\end{tikzpicture}
\]
On positions, $\epsilon^{(\ell)}_p\tri\epsilon^{(m)}_p$ sends the $p$-tree $T$ to its stage-$\ell$ pretree $\pi^{(\ell)}T$, and it sends each $p$-tree $U(v)$ associated to a height-$\ell$ vertex $v\in\vtx_\ell(T)$ to its stage-$m$ pretree $\pi^{(m)}U(v)$, to be $p\tripow{m}$-pretree associated with the height-$\ell$ leaf $v$ of $\pi^{(m)}T$.
Equivalently, this specifies a $p\tripow{(\ell+m)}$-pretree on the right: its bottom $\ell$ levels coincide with the bottom $\ell$ levels of $T$, and its top $m$ levels coincide with the bottom $m$ levels of the $U(v)$'s for $v\in\vtx_\ell(T)$.
Then on directions, $\epsilon^{(\ell)}_p\tri\epsilon^{(m)}_p$ is the canonical inclusion of vertices $\vtx_\ell(T)\inj\vtx(T)$ sending $v\mapsto v$, followed by the canonical inclusion of vertices $\vtx_m(U(v))\inj\vtx(U(v))$.
...
\end{tikzcd}
\end{equation}
In fact, we can take this to be the definition of $\delta_p$: it is the unique lens for which the above diagram commutes for all $\ell,m\in\nn$.
...
Expressing this commutativity as an equation of polyboxes, using our usual labels for the arrows of the duplicator $\delta_p$ on the left and our depiction of the projection $\epsilon^{(\ell+m)}_p\colon\ema{t}_p\to p\tripow\ell\tri p\tripow{m}$ \cref{ex.eps_n_polybox} on the right, we have
\[
\begin{tikzpicture}
	\node (1) {
  \begin{tikzpicture}[polybox, mapstos]
  ...
  \begin{tikzpicture}[polybox, mapstos]
  	...
  	\node[poly, cod, above=of q, "$p\tripow{m}$" right] (q') {$w$\at$\pi^{(m)}T_v$};
...
\end{tikzpicture}
\]
Recall from \cref{ex.eps_n_polybox} that $T_v$ denotes the $p$-tree equal to the subtree of $T$ rooted at $v\in\vtx(T)$, while $v\leadsto w\in\vtx(T)$ for $w\in\vtx(T_v)$ is the descendent of $v$ that coincides with $w$ when $T_v$ is identified with the subtree of $T$ rooted at $v$.
Equivalently, we can identify $v\in\vtx(T)$ with the rooted path in $T$ that ends at the vertex $v$ and $w\in\vtx(T_v)$ with the rooted path of $T_v$ that ends at the vertex $w$, so $v\leadsto w$ becomes the rooted path in $T$ obtained by concatenating $v$ and $w$.
Then for this equality to hold over all $\ell,m\in\nn$, we want $\cod v\coloneqq T_v$ and $v\then w\coloneqq v\leadsto w$; in fact, this will henceforth be our preferred notation.

\subsubsection{Verifying the comonoid laws}

Putting together our constructions of the carrier, the eraser, and the duplicator of the cofree comonoid yields the following result.
...
\begin{itemize}
    \item Objects are $p$-trees in $\tr_p$.
    \item Morphisms are vertices of $p$-trees: for each $p$-tree $T\in\tr_p$ and vertex $v\in\vtx(T)$, there is a morphism $v\colon T\to\_$ whose codomain $\cod v\in\tr_p$ is the $p$-subtree of $T$ rooted at $v$.
    Equivalently, a morphism is a finite rooted path in its domain whose codomain is the $p$-subtree rooted at the end of the path.%
    \footnote{Both the morphisms-as-vertices and the morphisms-as-(finite-)rooted-paths perspectives are helpful for interpreting the category $\cofree{p}$, so we include both; it is not terribly difficult to translate between them.}
    \item Identity morphisms are roots: for each $p$-tree $T\in\tr_p$, the identity morphism $\id_T\in\vtx(T)$ on $T$ is the root vertex of $T$.
    Equivalently, identity morphisms are empty rooted paths that start and end at the root.
    \item Composition is given as follows: for each $p$-tree $T\in\tr_p$, vertex $v\in\vtx(T)$, and vertex $w\in\vtx(\cod T)$, the composite morphism $v\then w\in\vtx(T)$ is the descendent of $v$ in $T$ that coincides with $w$ when $\cod v$ is identified with the subtree of $T$ rooted at $v$.
    Equivalently, rooted paths $v$ and $w$ compose via concatentation: given a rooted path $v$ in $T$ and a rooted path $w$ in $\cod v$ (the $p$-subtree rooted at the end of $v$), we identify $w$ with the corresponding path in $T$ that starts where $v$ ends, then concatenate the two paths in $T$ to obtain the composite morphism $v\then w$, another rooted path in $T$. 
\end{itemize}
\end{proposition}
\begin{proof}[Proof of \cref{prop.cofree_as_cat}]
We have already shown how to construct its carrier, eraser, and duplicator purely diagrammatically, and we have given them concrete interpretations in terms of $p$-trees, their vertices, and their $p$-subtrees.
So to complete the proof of \cref{prop.cofree_as_cat}, it remains to verify that the category laws hold for $\cofree{p}$, or equivalently that the comonoid laws hold for $\left(\ema{t}_p,\epsilon^{(0)}_p,\delta_p\right)$.
Again, our argument will be purely formal, although of course it is not so hard to see that our concrete characterization above in terms of $p$-trees satisfies the laws for a category.
...
\]
commutes.
We know the lens $\epsilon^{(0)}_p\tri\ema{t}_p\colon\ema{t}_p\tri\ema{t}_p\to\yon\tri\ema{t}_p$ is specified by its components $\epsilon^{(0)}_p\tri\epsilon^{(n)}_p\colon\ema{t}_p\tri\ema{t}_p\to p\tripow0\tri p\tripow{n}$, a projection out of the limit $\ema{t}_p\tri\ema{t}_p$, for all $n\in\nn$.
...
Together, these specify the composite lens $\delta_p\then\left(\epsilon^{(0)}_p\tri\ema{t}_p\right)\colon\ema{t}_p\to\yon\tri\ema{t}_p\iso\ema{t}_p$ as the lens whose components are the projections $\epsilon^{(n)}_p\colon\ema{t}_p\to p\tripow{n}$ from the limit.
...
\]
for each $i,j,k\in\nn$, with additional arrows between the constant polynomials.
...
\begin{align*}
    ...
    \delta_p\then\left(\left(\delta_p\then\left(\epsilon^{(i)}_p\tri\epsilon^{(j)}_p\right)\right)\tri\epsilon^{(k)}_p\right) \\ &=
...
In the next section, we will prove that it really is the cofree category on $p$, but first we give some examples.

\subsubsection{Examples of $p$-tree categories}

Throughout these examples, we will freely switch between the morphisms-as-vertices and the morphisms-as-rooted-paths perspectives of $p$-tree categories given in \cref{prop.cofree_as_cat} as is convenient.
Since only finite rooted paths correspond to vertices, all our rooted paths will be assumed to be finite.
When we wish to refer to what one might call an infinite (rooted) ``path,'' we will instead call it a (rooted) \emph{ray}.

\begin{example}[The category of $\1$-trees is the terminal category]
...
\begin{exercise}[The category of trees on a constant polynomial]
...
\begin{example}[The category of $\yon$-trees is the natural numbers monoid]
...
\begin{exercise}[The category of trees on a linear polynomial]
...
% ** Exercise (above) or example (below)?
...
% ** borrow nLab pf, check old paper



% ** include examples from systems stuff
...
% \begin{remark}
% The construction of the cofree comonoid from a copointed endofunctor in the proof of \cref{thm.cofree2} is fairly standard; see \cite{lack2010note}. 
% One could also construct this right adjoint using the following limit, again applied to the original polynomial $p$:
% \[
