...
\begin{example}[The do-nothing enclosure in polyboxes]
...
We'll see how to address this issue shortly in \cref{sec.comon.comp.to_comp.trans}.
\end{example}
...
We will read equations off of polyboxes like this repeatedly in the rest of the book, including in the next section.

\subsection{Transition lenses for state systems} \label{sec.comon.comp.to_comp.trans}
...
\end{tikzpicture}
\]
This \emph{almost} models the behavior of running through the system twice, except that we should really only have one blue box on the domain side, the one we fill with the initial state $s_0$; the second blue box on the domain side, the one we fill with $s_1$, should instead be filled automatically with the same state as the direction in the white box below it.

In fact, it would be nice if our domain were still just $S\yon^S$, to obtain a lens $S\yon^S\to p\tri p$ that takes an initial state $s_0\in S$ and runs the original system $\phi$ twice, returning two outputs and receiving two inputs before stopping at the new state $s_2$.
But we just learned how to take composition product of lenses $\phi\tripow{2}\colon S\yon^S\tri S\yon^S\to p\tri p$ and convert its domain to a new polynomial $S\yon^S$, with only one blue box on the domain side---just compose it with another lens $S\yon^S\to S\yon^S\tri S\yon^S$ like so:
\[
\begin{tikzpicture}
	\node (1) {
  \begin{tikzpicture}[polybox, mapstos]
...
\end{tikzpicture}
\]
We'll denote our new lens $S\yon^S\to S\yon^S\tri S\yon^S$ on the left by $\delta$.
...
**
We call $\delta$ the \emph{transition lens} of the state system $S\yon^S$.
