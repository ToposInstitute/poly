...
\end{equation}
and refer to $p(\1)$ as the set of positions (or position-set), and for each $i\in p(\1)$ we'll refer to $p[i]$ as the set of directions (or direction-set) at position $i$.
...
\begin{solution}
Given $p,q,r \in \poly$, recall that the position-set of the sum $p+q$ is $p(\1)+q(\1)$, while the direction-set at each position $i \in p(\1) + q(\1)$ is $p[i]$ if $i \in p(\1)$ and the $q[i]$ if $i \in q(\1)$.
...
\end{split}
\]
We can also recall that the position-set of the product $pq$ is $p(\1) \times q(\1)$, while the direction-set at each position $(i, i') \in p(\1) \times q(\1)$ is $p[i] + q[i']$.
So by \cref{eqn.composite_formula}, we have that
\[
\begin{split}
