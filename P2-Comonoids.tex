...
In particular, $p\tripow{0}\coloneqq\yon$ and $p\tripow{1}\coloneqq p$.
...
\end{equation}
obtained by composing the $q(X)$-component of $f$ with $p'$ applied to the $X$-component of $g$.
\end{definition}
...
\begin{tikzpicture}[polybox, mapstos]
    \node[poly, dom, "$p$" left] (p) {$d$\nodepart{two}$i$};
    \node[poly, cod, "$q$" right, right=of p] (q) {$e$\nodepart{two}$j$};
...
\begin{tikzpicture}[polybox, mapstos]
	\node[poly, dom, "$p$" left] (p) {$d$\nodepart{two}$i$};
	\node[poly, cod, right=1.5cm of p.south, yshift=-1ex, "$q_1$" right] (q1) {$e_1$\nodepart{two}$j_1$};
	\node[poly, cod, above=of q1, "$q_2$" right] (q2) {$e_2$\nodepart{two}$j_2$};
...
In fact, \eqref{eqn.map_to_0ary_composite}, \eqref{eqn.map_to_2ary_composite}, and \eqref{eqn.polybox_lens} are the polybox depictions of the $n=0, n=1,$ and $n=2$ examples of lenses $p\to q_1\tri\cdots\tri q_n$ to $n$-fold composites.
...
\slogan{A lens $p\to q_1\tri\cdots\tri q_n$ is a multi-step policy for $p$ to make decisions by asking for decisions from $q_1$, then $q_2$, etc., all the way to $q_n$, and interpreting the results.}
...
% A special case of a lens whose codomain is a composite is a lens that is itself the composition product of lenses.  — just stacking the lenses on top of each other. Here is an example of a $2$-fold composition product of lenses, depicted as polyboxes

% How does this relate to our usual polybox depiction of a lens into a composite? We could think of filling the two independently. Or we could think of already deciding what to put in the circle for every chocie of what's in the square, which is just picking a position of the composite domain...bottom 3 arrows give position of  ...
