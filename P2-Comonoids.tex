...
\begin{tabular}{cc|cc}
  ...
  & 
  \rule{0pt}{2ex} $\delta_i^\sharp\colon(\ema{c}\tri\ema{c})[\delta_\1(i)]\to\ema{c}[i]$ & composition* & $\then$ \\ %$\displaystyle\comp\colon\sum_{f\in\cat{C}[i]}\cat{C}[\cod(f)]\to\cat{C}[i]$
  \hline
...
\end{tikzcd}
\end{equation}
\end{definition} %** maybe "preserves" erasers and duplicators??

Let's see the two laws of comonoid morphisms using polyboxes. First the eraser law:
...
\begin{enumerate}
	\item Write out the duplicator law from \eqref{eqn.unit_law_draw} in terms of polyboxes.
