...
A \emph{morphism} of $\cat{C}$-coalgebras $(S,\alpha)\to(T,\beta)$ is a function $f\colon S\to T$ such that the following diagram commutes:
\[
\begin{tikzcd}
    S \ar[d, "f"'] \ar[r, "\alpha"] &
    \car{c}\tri S \ar[d, "\car{c}\:\tri\:f"] \\
    T \ar[r, "\beta"'] &
    \car{c}\tri T
\end{tikzcd}
\]
...
\begin{proposition}
Up to isomorphism, cofunctors $S\yon^S\cof\cat{C}$ can be identified with $\cat{C}$-coalgebras carried by $S$.
\end{proposition}
\begin{proof}
**
\end{proof}
...
The equivalences don't stop here, however.
...
\begin{definition}[Pretree on a polynomial] \label{def.pretree}
