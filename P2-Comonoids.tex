...
\index{two-step procedure!composition product as}
...
\index{decision!multi-step}\index{menu}
\index{polybox}
...
\index{dynamical system!composite interface}
...
\index{category!connected}\index{equalizer}\index{pullback}\index{directed limit}
...
\index{independence}\index{polybox}
...
\index{dynamical system!running}
...
\begin{example}[Returning every other output] \label{ex.long_div_skip}
...
\index{polynomial comonoid!categorical structure|(}
...
\begin{tabular}{cc|cc}
  ...
  \hline
  \multicolumn2{c|}{\multirow{2}{*}{coassociative law}} & codomains of composites & $\cod(f\then g)=\cod g$ \\\index{coassociativity}
...
or that
\slogan{
    a comonoid structure on an polynomial turns\\
...
\index{cofunctor}\index{polynomial comonoid!morphism of|see{cofunctor}}
...
\end{enumerate}
We let $\catsharp\iso\comon(\poly)$ denote the category of (small) categories and cofunctors.
\end{definition}
...
\end{enumerate}
\begin{solution}
Here $F\colon\cat{C}\cof\cat{D}$ and $G\colon\cat{D}\cof\cat{E}$ are cofunctors in $\catsharp$.
\begin{enumerate}
...
\index{cofunctor!similarity with functor}
...
Similarly, $\cat{C}$ and $\cat{D}$ are isomorphic in $\catsharp$ if there exist cofunctors $H\colon\cat{C}\cof\cat{D}$ and $K\colon\cat{D}\cof\cat{C}$ that are mutually inverse, i.e.\ $H\then K$ and $K\then H$ are identity cofunctors on $\cat{C}$ and $\cat{D}$.
...
Conversely, assume $\cat{C}$ and $\cat{D}$ are isomorphic in $\catsharp$, so that there exist mutually inverse cofunctors $H\colon\cat{C}\cof\cat{D}$ and $K\colon\cat{D}\cof\cat{C}$.
...
\index{cofunctor!to discrete categories}
...
\index{cofunctor!to monoids}


\subsubsection{Cofunctors to monoids}

When a monoid $(M,e,*)$, viewed as a $1$-object category $\yon^M$, is the codomain of a cofunctor $\cat{C}\cof\yon^M$, it plays the role of a joystick: an ``input device'' that ``reports its\dots direction to the device it is controlling.''\index{control}%
...
\index{dynamical systems!as cofunctors}
...
\index{state category!cofunctor from|(}
...
\begin{example}[Cofunctors from state categories to $\cat{C}$ are $\cat{C}$-coalgebras]\index{coalgebra!of polynomial comonad}\index{polynomial comonad!coalgebra of}
...
\index{lens!very well behaved|seealso{cofunctor, between state categories}}\index{cofunctor!between state categories}
...
\index{coalgebra!as cofunctor from state category}
...
\begin{proposition}
Cofunctors $S\yon^S\cof\cat{C}$ can be identified (up to isomorphism) with $\cat{C}$-coalgebras carried by $S$.\index{coalgebra!carrier of}
\end{proposition}
\begin{proof}
...
Then it suffices to show that $\Phi$ satisfies the cofunctor laws if and only if $\varphi$ satisfies the coalgebra laws.\index{coalgebra}
We can verify this using polyboxes.
From \eqref{eqn.pres_era_draw}, the eraser preservation law for $\Phi$ would state the following (remember that the arrow in the eraser for the state category $S\yon^S$ is just an equality):\index{polybox}\index{state category}
\[
\begin{tikzpicture}
	\node (id1) {
	\begin{tikzpicture}[polybox, mapstos]
...
\index{discrete opfibration!as cofunctor}
...
\end{tikzcd}
\end{equation}
Under the forgetful-cofree adjunction (\cref{thm.cofree}), the lens $\varphi$ coincides with a cofunctor $F\colon S\yon^S\cof\cofree{p}$ from the state category on $S$ to the category of $p$-trees.
...
While we defined the category $\catsharp$ of categories and cofunctors in the last chapter, we have only begun to scratch the surface of the properties it satisfies.
...
\index{category!of $p$-trees|see{cofree comonoid}}
...
\begin{theorem}[Cofree comonoid] \label{thm.cofree}\index{cofree comonoid}
...
\end{tikzcd}
\]
That is, for any category $\cat{C}\in\catsharp$ with carrier $\car{c}\coloneqq U\cat{C}$, there is a natural isomorphism
\[
...
\index{cofree comonoid!interfaces for}

The forgetful-cofree adjunction of \cref{thm.cofree} tells us that given a category $\cat{C}\in\catsharp$ with carrier $\car{c}\coloneqq U\cat{C}$ and a polynomial $p\in\poly$, there is a natural isomorphism
\[
    \poly(\car{c},p)\iso\catsharp(\cat{C},\cofree{p}).
\]
...
\begin{example}[Input sequences to output sequences] \label{ex.input_output}
...
\index{cofree comonoid!morphisms between}
...
\index{cofree comonoid!as free category on a graph}
...
\index{polynomial comonoids!representable}
...
\index{polynomial comonoids!linear}
...
\index{cofunctor!vertical}\index{cofunctor!cartesian}
...
\index{discrete opfibration!as cartesian cofunctor}
...
subcategory of cartesian cofunctors in $\catsharp$ is isomorphic to the wide subcategory of discrete opfibrations in $\smcat$.
\end{proposition}
\begin{proof}
...
\index{polynomial comonoid!colimits of}\index{colimit!in $\catsharp$}
...
\index{polynomial comonoid!coproducts of}\index{coproduct!of polynomial comonoids}
...
\index{parallel product!of polynomial comonoids}\index{polynomial comonoids!parallel product of}
...
\subsection{More equivalences}\index{cofunctor!from state category}\index{coalgebra}

We have seen that a cofunctor from a state category to a category $\cat{C}$ carries the same data as a $\cat{C}$-coalgebra, which is in turn equivalent to a discrete opfibration over $\cat{C}$ or a copresheaf on $\cat{C}$.\index{copresheaf}
We then showed that cartesian cofunctors to $\cat{C}$ is an equivalent notion as well. % TODO: refs?
...
There is an equivalent characterization of bicomodules over a pair of polynomial comonoids due to Richard Garner.\index{Garner, Richard} Thus we attribute the foundational theory of $\catsharp$ to Ahman-Uustalu-Garner.
...
\subsection{Bicomodules in dynamics}\index{bicomodule!dynamical systems and bicomodules}\index{dynamical systems!as bicomodules}
