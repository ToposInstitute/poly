...
It is the limit of the diagram \eqref{eqn.cofree_diagram}, with projections $\epsilon^{(n)}_p\colon\ema{t}_p\to p\tripow{n}$ for every $n\in\nn$ making the following diagram commute:
...
Concretely, we know that a $\ema{t}_p\tri\ema{t}_p$-position is just a $p$-tree $T\in\tr_p$ along with a $p$-tree $T_v\in\tr_p$ associated with every vertex $v\in\vtx(T)$.
Then a direction at that position is a choice of vertex $v\in\vtx(T)$ and another vertex $w\in\vtx(T_v)$ of the $p$-tree $T_v$ associated with $v$.
...
\begin{tikzpicture}[polybox, mapstos]
	\node[poly, dom, "$\ema{t}_p$" below] (p) {$v$\at$T$};
        \node[left=0pt of p_pos] {$\tr_p$};
        \node[left=0pt of p_dir] {$\vtx(-)$};
	\node[poly, dom, above=.8 of p, "$\ema{t}_p$" above] (p') {$w$\at$T_v$};
	...
	\node[poly, cod, above=.8 of q, "$p\tripow{m}$" above] (q') {$w$\at$\pi^{(m)}T_v$};
...
\end{tikzpicture}
\]
On positions, $\epsilon^{(\ell)}_p\tri\epsilon^{(m)}_p$ sends the $p$-tree $T$ to its stage-$\ell$ pretree $\pi^{(\ell)}T$, and it sends each $p$-tree $T_v$ associated to a height-$\ell$ vertex $v\in\vtx_\ell(T)$ to its stage-$m$ pretree $\pi^{(m)}T_v$, to be $p\tripow{m}$-pretree associated with the height-$\ell$ leaf $v$ of $\pi^{(m)}T$.
On directions, it is the canonical inclusion of vertices $\vtx_\ell(T)\inj\vtx(T)$ sending $v\mapsto v$, followed by the canonical inclusion of vertices $\vtx_m(T_v)\inj\vtx(T_v)$.
