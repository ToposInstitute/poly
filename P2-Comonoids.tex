...
Consider the dynamical system $\varphi\colon S\yon^S\to p$ corresponding to the halting deterministic state automaton \eqref{eqn.halt_dsa} from \cref{exc.halt_dsa}, depicted here again for your convenience:
\[
...
% Let $S\coloneqq\{\bul[dgreen],\bul[dyellow],\bul[red]\}$ and $p\coloneqq\yon^\2+\1$, and c
\begin{solution}
**
\end{solution}
\end{exercise}
...
What is the induced lens $\text{Run}_n(\varphi)$?
First of all, it is a dynamical system
\[\text{Run}_n(\varphi)\colon S\yon^S\to\rr^n\yon,\]
