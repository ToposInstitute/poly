...
%	\item Show that there exists a polynomial $\fl{c}$ and a vertical morphism $\varphi_c\colon c\to \fl{c}$ such that the induced map $(\varphi_c,\epsilon)\colon c\to \fl{c}\yon$ is an isomorphism, i.e.\ $\varphi_c$ is a product projection.
%	\item Show that $\varphi_c$ is an epimorphism using \cref{prop.epis_in_poly}.
%\qedhere
%\end{enumerate}
%\end{exercise}
%
%\begin{definition}\label{def.fleece}
%Suppose $\com{C}=(c,\epsilon,\delta)$ is a comonoid, and let $\varphi_c\colon c\to \fl{c}$ be as in \cref{ex.fleece}. We refer to the polynomial $\fl{c}$ as the \emph{fleece} of $c$ and $\varphi_c$ as the \emph{fleecing map}.
%
%If there exists a map $\fl{\delta}\colon \fl{c}\to \fl{c}\tri \fl{c}$ making the diagram
%\[
%\begin{tikzcd}
%	c\ar[r, "\delta"]\ar[d, "\varphi_c"']&
%	c\tri c\ar[d, "\varphi_c\tri\varphi_c"]\\
%	\fl{c}\ar[r, "\fl{\delta}"']&
%	\fl{c}\tri \fl{c}
%\end{tikzcd}
%\]
%commute,%
%\tablefootnote{Note that such a morphism $\fl{\delta}$ must be unique since $\varphi_c$ is an epimorphism.}
...
\begin{itemize}
    \item an object $m\in\Cat{C}$, equipped with
...
\end{tikzcd}
\end{equation}
When referring to a left $\cat{C}$-comodule, we may omit its coaction if it can be inferred from context.

A \emph{morphism} of left $\cat{C}$-comodules $m$ and $n$ is a morphism $m\to n$ such that the following diagram commutes:
\[
\begin{tikzcd}
	\car{c}\tri m\ar[d]&
	m\ar[l]\ar[d]\\
...
When $\cat{C}$ is a polynomial comonoid, we denote the category of left $\cat{C}$-comodules in $\poly$ by $\lcmod{\cat{C}}$.
\end{definition}
...
\end{tikzcd}
\end{equation}
When referring to a right $\cat{D}$-comodule, we may omit its coaction if it can be inferred from context.

A \emph{morphism} of right $\cat{D}$-comodules $m$ and $n$ is a morphism $m\to n$ such that the following diagram commutes:
\[
\begin{tikzcd}
	m\ar[r]\ar[d]&
	m\tri\car{d}\ar[d]\\
...
When $\cat{D}$ is a polynomial comonoid, we denote the category of right $\cat{D}$-comodules in $\poly$ by $\rcmod{\cat{D}}$.
\end{definition}
