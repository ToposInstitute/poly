...
From \eqref{eqn.pres_era_draw}, the eraser preservation law for $\Phi$ would state that
\[
\begin{tikzpicture}
	\node (id1) {
	\begin{tikzpicture}[polybox, mapstos]
	...
	\node[right=of id1] (id2) {
	\begin{tikzpicture}[polybox, mapstos]
		\node[poly, dom, blue, "$S\yon^S$" left] (p) {\at$\vphantom{F}s$};
		\node[poly, red, right=1 of p, "$\car{c}$" above] (q) {$\id_{\Phi_\1 s}$\at$\Phi_\1 s$};
...
\begin{tikzpicture}
	...
	\begin{tikzpicture}[polybox, mapstos]
    	\node[poly, constant dom, "$S$" left, blue] (p) {\at$s$};
    	\node[poly, constant, "$S$" left, blue, right=1 of p] (q) {\at$s$};
		\draw[double] (p_pos) to[first] node[below] {$\Phi_\1$} (q_pos);
		\draw[double] (q_dir) to[last] node[above] {$\Phi^\sharp$} (p_dir);
	\end{tikzpicture}	
	};
	\node[right=of id1] (id2) {
	\begin{tikzpicture}[polybox, mapstos]
    	\node[poly, constant dom, "$S$" left, blue] (p) {\at$s$};
    	\node[poly, cod, right=1.5cm of p.south, yshift=-1.25ex, "$\car{c}$" below, red] (r) {$f$\at$i$};
    	\node[poly, above=of r, "$S$" right, constant, blue] (q) {\at$t$};
...
\end{tikzpicture}
\]
But these are entirely equivalent.

Then from \eqref{eqn.pres_dup_draw}, the duplicator preservation law for $\Phi$ would state that
\[
\begin{tikzpicture}
	\node (sp1) {
	\begin{tikzpicture}[polybox, mapstos]
...
But these are equivalent as well.

Hence the cofunctor laws for $\Phi$ are equivalent to the coalgebra laws for $\varphi$.
\end{proof}

Hence a cofunctor from a state category to $\cat{C}$ bears the same data as a $\cat{C}$-coalgebra.
...
\begin{enumerate}
	\item Construct a cartesian cofunctor $\Cat{Fr}(G)\cof\cofree{\1+\yon+\yon^\2+\yon^\3+\cdots}$.
