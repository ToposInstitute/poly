...
Before we get into how $\otimes$ interacts with $\tri$, here are some warm-up exercises.

\begin{exercise}
Let $A$ and $B$ be arbitrary sets, and let $p$ be an arbitrary polynomial.
Which of the following isomorphisms always hold?
In the cases where the isomorphism does not always hold, is there still a canonical map in one direction or the other?
\begin{enumerate}
	\item $(A\yon)\otimes(B\yon) \iso (A\yon)\tri (B\yon)$?
	\item $\yon^A\otimes\yon^B\iso\yon^A\tri\yon^B$?
	\item $A\otimes B\iso A\tri B$?
	\item $A\yon\otimes p\iso A\yon\tri p$?
	\item $\yon^A\otimes p\iso \yon^A\tri p$?
	\item $p\otimes A\yon\iso p\tri A\yon$?
	\item $p\otimes \yon^A\iso p\tri\yon^A$?
\end{enumerate}
\begin{solution}
Here $A$ and $B$ are sets and $p$ is a polynomial.
\begin{enumerate}
	\item The isomorphism holds: we have that $(A\yon)\otimes(B\yon) \iso AB\yon \iso (A\yon)\tri (B\yon)$.
	\item The isomorphism holds: we have that $\yon^A\otimes\yon^B \iso \yon^{AB} \iso \yon^A\tri\yon^B$.
	\item The isomorphism does not always hold: while $A\otimes B \iso AB$, we have that $A\tri B \iso A$.
	\item The isomorphism holds: we have that $A\yon \otimes p \iso \sum_{i \in p(\1)} A\yon \otimes \yon^{p[i]} \iso \sum_{i \in p(\1)} A\yon^{p[i]} \iso Ap \iso A\yon\tri p$.
	\item The isomorphism does not always hold: if, say, $p = B$, then $\yon^A \otimes B \iso B$, while $\yon^A \tri B \iso B^A$.
	\item The isomorphism does not always hold: if, say, $p = \yon^B$, then $\yon^B \otimes A\yon \iso A\yon^B$, while $\yon^B \otimes A\yon \iso (A\yon)^B \iso A^B\yon^B$.
	\item The isomorphism holds: we have that $p \otimes \yon^A \iso \sum_{i \in p(\1)} \yon^{p[i]} \otimes \yon^A \iso \sum_{i \in p(\1)} \yon^{Ap[i]} \iso p \tri \yon^A$.
\end{enumerate}
\end{solution}
\end{exercise}
