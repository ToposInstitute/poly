...
\begin{tikzpicture}[polybox, mapstos]
	...
	\draw (p_pos) -- node[below] {$f_\1$} (q_pos);
	\draw (q_dir) -- node[above] {$f^\sharp$} (p_dir);
	\draw (p'_pos) -- node[below] {$g_\1$} (q'_pos);
	\draw (q'_dir) -- node[above] {$g^\sharp$} (p'_dir);	
\end{tikzpicture}
\]
How does this relate to our usual polybox depiction of a lens to a composite?
We could think of filling the two independently. Or we could think of already deciding what to put in the circle for every chocie of what's in the square, which is just picking a position of the composite domain...bottom 3 arrows give position of  ...
