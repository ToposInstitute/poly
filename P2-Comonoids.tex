...
Often we will denote a category by its carrier when its comonoid structure can be deduced from context, and $\cat{C}$ will be a category throughout.
...
We actually defined this way back in \cref{ex.lens_get_put}, where we called the on-objects (on-positions) function of such a cofunctor $\text{get}\colon S\to T$, and the on-morphisms (on-directions) function $\text{put}\colon S\times T\to S$.%
...
\begin{enumerate}
    ...
    \]
    for all $s\in S$, known as the \emph{get-put law} (named in diagrammatic order: we apply $\text{get}$ before we apply $\text{put}$).
...
\begin{example}[Very well-behaved lenses are product projections]\label{ex.well_behaved_boring}
...
\end{tikzcd}
\]
Expressing this commutativity as an equation of polyboxes, using our usual labels for the arrows of the duplicator $\delta_p$ on the left and our depiction of the $\epsilon^{(\ell+m)}_p\colon\ema{t}_p\to p\tripow\ell\tri p\tripow{m}$ \cref{ex.eps_n_polybox} on the right, we have
\[
\begin{tikzpicture}
	\node (1) {
  \begin{tikzpicture}[polybox, mapstos]
...
As defined above, $\left(\ema{t}_p,\epsilon^{(0)}_p,\delta_p\right)$ is a polynomial comonoid corresponding to a category characterized as follows.
\begin{itemize}
    ...
    \item Composition is given as follows: for each $p$-tree $T\in\tr_p$, vertex $v\in\vtx(T)$, and vertex $w\in\vtx(\cod T)$, the composite morphism $v\then w\in\vtx(T)$ is the descendent of $v$ in $T$ that coincides with $w$ when $T_v$ is identified with the subtree of $T$ rooted at $v$.
\end{itemize}
\end{proposition}
Unsurprisingly, we call this category the \emph{category of $p$-trees}; it will turn out to be our cofree category $\cofree{p}$ on $p$.
...
% Let $p\in\poly$ be a polynomial. The comonoid $\cofree{p}=(\tr_p,\rt,\fs)$ as in \cref{thm.cofree} is called the \emph{cofree comonoid on $p$} or the category of \emph{$p$-trees}.

% An object $t\in \tr_p(\1)$ is a called a \emph{tree on $p$.} Given such an object $t$ in the category, an emanating morphism $n\in\tr_p[t]$ is called a \emph{path from root}.
% \end{definition}
