...
In 2018, researchers Daniel Ahman and Tarmo Uustalu showed that comonoids in $(\poly,\yon,\tri)$ can be identified with categories.
Every category in the usual sense is a comonoid in $\poly$, and every comonoid in $\poly$ is a category. 
We find this revelation to be truly shocking, and it suggests some very different ways to think about categories.
Let's go through it.
% The story begins with state systems of dependent dynamical systems
...
We will prove this in more generality in \cref{prop.n_duplication}, but here's some practice with the $n=4$ case for a taste of what's to come.
...
Given a dynamical system $\phi\colon\ema{s}\to p$ with interface $p\in\poly$, %** run through the system n times...
...
\end{tikzcd}
\end{equation}
%** alt interp: faster, speed up...
we want to make it go $k$-times faster. That is, in every moment, we want it to process $k$-many inputs, rather than one. 
...
Here is a polybox drawing for the $n=3$ case:
\[
\begin{tikzpicture}
	\node (given) {
	\begin{tikzpicture}[polybox, tos]
...
\begin{example}
Let $p\coloneqq\rr\yon^\1$, let $S\coloneqq\nn$, and let $\phi\colon S\yon^S\to p$ be given by $\phi_\1(n)\coloneqq n$ and $\phi^\sharp_n(1)\coloneqq n+1$. What is this speedup map $\text{Run}_k(\phi)$? First of all, its type is 
\[\text{Run}_k(\phi)\colon S\yon^S\to\rr^k\yon,\]
...
In a given monoidal category $(\cat{C},\yon,\tri)$, a \emph{comonoid} $(\ema{c},\epsilon,\delta)$ consists of:
\begin{itemize}
    \item an object $\ema{c}\in\cat{C}$, called the \emph{carrier};
    \item a morphism $\epsilon\colon\ema{c}\to\yon$ in $\cat{C}$, called the \emph{erasure} (or the \emph{counit}); and
    \item a morphism $\delta\colon\ema{c}\to\ema{c}\tri\ema{c}$ in $\cat{C}$, called the \emph{duplication} (or the \emph{comultiplication});
\end{itemize}
...
\end{tikzcd}
\end{equation}
known as the \emph{coassociativity law}.

We refer to a comonoid $(\ema{c},\epsilon,\delta)$ in $(\poly,\yon,\tri)$ as a \emph{polynomial comonoid}.
\end{definition}
...
The alternative names \emph{erasure} for the \emph{counit} and \emph{duplication} for the \emph{comultiplication} are less standard, but we will favor them to avoid confusion between the counit of a \emph{comonoid} and the counit of an \emph{adjunction}---and so that their names match up with the Greek letters $\epsilon$ and $\delta$ that we will so often use to label them.
The word ``duplication'' comes from the fact that $\delta\colon\ema{c}\to\ema{c}\tri\ema{c}$ effectively turns one $\ema{c}$ into two, while the ``erasure'' $\epsilon\colon\ema{c}\to\yon$ erases the $\ema{c}$ altogether, leaving only the monoidal unit $\yon$.
...
%**duplication—more generally? laws let us do this!

Here's a well-known fact about comonoids that we have already made use of in the case of state systems, along with a corresponding piece of notation.
...
Finish the proof of \cref{prop.n_duplication} as follows.
\begin{enumerate}
    \item Prove part (a).
    \item Prove part (b).
...
%**Integrate the below into previous prop.
%**Discuss how we used this notation before for state systems

Nearly all our work on state systems up until now can be summarized thusly:
\slogan{
    every state system is a polynomial comonoid, whose erasure is the do-nothing enclosure and whose duplication is the transition lens.
}
%** speed ups here??


% the only part that doesn't fit into this story is ... feature not bug ... generalize state systems to arbitrary polynomial comonoids
...
% ** modify below...out of date...just say this is smth known in CS? Is it??
% \begin{example}[The state comonad $S\yon^S$]\label{ex.state_comonad_1}
% Let $S$ be a set, and consider the polynomial $p\coloneqq S\yon^S$. It has a canonical comonoid structure---often called the \emph{state} comonad---as we discussed in \cref{sec.dynam_in_poly}, page~\pageref{page.poly_comonad}. 
% \end{example}

% ** This is good, but update all language to current
\begin{example}[Picturing the comonoid $S\yon^S$ as trees] \label{ex.picturing_SyS}
...
It remains to check the comonoid laws.
The two erasure laws in \cref{eqn.erasure_law} say that the composites
\[
...
\end{tikzpicture}
\]
We do not prove the coassociativity law here, but instead leave it to the reader in \cref{exc.state_comonoid_assoc}.
\end{example}
...
\begin{enumerate}
	...
	\item We know $c$ is supposed to be the carrier of a comonoid $(c,\epsilon,\delta)$. Which two maps $ c\to c\tripow3$ are supposed to be equal by associativity?
...
Once you know that state systems are comonoids in $\poly$, it's interesting to ask ``what are all the other comonoids in $\poly$?''
% **
...
There is a one-to-one isomorphism-preserving correspondence between small categories and polynomial comonoids.
\end{theorem}
...
\begin{definition}[Emanation polynomial]
