...
Our goal is to define and demonstrate this correspondence so that we can justly proclaim:
...
Keeping our label $\idy$ for the arrow in $\epsilon$, the right erasure law $\delta\then(\ema{c}\tri\epsilon)=\id_\ema{c}$ can be drawn in polyboxes like so:
\[
\begin{tikzpicture}
	\node (1) {
        \begin{tikzpicture}[polybox, tos]
        	\node[poly, dom, "$\ema{c}$" left] (r) {\at$s$};
        	\node[poly, cod, right=2 of r.south, yshift=-2.5ex, "$\ema{c}$" below] (p) {\at$s$};
	...
	\node[right=1.8 of 1] (2) {
        \begin{tikzpicture}[polybox, tos]
          	\node[poly, dom, "$\ema{c}$" left] (c) {\at$s$};
          	\node[poly, cod, right=of c, "$\ema{c}$" right] (c') {\at$s$};
          	\draw[double] (c_pos) -- (c'_pos);
          	\draw[double] (c'_dir) -- (c_dir);
	    \end{tikzpicture}
	};
...
We therefore label these arrows as follows:
\[
\begin{tikzpicture}[polybox, tos]
...
So $\delta$ takes an object $i\in\Ob\cat{C}$ and a morphism $f\colon i\to\_$, then assigns another object $\cod f\in\Ob\cat{C}$ to be the codomain of $f$.
It then takes a second morphism $g\colon\cod f\to\_$ and assigns a third morphism $f\then g\colon i\to\_$ to be the composite of $f$ and $g$.
...
As with the identity morphism, we don't know what the codomain of this composite morphism is yet, but we do know that the domain of $f\then g$ matches the domain of $f$.
...
\end{tikzpicture}
\]
First it's saying that $\delta_1(i)=i$. This is great news; it means we can forget about $\delta_1$, just as we said earlier. Second it's saying that $d\then\idy(\cod(d))=d$. Unpacking, this means that composing a morphism $d$ with the identity morphism on its codomain returns $d$. It's neat to watch the comonoid laws declaring the standard laws of categories.
...
%\end{tikzpicture}
\]
Ah, it's saying that it wants $\cod(\idy(i))=i$, which makes sense---the codomain of the identity on $i$ should be $i$---and that it wants $\idy(i)\then d=d$, i.e.\ that composing the identity on $i$ with $d$ should return $d$.
