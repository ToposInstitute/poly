...
\subsection{Polyboxes in action} %** Maybe actually leave this out, and move the \otimes\to\tri stuff later, to "interaction with parallel products"

We end this section with a few simple demonstrations of how to define lenses and perform computations using polyboxes.
...
\[
\begin{tikzpicture}[polybox, mapstos]
	\node[poly, dom, "$p\otimes q$" left] (p) {$(d,e)$\nodepart{two}$(i,j)$};
	\node[poly, cod, "$p$" right, right= 1.5cm of p.south, yshift=-1ex] (q) {$d$\nodepart{two}$i$};
	\node[poly, cod, "$q$" right, above=of q] (r) {$e$\nodepart{two}$j$};
  	\draw (p_pos) to[first] (q_pos);
  	\draw (q_dir) to[climb] (r_pos);
  	\draw (r_dir) to[last] (p_dir);
\end{tikzpicture}
...
But in the polyboxes above, $j$ is not allowed to depend on $d$ in $p\otimes q$ on the left, so the arrow from the positions box of $q$ to the directions box of $p$ on the right doesn't actually take $d$ into account at all.
...
%** Draw it out exercise?

%** This is a little out of place right now
\begin{exercise}
...
%** Any other good polybox examples??
We'll see several more examples of polybox computations throughout the rest of the book.
...
\begin{exercise}
Let $A$ and $B$ be sets, and let $p$ be a polynomial.
Show that
\begin{equation}\label{eqn.monomials_and_comp}
	\poly(A\yon^B,p)\iso\smset(A,p(B)).
\end{equation}
\begin{solution}
Given $A,B\in\smset$ and $p\in\poly$, we have by \eqref{eqn.lchom_def} that
\[
    \lchom{B}{A}=\sum_{i\in A}\yon^{B}\iso A\yon^B,
\]
so by \eqref{eqn.lchom_adj_iso}, we have that
\[
...
\begin{enumerate}
    ...
    **% check functoriality
    ...
    function obtained by applying the functor $q$ to the corresponding on-directions function $g^\sharp_i\colon p'[g_\1(i)]\to p[i]$.
    **% check functoriality
\end{enumerate}
\end{solution}
\end{exercise}
...
% **Turn this into an exercise. Then revisit after indexed right coclosure later.
...
%** For ease of notation, we suppress...when clear from context...so for example, in ** below...
\begin{proposition}
Given polynomials $p,q,r\in\poly$ and a lens $f\colon p\to q\tri\1$, there is a natural isomorphism
\[
    \poly/q\tri\1\left(p\To{f}q\tri\1, q\tri r\To{q\:\tri\:!}q\tri\1\right)\iso\poly\left(\rchom{q,p\To{f}q\tri\1}, r\right),
\]
where
\begin{equation} \label{eqn.rchom_def}
    \rchom{q,p\To{f}q\tri\1}\coloneqq\sum_{i\in p(\1)}q[f_\1(i)]\yon^{p[i]}.
\end{equation} %**Replace with frown notation
\end{proposition}
\begin{proof} %**Another exercise about functoriality?
...
\begin{tikzpicture}
  ...
  \begin{tikzpicture}[polybox, mapstos]
	...
	\node[poly, right=1.8 of p.south, yshift=-2.5ex, "$q$" below] (q) {$e$};
	\node[poly, above=.8 of q, "$r$" above] (r) {};
	\node[poly, cod, right=of q, "$q$" right] (q') {$e$};
  ...
  \begin{tikzpicture}[polybox, mapstos]
  	\node[poly, dom, "$p$" left] (p) {\at$i$};
  	\node[poly, cod, right=1.8 of p.south, yshift=-1ex, "$q$" right] (q) {$e$};
...
% , so specifying such a morphism amounts to specifying a position $k$ of $r$ and an on-directions function $r[k]\to p[i]$ for each $i\in p(\1)$ and $e\in q[f_\1(i)]$.
So morphisms from $f$ to $q\:\tri\:!$ are equivalent to gadgets
\[
\begin{tikzpicture}[polybox, mapstos]
    \node[poly, dom, "$p$" left] (p) {\at$i$};
    \node[poly, cod, right=1.8 of p.south, yshift=-2.5ex, "$q$" below] (q) {$e$};
    \node[poly, cod, above=.8 of q, "$r$" above] (r) {$d$};
    \draw (p_pos) to[first] node[below] {$f_\1$} (q_pos);
    \draw (q_dir) to[climb] node[right] {$g^r$} (r_pos);
    \draw (r_dir) to[last] node[above] {$g^\sharp$} (p_dir);
\end{tikzpicture}
\]
with $f_\1$ fixed.
But this, in turn, is equivalent to a gadget
\[
\begin{tikzpicture}[polybox, mapstos]
    \node[poly, dom] (h) {\at$(i,e)$};
    \node[poly, cod, "$r$" right, right=of h] (r) {$d$};
    \draw (h_pos) -- node[below] {$g^r$} (r_pos);
    \draw (r_dir) -- node[above] {$g^\sharp$} (h_dir);
\end{tikzpicture}
\] % ** center??
where the polyboxes on the left represent a polynomial whose positions are pairs $(i,e)$ with $i\in p(\1)$ and $e\in q[f_\1(i)]$, and whose directions at $(i,e)$ are the directions of the original polynomial $p$ at $i$.
This is precisely the polynomial we defined in \eqref{eqn.rchom_def}, so the isomorphism holds. % **Refactor a little/finish
\end{proof}
...
%** Replace this with right coclosure/slice argument
\end{proof}
...
\begin{proposition} %** Check this; if can be deduced similarly to the above, make it an exercise and put it at the end of the previous section instead of this one
