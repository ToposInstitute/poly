...
Let $\cat{C}$ be a category.
A functor $\pi\colon\cat{S}\to\cat{C}$ from some other category $\cat{S}$ to $\cat{C}$ is called a \emph{discrete opfibration over $\cat{C}$} if it satisfies the following condition:
\begin{itemize}
	\item given an object $s\in\cat{S}$, for every morphism $f\colon\pi(s)\to\_$ in $\cat{C}$, there exists a unique morphism $\ol{f}\colon s\to\_$ such that $\pi(\ol{f})=f$ (in particular, $\pi$ must send $\cod\ol{f}$ to $\cod f$).
\end{itemize}
\[
\begin{tikzcd}
...
\begin{enumerate}
	...
	\item Consider the discrete opfibrations $\elts^\cat{C}I\to\cat{C}$ and $\elts^\cat{C}J\to\cat{C}$ guaranteed by \cref{exc.copre_to_dopf}.
	Show that $\alpha$ is a morphism of these discrete opfibrations, in the sense of \cref{def.dopf}.
