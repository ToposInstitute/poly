...
We can draw what the target function is doing more compactly by taking the corollas of $\ema{s}$ and ``bending the leaves'' so that they point to their targets:
\[
\begin{tikzpicture}
    \node[circle,minimum size=2cm] (b) {};
    \foreach\x in{1,...,3}{
      \node[minimum size=0.75cm,draw,circle] (3-\x) at (b.{360/3*\x}){\x};
    }
    \foreach\x in{1,...,3}{
      \foreach\y in{1,...,3}{
        \ifnum\x=\y\draw[->] (3-\x) to [in=360/3*\x-15,out=360/3*\x+15,loop] ();\relax\else
          \draw (3-\x) edge[->,bend right=3] (3-\y);
        \fi
      }
    }
\end{tikzpicture}
